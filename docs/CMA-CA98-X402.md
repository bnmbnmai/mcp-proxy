# UK CMA CA98 infringement-decision text — x402 door (live-apply, not deployed)
...
Cherry-picked onto `cursor/ico-mpn-live-b565`. `/cma-ca98` is wired on this lineage but stays off well-known / OpenAPI / llms.txt / shop catalog / PUBLIC_BAZAAR_SKUS until a later live-apply deploy. Do not restart apollo. Do not deploy.
...
`data/cma-ca98/` is gitignored. This branch only ports the door. Do not collect on apollo from this PR. A later deploy is done when `GET https://ticks.bnm.farm/cma-ca98` is HTTP 402.
