...
`data/occ-cd/` is gitignored. Order bodies get lost on a dead VM — do not harvest the whole catalog here. Full fill waits for apollo. **This PR does not restart apollo and does not deploy.**
...
This cloud VM cannot publish `ticks.bnm.farm` and must not restart `idaho-ticks-x402`. Live `/occ-cd` stays **404** until a later live-apply. Do not collect on apollo from this PR.
...
OCC_CD_LIMIT=5 OCC_CD_MAX_FETCH=8 npm run collect:occ-cd
```
