...
Applied on apollo (2026-09-15): **serving SHA `f4c98c8`** (apply-record [PR 235](https://github.com/bnmbnmai/mcp-proxy/pull/235); cherry-pick of [PR 234](https://github.com/bnmbnmai/mcp-proxy/pull/234) `cursor/oalj-decisions-6296` @ `756aa16` onto `cursor/morning-collect-babysit-69a6` @ `ccf5710` / prior running `ff18566`). Restarted **only** `idaho-ticks-x402.service` at **2026-09-15 16:33:17 MDT** (PID 514536; was 3804817). Added `OALJ_DECISIONS_DIR` to the user unit. One-door collect EXIT 0: listedCount **79**, `addedThisRun` 4, `fetchedPdfs` 4, skipped 0, asOf **2026-07-30**, fetchedAt **2026-09-15T22:33:11.922Z**. First-slice: `2026-spa-00001` McLaughlin (OALJ SPA), `2025-spa-00006` Arms, `2025-spa-00007` McClane, `2026-0010` Sheffield (ARB). Scout seeds are examples, not a frozen list. Free manifest is case / board / date / institution + `paidUrl` only; **no** `sourceUrl`, no Decision/Order narrative. Unpaid `GET https://ticks.bnm.farm/oalj-decisions` is **402** at $0.05; `?id=2026-spa-00001` is **402** at $0.02. Empty `?since=` is unpaid **304**. `/.well-known/x402` lists `/oalj-decisions` among **60** doors (was 59). Banner `mcp /mcp — 60 tools`. Siblings `/oshrc-orders` `/fmshrc-orders` `/stb-decisions` `/epa-alj` `/ticks` still 402. payTo unchanged `0xf59621FC406D266e18f314Ae18eF0a33b8401004`. docker-gw / Caddy / tunnel untouched. No new collect cron. Lander card lives in tv-remote / bnm.farm and was not on this tip — apply is not blocked on a second repo.
