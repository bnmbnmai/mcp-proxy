...
Applied on apollo (2026-09-15): **serving SHA `a18f4c0`** (apply-record [PR 237](https://github.com/bnmbnmai/mcp-proxy/pull/237); merge of [PR 236](https://github.com/bnmbnmai/mcp-proxy/pull/236) onto `cursor/apply-oalj-decisions-a04c`; prior serving `f4c98c8`). Restarted **only** `idaho-ticks-x402.service` at **2026-09-15 22:12:58 MDT** (PID 821522; was 514536). `parseSnapshotPretty` + refuse-overwrite are in the running tree (`build/superfund-rods.js` via `/proc/821522/cwd`). Evening collect had wiped **730 → 24** when one-shot `JSON.parse` of the ~530MB bag failed. Public manifest still **200** cardCount **24**, asOf **2026-09-11**, fetchedAt **2026-09-16T02:28:53.720Z**. Unpaid `GET /superfund-rods` still **402** at $0.05 / `50000`. `/.well-known/x402` still **60** doors including `/oalj-decisions`. One-door PDF rebuild left running (PID **808540**, LIMIT=2500, MAX_FETCH=80, started 21:59 Boise; EPA HTTPS in flight; 801 PDFs on disk). docker-gw / Caddy / tunnel untouched. No shop USDC. Did not merge to catalog `main`.
