...
Always listed on well-known / OpenAPI / llms.txt / shop catalog once this apply branch is restarted on apollo. MCP at `/mcp` generates an `ofgem-enforcement` tool from live well-known (do not hardcode a door count). This listing/apply PR does **not** apply on apollo from the cloud VM. Unpaid `GET /ofgem-enforcement` is HTTP 402.
