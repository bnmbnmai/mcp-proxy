...
import { z } from "zod";
import { extraMcpToolNames, fetchLiveCatalog, FIRM_CHECK_TOOL_NAME, GET_ONE_TOOL_NAME, GET_PAGE_TOOL_NAME, LIVE_ORIGIN, MCP_PATH, PAY_TO, SEARCH_TOOL_NAME, } from "./shop-catalog.js";
const UA = "bnm-data-shop-mcp/1.0";
async function getJson(url, xPayment) {
    const headers = { Accept: "application/json", "User-Agent": UA };
    if (xPayment?.trim())
        headers["X-PAYMENT"] = xPayment.trim();
    const response = await fetch(url, { method: "GET", headers });
    return { status: response.status, body: await response.text() };
}
function paidText(url, status, body) {
...
function registerPaidTool(server, sku) {
    ...
    server.tool(sku.name, `GET ${LIVE_ORIGIN}${sku.path} — ${sku.price} USDC on Base to ${PAY_TO}. ${sku.bag} Unpaid returns HTTP 402.`, shape, async (args) => {
        const q = new URLSearchParams();
        if (typeof args.id === "string" && args.id.trim())
            q.set("id", args.id.trim());
        else if (typeof args.before === "string" && args.before.trim())
            q.set("before", args.before.trim());
        else if (typeof args.page === "string" && args.page.trim())
            q.set("page", args.page.trim());
        const qs = q.toString();
...
async function main() {
    ...
    for (const sku of skus)
        registerPaidTool(server, sku);
    server.tool(FIRM_CHECK_TOOL_NAME, "Free firm-name search across official caches. HTTP 200. Not a SKU.", { q: z.string().describe("Firm, institution, bank, company, or import-alert number. Free.") }, async ({ q }) => {
        const url = `${LIVE_ORIGIN}/firm-check?q=${encodeURIComponent(q)}`;
        const result = await getJson(url);
        return { content: [{ type: "text", text: paidText(url, result.status, result.body) }] };
    });
    server.tool(SEARCH_TOOL_NAME, "Free catalog search on an extracted-body door. Returns matching rows plus id and the ?id= URL. Not a paid SKU.", {
    ...
    server.tool(GET_PAGE_TOOL_NAME, "Paid get-page on an extracted-body door. Same URL as the door GET ($0.05). Not a new SKU.", {
        ...
        if (before?.trim())
            url.searchParams.set("before", before.trim());
        const result = await getJson(url.toString(), x_payment);
        return { content: [{ type: "text", text: paidText(url.toString(), result.status, result.body) }] };
    });
    server.tool(GET_ONE_TOOL_NAME, "Paid get-one on an extracted-body door. Same URL ?id= ($0.02). Not a new SKU.", {
        door: z.string().describe("Extracted-body door name."),
    ...
    console.error(`[bnm-data-shop] MCP tools from ${LIVE_ORIGIN}/.well-known/x402: ${skus.length} paid + ${extra.join(", ")}. Connect live: npx -y mcp-remote ${LIVE_ORIGIN}${MCP_PATH}`);
