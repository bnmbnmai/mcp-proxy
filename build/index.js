...
async function makeApolloRequest(endpoint, params) {
    ...
    try {
        ...
        if (response.status === 402) {
            ...
            return {
                data: null,
                error: `Payment required (x402).${previewText || ` Configure your agent with an x402-compatible wallet. See https://apolloai.team`}`,
                statusCode: 402,
            };
        }
        if (!response.ok) {
...
server.registerTool("web_scrape", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Scrape failed: ${result.error}` }], isError: true };
    }
...
server.registerTool("web_search", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Search failed: ${result.error}` }], isError: true };
    }
    const data = result.data;
    const parts = [
...
server.registerTool("x_search", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `X search failed: ${result.error}` }], isError: true };
    }
    const data = result.data;
...
server.registerTool("pain_points", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Pain points failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
    const parts = [
...
server.registerTool("agentic_trends", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Trends failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
    const parts = [
...
server.registerTool("keyword_opportunities", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Keyword opps failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
    const parts = [
...
server.registerTool("opportunity_bundle", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Bundle failed: ${result.error}` }], isError: true };
    }
...
server.registerTool("agent_intel", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Agent intel failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
    const parts = [
        `## Agent Economy Intel`,
...
server.registerTool("sentiment", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Sentiment failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
...
server.registerTool("micro_saas", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Micro-SaaS failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
...
server.registerTool("web3_hackathons", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Hackathons failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
...
server.registerTool("github_trending", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `GitHub trending failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
...
server.registerTool("producthunt", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Product Hunt failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
...
server.registerTool("weekly_digest", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Weekly digest failed: ${result.error}` }], isError: true };
    }
...
server.registerTool("agentic_insights_bundle", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Agentic insights bundle failed: ${result.error}` }], isError: true };
    }
...
server.registerTool("builder_intel_bundle", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Builder intel bundle failed: ${result.error}` }], isError: true };
    }
...
server.registerTool("proxy_fetch", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Proxy fetch failed: ${result.error}` }], isError: true };
    }
    const data = result.data;
...
server.registerTool("proxy_status", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Status check failed: ${result.error}` }], isError: true };
    }
    const data = result.data;
    const status = [
...
server.registerTool("crypto_prices", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Crypto prices failed: ${result.error}` }], isError: true };
    }
...
server.registerTool("crypto_trending", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Crypto trending failed: ${result.error}` }], isError: true };
    }
...
server.tool("ip_intel", "Multi-source IP intelligence — geolocation, open ports, vulnerabilities, threat classification from Shodan, GreyNoise, AlienVault OTX, ip-api. $0.03/query via x402.", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `IP intel failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
    const parts = [
...
server.tool("domain_intel", "Multi-source domain intelligence — DNS, SSL certificates, geo, threat analysis from CertSpotter, AlienVault OTX, ip-api. $0.03/query via x402.", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `Domain intel failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
    const parts = [
...
server.tool("fx_rates", "Live foreign exchange rates for 30+ currencies from ECB data. $0.005/query via x402.", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `FX rates failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
    const rates = d.rates || {};
...
server.tool("defi_yields", "Top DeFi yields across 18K+ pools from DefiLlama. Filter by chain, TVL, stablecoin. $0.03/query via x402.", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `DeFi yields failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
...
server.tool("defi_protocols", "Top DeFi protocols by TVL from DefiLlama. 7000+ protocols, category filtering. $0.02/query via x402.", {
    ...
    if (result.error) {
        return { content: [{ type: "text", text: `DeFi protocols failed: ${result.error}` }], isError: true };
    }
    const d = result.data;
