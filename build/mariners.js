...
export function marinersDir() {
    if (env("MARINERS_DIR"))
        return resolve(env("MARINERS_DIR"));
...
export function emptySnapshot(reason, editions = []) {
...
function pdfPathOverride() {
    return env("MARINERS_PDF_PATH");
}
export async function collectMariners() {
...
export function readSnapshot() {
    ...
    try {
        ...
        if (raw && raw.product === PRODUCT_ID && Array.isArray(raw.notices))
            return raw;
    }
    catch {
        /* corrupt */
...
export async function loadMariners() {
    ...
    try {
        ...
        return emptySnapshot(`USCG D13 LNM PDF is not on this host and live fetch failed. ${err instanceof Error ? err.message : String(err)}`);
    }
}
...
export function buildMarinersManifest(snap) {
...
export async function loadMarinersManifest() {
    const cached = readSnapshot();
    if (cached)
        return buildMarinersManifest(cached);
    try {
