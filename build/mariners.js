...
export const LNM_LISTING_URL = "https://www.navcen.uscg.gov/local-notices-to-mariners?district=13+0&subdistrict=n";
export const D11_LNM_LISTING_URL = "https://www.navcen.uscg.gov/local-notices-to-mariners?district=11+0&subdistrict=n";
export const D7_LNM_LISTING_URL = "https://www.navcen.uscg.gov/local-notices-to-mariners?district=7+0&subdistrict=n";
export const D8_LNM_LISTING_URL = "https://www.navcen.uscg.gov/local-notices-to-mariners?district=8+0&subdistrict=g";
export const D1_LNM_LISTING_URL = "https://www.navcen.uscg.gov/local-notices-to-mariners?district=1+0&subdistrict=n";
export const D5_LNM_LISTING_URL = "https://www.navcen.uscg.gov/local-notices-to-mariners?district=5+0&subdistrict=n";
export const D9_LNM_LISTING_URL = "https://www.navcen.uscg.gov/local-notices-to-mariners?district=9+0&subdistrict=n";
export const D14_LNM_LISTING_URL = "https://www.navcen.uscg.gov/local-notices-to-mariners?district=14+0&subdistrict=n";
export const D17_LNM_LISTING_URL = "https://www.navcen.uscg.gov/local-notices-to-mariners?district=17+0&subdistrict=n";
...
export const D1_SPEC = leftoverSpec({
...
export const D5_SPEC = leftoverSpec({
...
export const D9_SPEC = leftoverSpec({
...
export const D14_SPEC = leftoverSpec({
...
export const D17_SPEC = leftoverSpec({
...
export const LIVE_LNM_SPECS = [D13_SPEC, D11_SPEC, D7_SPEC, D8_SPEC];
/** Dual-hunt TAKE 2026-09-06 leftover districts (same walker, new paths). */
export const LEFTOVER_LNM_SPECS = [D1_SPEC, D5_SPEC, D9_SPEC, D14_SPEC, D17_SPEC];
export const ALL_LNM_SPECS = [...LIVE_LNM_SPECS, ...LEFTOVER_LNM_SPECS];
/** NavCEN weekly PDF prefix is two digits (`lnm01…`, `lnm07…`, `lnm08…g`, `lnm11…`, `lnm13…`). */
export function lnmPdfCode(district) {
    return district.padStart(2, "0");
}
export function lnmPdfFilename(district, week, year, infix = "") {
    return `lnm${lnmPdfCode(district)}${String(week).padStart(2, "0")}${infix}${year}.pdf`;
}
export const NOTICE_FIELDS = ["week", "section", "text", "sourceUrl"];
...
export const LNM_SECTIONS = [...AID_SECTIONS, MSI_SECTION];
...
export function marinersDir(spec = D13_SPEC) {
    if (env(spec.dirEnv))
        return resolve(env(spec.dirEnv));
    return resolve(join(homedir(), "projects/mcp-proxy/data", spec.cacheDir));
}
export function snapshotPath(spec = D13_SPEC) {
    return join(marinersDir(spec), "snapshot.json");
}
export function absolutePdfUrl(href) {
    if (href.startsWith("http://") || href.startsWith("https://"))
        return href;
    if (href.startsWith("/"))
        return `https://www.navcen.uscg.gov${href}`;
    return `${LNM_PDF_BASE}${href.replace(/^\.\//, "")}`;
}
export function parseListingHtml(html) {
    const found = [];
    const seen = new Set();
    const re = /week_lnm_format">(\d+)<\/span>\s*-\s*<span class="year_lnm_format">(\d{4})<\/span>[\s\S]{0,400}?href="([^"]+\.pdf)"/gi;
    for (const match of html.matchAll(re)) {
        const week = Number(match[1]);
        const year = Number(match[2]);
        const href = match[3];
        if (!Number.isFinite(week) || !Number.isFinite(year) || !href)
            continue;
...
export function latestEdition(editions) {
...
function isNoiseLine(line) {
    ...
    if (t === "\f" || t.includes("\f"))
        return true;
    if (/Maritime Safety Information Products/.test(t))
        return true;
    if (/https:\/\/navcen\.uscg\.gov\/msi/i.test(t))
        return true;
    if (PAGE_DATE_RE.test(t))
        return true;
    if (/^\d+\/\d+$/.test(t))
        return true;
    return false;
}
function matchSection(trimmed) {
...
function isTableHeader(trimmed) {
    return /\bNAME\b.*\bLLNR\b/.test(trimmed) || /\bTITLE\b.*\bSUBCATEGORY\b/.test(trimmed);
}
function isWaterwayLine(line, trimmed) {
    if (matchSection(trimmed) || isTableHeader(trimmed))
        return false;
    if (/Local Notice to Mariners/i.test(trimmed))
        return false;
    if (/^\d{2}\/\d{2}\/\d{4}$/.test(trimmed))
        return false;
    if (trimmed.length < 3 || trimmed.length > 55)
        return false;
    if (/\d{4,}/.test(trimmed))
        return false;
...
export function parseLnmText(text, meta) {
    const notices = [];
    let waterway = null;
    let section = null;
    let asOf = null;
    ...
    const flushMsi = () => {
        ...
        if (!body)
            return;
        if (/^(TITLE|SUBCATEGORY|DESCRIPTION|LOCATION)(\s|$)/i.test(body) && body.length < 80)
            return;
        notices.push({
            week: meta.week,
            section: MSI_SECTION,
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (isTableHeader(trimmed))
            continue;
        if (section && AID_SECTIONS.includes(section)) {
        ...
        if (section === MSI_SECTION) {
            if (/^(TITLE|SUBCATEGORY|DESCRIPTION|LOCATION)$/i.test(trimmed))
                continue;
            msiBuf.push(trimmed);
        }
    }
...
export function emptySnapshot(reason, editions = [], spec = D13_SPEC) {
...
async function fetchBytes(url, accept, spec) {
...
async function fetchText(url, spec) {
    const res = await fetch(url, {
...
function listingPath(spec) {
    return env(spec.listingPathEnv);
}
function pdfPathOverride(spec) {
    return env(spec.pdfPathEnv);
}
export async function collectMariners(spec = D13_SPEC) {
    ...
    const latest = latestEdition(editions);
    if (!latest) {
        const snap = emptySnapshot(`Official ${spec.districtName} listing had no weekly PDF links.`, [], spec);
...
export function readSnapshot(spec = D13_SPEC) {
    const path = snapshotPath(spec);
    if (!existsSync(path))
        return null;
    try {
        const raw = JSON.parse(readFileSync(path, "utf-8"));
        if (raw && raw.product === spec.productId && Array.isArray(raw.notices))
            return raw;
    }
    catch {
        /* corrupt */
...
export function writeSnapshot(snap, spec = D13_SPEC) {
    const path = snapshotPath(spec);
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, JSON.stringify(snap, null, 2) + "\n");
}
export async function loadMariners(spec = D13_SPEC) {
    ...
    try {
        ...
        return emptySnapshot(`USCG ${spec.districtName} LNM PDF is not on this host and live fetch failed. ${err instanceof Error ? err.message : String(err)}`, [], spec);
    }
}
export function sectionCounts(notices) {
...
export function buildMarinersManifest(snap, spec = D13_SPEC) {
...
export async function loadMarinersManifest(spec = D13_SPEC) {
    const cached = readSnapshot(spec);
    if (cached)
        return buildMarinersManifest(cached, spec);
    try {
...
export function collectMarinersD11() {
    return collectMariners(D11_SPEC);
}
export function loadMarinersD11() {
    return loadMariners(D11_SPEC);
}
export function loadMarinersD11Manifest() {
    return loadMarinersManifest(D11_SPEC);
}
export function collectMarinersD7() {
    return collectMariners(D7_SPEC);
}
export function loadMarinersD7() {
    return loadMariners(D7_SPEC);
}
export function loadMarinersD7Manifest() {
    return loadMarinersManifest(D7_SPEC);
}
export function collectMarinersD8() {
    return collectMariners(D8_SPEC);
}
export function loadMarinersD8() {
    return loadMariners(D8_SPEC);
}
export function loadMarinersD8Manifest() {
    return loadMarinersManifest(D8_SPEC);
}
export function collectMarinersD1() {
    return collectMariners(D1_SPEC);
}
export function loadMarinersD1() {
    return loadMariners(D1_SPEC);
}
export function loadMarinersD1Manifest() {
    return loadMarinersManifest(D1_SPEC);
}
export function collectMarinersD5() {
    return collectMariners(D5_SPEC);
}
export function loadMarinersD5() {
    return loadMariners(D5_SPEC);
}
export function loadMarinersD5Manifest() {
    return loadMarinersManifest(D5_SPEC);
}
export function collectMarinersD9() {
    return collectMariners(D9_SPEC);
}
export function loadMarinersD9() {
    return loadMariners(D9_SPEC);
}
export function loadMarinersD9Manifest() {
    return loadMarinersManifest(D9_SPEC);
}
export function collectMarinersD14() {
    return collectMariners(D14_SPEC);
}
export function loadMarinersD14() {
    return loadMariners(D14_SPEC);
}
export function loadMarinersD14Manifest() {
    return loadMarinersManifest(D14_SPEC);
}
export function collectMarinersD17() {
    return collectMariners(D17_SPEC);
}
export function loadMarinersD17() {
    return loadMariners(D17_SPEC);
}
export function loadMarinersD17Manifest() {
    return loadMarinersManifest(D17_SPEC);
}
export function specFromDistrict(raw) {
    const key = (raw ?? "").trim().replace(/^0+/, "") || "13";
    const hit = ALL_LNM_SPECS.find((spec) => spec.district === key);
    return hit ?? D13_SPEC;
}
export function specFromArgv(argv = process.argv) {
    const raw = argv.find((arg) => arg.startsWith("--district="))?.slice("--district=".length);
    return specFromDistrict(raw);
}
export function leftoverCollectArgv(spec) {
...
//# sourceMappingURL=mariners.js.map