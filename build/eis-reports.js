...
const CHROME_UA = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36";
export function paidBodyCatalogNote(paidPath, catalogLead) {
    const lead = catalogLead.trim().replace(/\.?$/, ".");
    return `${lead} Free index/search (?q=, optional before/date) stays free and includes id, the ?id= URL ($0.02), and the page cursor ($0.05). GET ${paidPath}?id= is one official text GET ?id= ($0.02). Plain paid GET ${paidPath} is the newest 10 official texts; older chunk if they ask (?before=<id or date>, another $0.05); newer than a watermark ?since=<ISO timestamp or official catalog id> ($0.05; empty new set is 304 or recordCount 0).`;
}
...
function storeCookies(jar, res) {
    ...
    const raw = setter.length ? setter : (res.headers.get("set-cookie") ? [res.headers.get("set-cookie")] : []);
...
async function cdxRequest(url, jar, init, hops = 0) {
    if (hops > 8)
        throw new Error(`CDX redirect hop limit: ${url}`);
    const headers = new Headers(init?.headers);
    if (!headers.has("User-Agent"))
        headers.set("User-Agent", HTTP_UA);
...
export const SEED_LISTINGS = [
    {
        ...
        attachmentTitle: "Supplemental Environmental Impact Statement for a Construction Permit at the Clinch River Nuclear Site, Final Report.pdf",
...
export function eisReportsDir() {
    if (env("EIS_REPORTS_DIR"))
        return resolve(env("EIS_REPORTS_DIR"));
...
export function detailsUrlFor(eisId) {
    return `${DETAILS_URL}?eisId=${eisId}`;
}
export function sourceUrlFor(eisId, attachmentId) {
    return `${DETAILS_URL}?eisId=${eisId}#attachment-${attachmentId}`;
}
export function formItem(html, label) {
    const re = new RegExp(`<h4[^>]*>\\s*${label}\\s*</h4>\\s*([^<]+)`, "i");
    return decodeHtml((html.match(re) || [])[1] || "");
}
export function isCommentLetterTitle(title) {
    return /\b(comment letter|epa comments on)\b/i.test(title);
}
export function isSummaryTeaserTitle(title) {
    const t = title.trim();
    if (/^summary for the\b/i.test(t))
        return true;
    if (/\bhighlights\b/i.test(t) && !/\benvironmental impact statement\b/i.test(t))
        return true;
    if (/^executive summary\b/i.test(t))
        return true;
    return false;
}
export function isSkippedEisAttachment(title) {
    if (!title)
        return true;
    if (isCommentLetterTitle(title))
        return true;
    if (isSummaryTeaserTitle(title))
        return true;
    if (/\.(png|jpe?g|gif|svg|csv|html?)$/i.test(title))
        return true;
    return false;
}
export function cardIdFor(ceqNumber, attachmentId, keptCount) {
    return keptCount === 1 ? ceqNumber : `${ceqNumber}-${attachmentId}`;
}
export function parseAttachmentSection(html, heading) {
...
export function parseSearchDownloadEis(html) {
    const m = String(html || "").match(/startDownload\(\s*['"]downloadEisDocuments['"]\s*,\s*['"]\d+['"]\s*,\s*['"]([^'"]*)['"](?:\s*,\s*['"]([^'"]*)['"])?/i);
...
export function parseDetailsListings(html, pageUrl, prior) {
    const official = officialEisPageUrl(pageUrl) || officialEisPageUrl(prior?.pageUrl);
    const eisId = (official && new URL(official).searchParams.get("eisId")) ||
        prior?.eisId ||
...
export function parseListingRows(rows) {
    ...
    for (const row of rows) {
        if (!row.id || !row.ceqNumber || !row.eisId)
            continue;
        if (!row.attachmentId && !row.downloadGroups)
            continue;
        if (isSkippedEisAttachment(row.attachmentTitle || row.title))
            continue;
        if (seen.has(row.id))
            continue;
        seen.add(row.id);
...
export function isChromeEisHtml(html) {
    const t = String(html || "");
    if (t.length < 200)
        return false;
...
export function looksLikeLeakedEisBody(text) {
    const t = String(text || "");
    if (t.trimStart().startsWith("%PDF-"))
        return true;
    if (t.includes(BODY_NEEDLE_CLINCH) || t.includes(BODY_NEEDLE_F35))
        return true;
    if (/\bML26035A285\b/.test(t) || /Supplement to NUREG-2226/i.test(t))
        return true;
    return false;
}
export function looksLikeLoginGov(htmlOrUrl) {
    const t = String(htmlOrUrl || "");
    return /login\.gov|secure\.login\.gov|idp\.int\.identitysandbox\.gov/i.test(t);
}
export function looksLikeCdxLogin(htmlOrUrl) {
    const t = String(htmlOrUrl || "");
    return /cdx\.epa\.gov\/CDX\/Login|name=["']userId["']|Sign in to CDX/i.test(t);
}
export function looksLikeLoginWall(htmlOrUrl) {
    return looksLikeLoginGov(htmlOrUrl) || looksLikeCdxLogin(htmlOrUrl);
}
export function looksLikeLeftoverRecaptchaScript(html) {
    const t = String(html || "");
    if (!/recaptcha\/api\.js/i.test(t))
        return false;
    if (/class=["'][^"']*g-recaptcha|grecaptcha\.render|recaptcha\/api2\/anchor|h-captcha|hcaptcha\.com\/1\/api/i.test(t)) {
        return false;
    }
    if (/Please complete the (reCAPTCHA|hCaptcha|picture)/i.test(t))
        return false;
    return true;
}
export function looksLikeHumanCaptcha(html) {
    const t = String(html || "");
    if (/Please complete the (reCAPTCHA|hCaptcha|picture)/i.test(t))
        return true;
    const visibleHuman = /class=["'][^"']*g-recaptcha|grecaptcha\.render|recaptcha\/api2\/anchor|h-captcha|hcaptcha\.com\/1\/api/i.test(t);
    if (!visibleHuman)
        return false;
    if (/altcha-widget/i.test(t) && /downloadFormCaptcha|Please check the altcha/i.test(t))
        return false;
    return true;
}
export function isSuperfundRodDump(text) {
    const t = String(text || "");
    if (!/RECORD OF DECISION/i.test(t))
        return false;
    if (!/superfund|SEMS|semspub\.epa\.gov/i.test(t))
        return false;
    return !/Environmental Impact Statement/i.test(t);
}
export function isRealEisBody(text) {
    const raw = String(text || "");
    const t = raw.replace(/\s+/g, " ").trim();
    if (t.length < MIN_BODY_CHARS)
        return false;
    if (isChromeEisHtml(raw) && /<[a-z][\s\S]*>/i.test(raw))
        return false;
    if (isSuperfundRodDump(t))
        return false;
    if (isCommentLetterTitle(t.slice(0, 240)))
        return false;
    if (/Please check the ALTCHA/i.test(t))
        return false;
    const eisPhrase = /Environmental Impact Statement/i.test(t) ||
        /\bNUREG-\d+/i.test(t) ||
...
export function parseEisReportText(text, meta) {
...
export function solveAltchaPow(challenge) {
    ...
    for (let n = 0; n <= cap; n += 1) {
        if (createHash("sha256").update(`${salt}${n}`).digest("hex") === target)
            return n;
    }
    throw new Error("ALTCHA PoW unsolved");
}
export function altchaPayload(challenge, number, took) {
...
export async function fetchAltchaChallenge(naasIp, naasToken) {
    ...
    if (!res.ok)
        throw new Error(`ALTCHA challenge HTTP ${res.status}`);
    const raw = (await res.json());
    if (!raw?.challenge || !raw.salt || !raw.signature)
        throw new Error("ALTCHA challenge missing fields");
    return raw;
}
...
export function emptyEisReportsSnapshot(reason) {
    return {
        ok: true,
        product: PRODUCT_ID,
        status: "empty",
...
export function assembleEisReportsSnapshot(cards, fetchedAt = new Date().toISOString()) {
    const kept = cards
...
export function readEisReportsSnapshot() {
...
export function writeEisReportsSnapshot(snap) {
    const path = snapshotPath();
...
export async function fetchEisText(url, jar) {
    ...
    while (Date.now() - start < timeoutMs) {
        ...
        if (hit) {
            ...
            try {
                ...
                if ((isPdfBytes(bytes) || isZipBytes(bytes)) && bytes.byteLength > 80)
                    return path;
            }
            catch {
                /* still writing */
...
export function pickMainEisPdf(paths) {
    const scored = paths
...
export function unpackEisDownload(downloadPath, destPath) {
    ...
    if (!isZipBytes(bytes))
        throw new Error(`${downloadPath} is neither a PDF nor a zip`);
    ...
    if (!main)
        throw new Error(`zip ${downloadPath} had no public EIS PDF`);
    const out = new Uint8Array(readFileSync(main));
    mkdirSync(dirname(destPath), { recursive: true });
    writeFileSync(destPath, out);
    return out;
}
async function injectAltchaAndStart(page, packed) {
    await page.evaluate((args) => {
        const cap = document.querySelector("#downloadFormCaptcha");
        if (!cap)
            throw new Error("downloadFormCaptcha missing");
        ...
        const start = window.startDownload;
        if (!start)
            throw new Error("startDownload missing");
    ...
    try {
        ...
        const tryStart = async (url, mode) => {
            ...
            if (looksLikeHumanCaptcha(html)) {
                throw new Error("BLOCKER: second human captcha (reCAPTCHA/hCaptcha) on CDX e-NEPA. Stop. Do not ask Bruce unless it is truly human-only and this is that case.");
            }
        ...
        throw new Error(`SKIP: no public EIS PDF after PoW for eisId=${opts.eisId} (login wall or empty download). captchaTookMs=0`);
    }
    finally {
        await browser.close();
    }
...
async function loadOfficialListings(dir) {
    ...
    if (dir) {
        ...
        if (searchHtml)
            fromSearch.push(...parseSearchRows(searchHtml));
    }
    else {
        try {
    ...
    const seenEis = new Set();
    for (const row of seedsAndSearch) {
        if (seenEis.has(row.eisId))
            continue;
        ...
        const pageHtml = dir
            ...
            : null;
        if (pageHtml) {
            enriched.push(...parseDetailsListings(pageHtml, row.pageUrl, row).map((c) => ({
                ...c,
        ...
        try {
            ...
            if (looksLikeLoginGov(liveHtml)) {
                if (row.downloadGroups || row.attachmentId)
                    enriched.push(row);
                continue;
            }
            ...
            if (String(err).includes("BLOCKER") || String(err).includes("KILL"))
                throw err;
            if (row.attachmentId || row.downloadGroups)
                enriched.push(row);
        }
...
export async function collectEisReports(opts) {
    ...
    for (const row of allListed) {
        ...
        try {
            ...
            if (msg.includes("BLOCKER") || msg.includes("KILL"))
                throw err;
...
export async function loadEisReports() {
    ...
    try {
        ...
        return emptyEisReportsSnapshot(`EPA NEPA EIS texts are not on this host and live fetch failed. ${err instanceof Error ? err.message : String(err)}`);
    }
}
export function buildEisReportsManifest(snap) {
    const cards = (snap?.cards ?? []).filter((c) => isRealEisBody(c.body));
    return {
        product: PRODUCT_ID,
        name: PRODUCT_NAME,
        free: true,
        note: paidBodyCatalogNote(EIS_REPORTS_PATH, "Count + CEQ number + date + title + agency + official e-NEPA page only. EIS body is the paid GET /eis-reports payload. This free manifest lists the full catalog. Search/details HTML is chrome. Skip EPA comment letters and Summary-for-the teasers. Distinct from Superfund RODs."),
        license: LICENSE,
        attribution: ATTRIBUTION,
        payTo: PAY_TO,
        network: "base",
...
export function filterEisReportsManifest(manifest, q) {
    ...
    const matched = cards.filter((raw) => {
        if (!raw || typeof raw !== "object")
            return false;
        const row = raw;
        return ["id", "ceqNumber", "eisId", "title", "agency", "state", "date", "pageUrl", "documentType"].some((k) => String(row[k] ?? "")
            .toLowerCase()
            .includes(needle));
    });
    return { ...manifest, cardCount: matched.length, cards: matched, q: needle };
}
export async function loadEisReportsManifest(q) {
    return filterEisReportsManifest(buildEisReportsManifest(readEisReportsSnapshot()), q);
}
export function selectEisReportCard(snap, opts) {
    const all = snap?.cards ?? [];
    const one = opts?.id?.trim();
    if (one) {
        const needle = one.toLowerCase();
        return (all.find((c) => c.id.toLowerCase() === needle ||
            c.ceqNumber === one ||
            c.attachmentId === one ||
            c.eisId === one) ?? null);
    }
    if (opts?.before?.trim()) {
        ...
        const idx = all.findIndex((c) => c.id.toLowerCase() === before || c.ceqNumber === opts.before || c.date === opts.before);
...
//# sourceMappingURL=eis-reports.js.map