...
export function shopPaidJsonSample() {
    return {
        example: true,
        comment: "Static canned examples of paid JSON keys. HTTP 200. Not live cache data. Not a wrap of the current table. Not a paid SKU.",
...
function sampleFieldString(value) {
    return typeof value === "string" ? value : value == null ? "" : String(value);
}
function sampleFieldNumber(value) {
    return typeof value === "number" && Number.isFinite(value) ? value : null;
}
/** Live-shaped sample object. Price comes from the official tick, never a placeholder. */
export function sampleFromTickRow(row, seriesLabel = "") {
    return {
        sample: true,
 ...
 */
export function selectTicksManifestSamples(ticks, seriesLabelById = new Map()) {
    const samples = [];
    const used = new Set();
