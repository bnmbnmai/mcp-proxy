...
export function leftoverDir() {
    if (env("TICKS_AMS_LEFTOVER_DIR"))
        return resolve(env("TICKS_AMS_LEFTOVER_DIR"));
...
export function jsonDumpsBody(raw) {
    const text = raw.trim();
    ...
    try {
        ...
        if (blob.includes("access is denied") || blob.includes("errorcode\":403"))
            return false;
...
export async function leakTestSlug(slug) {
...
export async function fetchOfficialPdf(slug) {
...
export async function collectLeftovers(reports = AMS_LEFTOVER_REPORTS) {
    const rows = [];
    const failed = [];
    const leakSkipped = [];
    const sources = [];
...
//# sourceMappingURL=ticks-ams-leftovers.js.map