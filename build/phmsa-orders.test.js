...
import { ATTRIBUTION, CARD_FIELDS, HUB_URL, LICENSE, PHMSA_ORDERS_MANIFEST_PATH, PHMSA_ORDERS_PATH, SEED_LISTINGS, buildPhmsaOrdersManifest, buildPhmsaOrdersPaidPage, collectPhmsaOrders, filterPhmsaOrdersManifest, isOfficialPhmsaOrderPdf, isOperatorResponseRow, isPeopleRow, isRealPhmsaOrderBody, officialPhmsaOrderPdfUrl, parseHubHtml, parseListingRows, parsePhmsaOrderText, } from "./phmsa-orders.js";
const fixtures = join(dirname(fileURLToPath(import.meta.url)), "../src/fixtures/phmsa-orders");
const AMOCO_PDF = "https://primis.phmsa.dot.gov/enforcement-documents/32026023CAO/32026023CAO_Corrective%20Action%20Order_08032026_(26-379109)_text.pdf";
const GULF_PDF = "https://primis.phmsa.dot.gov/enforcement-documents/32026020CAO/32026020CAO_Corrective%20Action%20Order_05162026_(26-372495).pdf";
const NAV_PDF = "https://primis.phmsa.dot.gov/enforcement-documents/42026012NOPV/42026012NOPV_Final%20Order_04072026_(23-266706).pdf";
const VALERO_PDF = "https://primis.phmsa.dot.gov/enforcement-documents/42026004NOPV/42026004NOPV_Final%20Order_04012026_(25-329817).pdf";
const ETC_PDF = "https://primis.phmsa.dot.gov/enforcement-documents/42025049NOPV/42025049NOPV_Final%20Order_03262026_(24-296979).pdf";
const ENT_PDF = "https://primis.phmsa.dot.gov/enforcement-documents/42026005CAO/42026005CAO_Corrective%20Action%20Order_02172026_(26-364755).pdf";
...
async function main() {
    ...
    assert.equal(listing.some((r) => /operator.response|NARRATIVE|Raw Data/i.test(r.sourceUrl)), false, "skip operator-response, incident NARRATIVE, and primis TSV");
    ...
    assert.equal(officialPhmsaOrderPdfUrl("https://primis.phmsa.dot.gov/enforcement-data/operator/395"), null, "enforcement-data TSV/operator card is not the sold PDF");
    assert.equal(officialPhmsaOrderPdfUrl("https://primis.phmsa.dot.gov/enforcement-documents/PHMSA%20Pipeline%20Enforcement%20Raw%20Data.txt"), null, "primis TSV / Raw Data.txt is dates/penalties only — not sold");
    assert.equal(officialPhmsaOrderPdfUrl("https://primis.phmsa.dot.gov/enforcement-documents/42026012NOPV/42026012NOPV_Operator_Response_03192026.pdf"), null, "operator-response PDF is not this SKU");
    assert.equal(officialPhmsaOrderPdfUrl("https://primis.phmsa.dot.gov/enforcement-documents/27nc-rsge/PHMSA_incident_NARRATIVE.zip"), null, "killed incident NARRATIVE zip is not this SKU");
    assert.equal(officialPhmsaOrderPdfUrl("https://cms.ferc.gov/sites/default/files/2026-04/20260417-195FERC61048-IN25-6-000.pdf"), null, "FERC PDFs stay on /ferc-orders");
    ...
    assert.equal(isOperatorResponseRow({
    ...
    assert.equal(manifest.cards[0]?.institution, "Amoco Oil Company");
    assert.equal(manifest.cards[0]?.id, AMOCO_ID);
    ...
    assert.ok(!("body" in (manifest.cards[0] ?? {})));
    ...
    assert.equal(searched.cards[0]?.id, AMOCO_ID);
    ...
    assert.equal(paidOne.records[0]?.type, "phmsa-orders");
    ...
    try {
        ...
        if (prevDir === undefined)
            delete process.env.PHMSA_ORDERS_DIR;
        else
            process.env.PHMSA_ORDERS_DIR = prevDir;
    }
...
//# sourceMappingURL=phmsa-orders.test.js.map