...
export function importAlertsDir() {
    if (env("IMPORT_ALERTS_DIR"))
        return resolve(env("IMPORT_ALERTS_DIR"));
...
function sliceListSection(html, list) {
    ...
    let end = rest.length;
    if (endOther > 0)
        end = Math.min(end, endOther);
    if (endScript > 0)
        end = Math.min(end, endScript);
    return { html: rest.slice(0, end), hasHeading: true };
}
function parseDatePublished(chunk) {
    const m = chunk.match(/Date Published\s*:\s*([0-9]{2}\/[0-9]{2}\/[0-9]{4})/i);
    return m ? m[1] : "";
}
function parseProducts(chunk) {
    const out = [];
    const re = /<div class="floatleft"><em>[\s\S]*?<\/em>\s*([^<]*?)(?:<br\s*\/?>)?\s*<\/div>\s*<div class="floatright[^"]*">\s*Date Published:\s*([0-9]{2}\/[0-9]{2}\/[0-9]{4})/gi;
    let m;
    while ((m = re.exec(chunk))) {
        ...
        if (product)
            out.push({ product, datePublished: m[2] });
    }
    return out;
}
export function parseFirmSection(html, list, meta) {
    const slice = sliceListSection(html, list);
    if (!slice.hasHeading)
        return { rows: [], hasHeading: false };
    const rows = [];
    let country = "";
    ...
    for (const token of tokens) {
        ...
        if (!/<div class="div-info">/i.test(token))
            continue;
        if (countryM)
            country = stripTags(countryM[1]);
...
export function parseAlertPage(html, pageId, catalogRow) {
...
export function sampleRowsFrom(rows, n = 2) {
    return rows.slice(0, n).map((r) => ({ ...r, sample: true }));
}
export function buildManifest(catalog, samples) {
    return {
...
export function assembleSnapshot(catalog, parsed, fetchedAt = new Date().toISOString()) {
    const ticks = parsed.flatMap((p) => p.rows.map(publicRow));
    const alerts = parsed.map((p) => p.summary);
    const asOf = alerts.map((a) => a.asOf).filter((d) => Boolean(d)).sort().at(-1) ?? null;
    return {
        ok: true,
        product: "fda-import-alerts",
...
export function readSnapshot() {
    ...
    try {
        ...
        if (raw && raw.product === "fda-import-alerts" && Array.isArray(raw.ticks))
            return raw;
    }
    catch {
        /* corrupt */
...
export async function collectImportAlerts(opts) {
    const dir = opts?.htmlDir ?? htmlDir();
    const pauseMs = opts?.pauseMs ?? (dir ? 0 : 1500);
    const catalogHtml = readHtmlDirFile(dir, ["ialist.html", "ialist-excerpt.html"]) ?? (await fetchFdaHtml(FDA_CATALOG_URL));
    ...
    for (const slice of FIRST_SLICE) {
        ...
        try {
            const html = readHtmlDirFile(dir, [`ia_${slice.pageId}.html`, `importalert_${slice.pageId}.html`]) ??
...
export async function loadImportAlerts() {
    ...
    try {
        ...
        return emptySnapshot(`FDA HTML is not on this host and live fetch failed. ${err instanceof Error ? err.message : String(err)}`);
    }
}
export function manifestFromSnapshot(snap, catalogHtml) {
...
export async function loadManifest() {
    const cached = readSnapshot();
    if (cached)
        return manifestFromSnapshot(cached);
    try {
        const dir = htmlDir();
        const catalogHtml = readHtmlDirFile(dir, ["ialist.html", "ialist-excerpt.html"]) ?? (await fetchFdaHtml(FDA_CATALOG_URL));
...
//# sourceMappingURL=import-alerts.js.map