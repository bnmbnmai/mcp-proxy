...
import { AAIB_REPORTS_MANIFEST_PATH, AAIB_REPORTS_PATH, ATTRIBUTION, CARD_FIELDS, INDEX_URL, LICENSE, SEED_LISTINGS, buildAaibReportsManifest, buildAaibReportsPaidPage, collectAaibReports, filterAaibReportsManifest, isOfficialAaibReportPdf, isRealAaibReportBody, officialAaibPageUrl, officialAaibReportPdfUrl, parseAaibReportText, parseContentDocument, parseListingRows, parseSearchJson, } from "./aaib-reports.js";
...
const EUROFOX_PDF = "https://assets.publishing.service.gov.uk/media/6a730cd0de77e2943cd3bbe8/Eurofox_2K_G-CMAX_09-26.pdf";
...
async function main() {
    ...
    assert.equal(fromSearch.some((r) => /annual-safety-review|raib-reports/i.test(r.pageUrl)), false, "skip Annual Safety Review and RAIB");
    ...
    assert.equal(officialAaibPageUrl("https://www.gov.uk/aaib-reports/aaib-annual-safety-review-2025"), null, "Annual Safety Review is not an investigation-report page");
    assert.equal(officialAaibPageUrl("https://www.gov.uk/raib-reports/some-rail-accident"), null, "RAIB stays PARK");
    assert.equal(officialAaibReportPdfUrl("https://assets.publishing.service.gov.uk/media/6a730cf4332792bd88ece534/Glossary_of_abbreviations.pdf"), null, "glossary PDF is not sold");
    ...
    const contentDoc = parseContentDocument(JSON.stringify({
        title: "AAIB investigation to Eurofox 2K, G-CMAX",
    ...
    const snapCards = SEED_LISTINGS.map((row) => parseAaibReportText(readFx(`${row.id}.txt`), row));
    ...
    assert.equal(manifest.cards[0]?.id, EUROFOX_ID);
    ...
    assert.equal(searched.cards[0]?.id, EUROFOX_ID);
    ...
    assert.equal(paidOne.records[0]?.type, "aaib-reports");
    ...
    try {
        ...
        if (prevDir === undefined)
            delete process.env.AAIB_REPORTS_DIR;
        else
            process.env.AAIB_REPORTS_DIR = prevDir;
    }
...
//# sourceMappingURL=aaib-reports.test.js.map