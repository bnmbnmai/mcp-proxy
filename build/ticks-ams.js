...
export const MNREPORTS_PDF = (slug) => `https://www.ams.usda.gov/mnreports/AMS_${slug}.pdf`;
export const VIEW_REPORT = (slug) => `https://mymarketnews.ams.usda.gov/viewReport/${slug}`;
 ...
 */
export const AMS_NATIONAL_REPORTS = [
...
export function amsNationalDir() {
    if (env("TICKS_AMS_DIR"))
        return resolve(env("TICKS_AMS_DIR"));
    return resolve(join(homedir(), "projects/mcp-proxy/data/ticks-ams"));
}
export function amsSnapshotPath(dir = amsNationalDir()) {
...
export function parseReportDate(text) {
    const weeksEnding = text.match(/Weeks Ending:\s+\d{1,2}\/\d{1,2}\/\d{4}\s*[-–]\s*(\d{1,2}\/\d{1,2}\/\d{4})/i);
    if (weeksEnding)
        return parseMdY(weeksEnding[1]);
    const endingNamed = text.match(/week ending\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}),\s+(\d{4})/i);
    if (endingNamed) {
        const mon = MONTHS[endingNamed[1].toLowerCase()];
        if (mon)
            return `${endingNamed[3]}-${mon}-${endingNamed[2].padStart(2, "0")}`;
    }
    const ending = text.match(/week ending\s+(\d{1,2}\/\d{1,2}\/\d{4})/i);
    if (ending)
        return parseMdY(ending[1]);
    const livestockThru = text.match(/Livestock Weighted Average Report for\s+\d{1,2}\/\d{1,2}\/\d{4}\s*-\s*(\d{1,2}\/\d{1,2}\/\d{4})/i);
    if (livestockThru)
        return parseMdY(livestockThru[1]);
    const hayAuction = text.match(/Hay Auction Weighted Average Report for\s+(\d{1,2}\/\d{1,2}\/\d{4})/i);
    if (hayAuction)
        return parseMdY(hayAuction[1]);
    const organicThru = text.match(/Report for\s+\d{1,2}\/\d{1,2}\/\d{4}\s*-\s*(\d{1,2}\/\d{1,2}\/\d{4})/i);
    if (organicThru)
        return parseMdY(organicThru[1]);
    const periodThru = text.match(/Information for the period\s+[A-Za-z]+\s+\d{1,2}\s*[-–]\s*(\d{1,2}),\s+(\d{4})/i);
    if (periodThru) {
        const named = text.match(/Information for the period\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}\s*[-–]\s*(\d{1,2}),\s+(\d{4})/i);
        if (named) {
    ...
    const weekOf = text.match(/WEEK OF\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}\s*[-–]\s*(\d{1,2}),\s+(\d{4})/i);
    if (weekOf) {
        const mon = MONTHS[weekOf[1].toLowerCase()];
        if (mon)
            return `${weekOf[3]}-${mon}-${weekOf[2].padStart(2, "0")}`;
    }
    const monthThru = text.match(/\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}\s*[-–]\s*(\d{1,2}),\s+(\d{4})\b/i);
    if (monthThru) {
        const mon = MONTHS[monthThru[1].toLowerCase()];
        if (mon)
            return `${monthThru[3]}-${mon}-${monthThru[2].padStart(2, "0")}`;
    }
    const grain = text.match(/Grain Report for\s+(\d{1,2}\/\d{1,2}\/\d{4})/i);
    if (grain)
        return parseMdY(grain[1]);
    const woolThru = text.match(/Report For:\s*\d{1,2}\/\d{1,2}\/\d{4}\s+(?:thru|to)\s+(\d{1,2}\/\d{1,2}\/\d{4})/i);
    if (woolThru)
        return parseMdY(woolThru[1]);
    const named = text.match(/\b(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)?\s*(January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[a-z]*\s+(\d{1,2}),\s+(\d{4})\b/i);
    if (named) {
        const mon = MONTHS[named[1].toLowerCase()];
        if (mon)
            return `${named[3]}-${mon}-${named[2].padStart(2, "0")}`;
...
export function esmisPdfUrls(html, slug, pdfNames = []) {
...
export function latestEsmisPdfUrl(html, slug) {
    return esmisPdfUrls(html, slug)[0] ?? null;
}
export function esmisPublicationUrl(report) {
    return `${ESMIS_HOST}/publication/${report.esmisPublication}`;
}
export function mnreportsPdfUrls(slug, pdfNames = []) {
...
export function officialPdfCandidateOrder(slug, esmisUrls = [], pdfNames = []) {
    return [...new Set([...mnreportsPdfUrls(slug, pdfNames), ...esmisUrls])];
}
const PACKAGE_RE = /\b(Large Square(?:\s+[34]x4)?|Medium Square(?:\s+3x3)?|Small Square(?:\s+3 Tie)?|Large Round|Standing)\b/i;
function hayRegion(line) {
    const m = line.match(/^(.+?)\s+Hay\s*(?:\((Conventional|Organic)\))?\s*$/i);
    if (!m)
        return null;
    const name = m[1].replace(/^#+\s*/, "").trim();
    if (/direct hay weighted|compared to|please note|volume|tons:|bales:/i.test(name))
        return null;
    if (name.length > 60)
        return null;
    return m[2] ? `${name} (${m[2]})` : name;
}
function hayPlaceLine(line) {
    if (line.length < 3 || line.length > 48)
        return null;
    if (/\d|\$/.test(line))
        return null;
    if (/alfalfa|bermuda|orchard|qty|price|source:|compared|direct hay|email us|usda |volume|please note|freight|crop age/i.test(line)) {
        return null;
    }
    return line;
}
function hayKindLine(line) {
    const m = line.match(/^Hay\s*\((Conventional|Organic)\)\s*$/i);
    return m ? m[1] : null;
}
function hayClass(line) {
    const m = line.match(/^([A-Za-z][A-Za-z0-9 /]+?)\s+-\s+(?:([A-Za-z][A-Za-z/ ]+?)\s+)?\((?:(?:Ask|Trade|Contract \(Trade\))\/)?Per\s+(Ton|Bale|Bundle)\)/i);
    if (!m)
        return null;
...
export function parseHayReport(text, report, sourceUrl) {
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (nextCls) {
            ...
            if (inlinePkg)
                parseHayRow(line, inlinePkg[1], cls, region, asOf, report, source, sourceUrl, out);
            continue;
        }
        if (!cls)
            continue;
...
function parseHayRow(line, pkg, cls, region, asOf, report, source, sourceUrl, out) {
    const afterPkg = line
    ...
    const money = nums
        .map((n) => parseMoney(n))
        .filter((n) => n !== null && n.mid >= loBound && n.mid <= hiBound);
    if (money.length === 0)
        return;
...
export function parseCattleAuctionReport(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const source = `USDA AMS ${report.title} Report (AMS_${report.slug})`;
...
export function parseCattleReport(text, report, sourceUrl) {
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (!line)
            continue;
        const hdr = line.match(/^(?:(?:Beef\/Dairy|Dairy)\s+)?(Steers|Heifers)\s+-\s+(Medium and Large [12](?:-[23])?|Large [123])\s+\(Per Cwt\)/i);
        if (hdr) {
            sex = hdr[1];
            grade = hdr[2];
            inCurrentFob = false;
...
function headlineCattle(rows, report, source, sourceUrl, asOf) {
    ...
    for (const sexTok of ["feeder-steer", "feeder-heifer"]) {
        ...
        const price = subset.reduce((sum, r, i) => sum + r.price * (heads[i] || 1), 0) / total;
...
export function parseGrainReport(text, report, sourceUrl) {
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (!commodity || !/\bCurrent\b/.test(line) || !/Bid\b/i.test(line))
            continue;
        ...
        if (!money || money.mid < 0.5 || money.mid > 30)
            continue;
...
const WOOL_MICRON_RE = /^(\d{2})\s+\(US\s+([^)]+?)\)\s+(\d+\.\d{2})\s+(?:\([^)]+\)|[0-9.]+)\s+(\d+\.\d{2})\s*-\s*(\d+\.\d{2})\s*$/i;
const WOOL_MERINO_RE = /^Merino Clippings\s+(\d+\.\d{2})\s+(?:\([^)]+\)|[0-9.]+)\s+(\d+\.\d{2})\s*-\s*(\d+\.\d{2})\s*$/i;
export function parseWoolReport(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const source = `USDA AMS ${report.title} Report (AMS_${report.slug})`;
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (!line)
            continue;
        if (/exch rate|passed in|awex emi|bales offered|^greasy\b|^volume \(lbs\)/i.test(line))
            continue;
        ...
        if (micron) {
            ...
            if (!Number.isFinite(us$) || us$ < 0.5 || us$ > 20)
                continue;
...
function tickBase(report, sourceUrl, asOf) {
    return {
...
function pushTick(out, report, sourceUrl, asOf, row) {
    const base = tickBase(report, sourceUrl, asOf);
    out.push({ ...row, ...base, series: row.id });
}
function lastDollarOnLine(line) {
...
function dairyWeeklyTableAverages(text) {
    ...
    for (const line of block.split(/\n/)) {
        for (const row of rows) {
            ...
            if (price != null && price > 0.2 && price < 8)
                out[row.key] = price;
        }
    }
    return out;
}
export function parseDairyWeeklyReport(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
...
function dairyFmmoClassPrices(text) {
    const block = text.match(/FEDERAL MILK ORDER CLASS PRICES[\s\S]{0,900}?(?:Further information|NATIONAL DAIRY PRODUCTS)/i)?.[0] ??
        "";
    const out = {};
    const rows = [
    ...
    for (const [key, re] of rows) {
        ...
        if (last != null && last >= 5 && last <= 40)
            out[key] = last;
    }
    return out;
}
export function parseDairyDrySummary(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    ...
    for (const line of compact) {
        const m = line.match(/^(NONFAT DRY MILK - [A-Z/ ]+?|DRY BUTTERMILK|DRY WHEY(?: ANIMAL FEED)?|LACTOSE|WHEY PROTEIN CONCENTRATE(?: 34%)?|DRY WHOLE MILK|CASEIN - (?:ACID|RENNET))\s+(CENTRAL AND EAST|CENTRAL AND WEST|U\.S\. IMPORTS|NORTHEAST|CENTRAL|NATIONAL|WEST)\s+(\d+\.\d{4})\s*-\s*(\d+\.\d{4})/i);
        if (!m)
            continue;
        ...
        const lo = Number(m[3]);
        const hi = Number(m[4]);
        if (!Number.isFinite(lo) || !Number.isFinite(hi) || lo < 0.2 || hi > 20)
            continue;
        const mid = (lo + hi) / 2;
        pushTick(out, report, sourceUrl, asOf, {
...
export function parseDairyFluidReport(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    for (const raw of text.split(/\r?\n/)) {
        const line = raw.replace(/\s+/g, " ").trim();
        const m = line.match(/F\.O\.B\.\s+Cream\s+(All Classes|Class II)\s+\$\/LB Butterfat\s+(\d+\.\d{4})\s+(\d+\.\d{4})/i);
        if (!m)
            continue;
        const lo = Number(m[2]);
        const hi = Number(m[3]);
        if (!Number.isFinite(lo) || lo < 0.5 || hi > 8)
            continue;
        pushTick(out, report, sourceUrl, asOf, {
...
export function parseDairyOrganicAds(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    const re = /((?:Butter|Cheese|Cream Cheese|Ice Cream|Milk|Sour Cream|Yogurt)[^\n$]{0,40}?)\s+\$(\d+\.\d{2})\s+(?:\$[\d.]+|n\.a\.)/gi;
    for (const m of text.matchAll(re)) {
        const commodity = m[1].replace(/\s+/g, " ").trim();
        const price = Number(m[2]);
        if (!Number.isFinite(price) || price < 0.5 || price > 40)
            continue;
        if (/n\.a\./i.test(commodity))
            continue;
        pushTick(out, report, sourceUrl, asOf, {
...
export function parseDairyNdpsr(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
...
export function parseDairyRetailAds(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    ...
    const packRe = /((?:\d+-?\d*\s*(?:oz|lb)|Half Gallon|Gallon)(?:\s+(?:Block|Shred|Sliced))?)\s+(\s*)(\d+)\s+(\d+\.\d{2})\b/i;
    for (const [header, channel] of sections) {
        ...
        for (const raw of block.split(/\r?\n/)) {
            ...
            if (!Number.isFinite(price) || price < 0.4 || price > 20)
                continue;
            if (!Number.isFinite(stores) || stores < 1)
                continue;
            const name = raw
...
const RETAIL_REGION_RE = /^(?:NORTHEAST|SOUTHEAST|MIDWEST|SOUTH CENTRAL|SOUTHWEST|NORTHWEST|ALASKA|HAWAII)\s+REGION\b/i;
function retailColNum(line, start, end) {
...
function retailSlice(line, start, end) {
...
function healRetailFields(item, env, cond) {
...
function retailUnit(raw) {
    if (/per\s*lb/i.test(raw))
        return "$/lb";
    if (/per\s*carton/i.test(raw))
        return "$/carton";
    if (/^each$/i.test(raw.trim()))
        return "$/each";
    if (/per\s*bunch/i.test(raw))
        return "$/bunch";
    if (/per\s*pkg|per\s*package/i.test(raw))
        return "$/pkg";
    const tok = token(raw);
    return tok ? `$/${tok}` : "$/pkg";
}
function nationalRetailBlock(text, startRe, stopRe) {
...
export function parseGroceryRetailLp(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    ...
    while (i < lines.length) {
        ...
        if (RETAIL_REGION_RE.test(raw.trim()))
            break;
        ...
        while (j < lines.length) {
            ...
            if (RETAIL_HEADER_RE.test(nxt) || RETAIL_PAGE_RE.test(nxt) || RETAIL_REGION_RE.test(nxt.trim()))
                break;
            const nxtLeft = nxt.slice(0, cols.item).replace(/\s+/g, " ").trim();
            if (/Per (?:lb|Carton|Pkg)|Each\b/i.test(nxtLeft))
                break;
    ...
    if (required.length > 0 && !required.every((id) => have.has(id)))
        return [];
    return dedupeTicks(out);
}
const SPECIALTY_REQUIRED = ["conventional.apples.honeycrisp.per_lb", "conventional.bananas.per_lb"];
function splitProduceLeft(left) {
    ...
    const unitM = cleaned.match(/^(.*?)(?:\s+)(per lb|each|per bunch|per package|\d[\w .()/%-]*)$/i);
    if (!unitM)
        return null;
...
export function parseSpecialtyCropsRetail(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const sections = [
    ...
    for (const [header, channel] of sections) {
        ...
        for (const raw of lines) {
            ...
            if (!cols)
                continue;
            if (/Wtd Avg - Simple weighted average|Page \d+ of|Weekly Grocery Store Specialty/i.test(raw))
                continue;
            const ads = retailColNum(raw, cols.ads0, cols.ads1);
            const avg = retailColNum(raw, cols.avg0, cols.avg1);
            const leftBit = raw.slice(0, cols.ads0).trim();
            if (leftBit && /^[A-Z]/.test(leftBit))
                pendingLeft = leftBit;
            else if (pendingLeft && leftBit)
                pendingLeft = `${pendingLeft} ${leftBit}`.replace(/\s+/g, " ").trim();
            if (ads == null || avg == null || ads < 1 || avg < 0.15 || avg > 40)
                continue;
            ...
            if (split.variety)
                parts.push(token(split.variety));
            parts.push(unitTok);
            pushTick(out, report, sourceUrl, asOf, {
                id: parts.join("."),
                group: "produce",
    ...
    if (!SPECIALTY_REQUIRED.every((id) => have.has(id)))
        return [];
    return dedupeTicks(out);
}
const EGG_SECTION_RE = /^(NATIONAL|CALIFORNIA)\s+SHELL EGGS[¹1]?\s*-\s*(Caged|Cage-Free|Free-Range|USDA Organic)\b/i;
const EGG_ROW_RE = /(?:(?:Graded Loose|Gradeable Nest Run)\s+)?(White|Brown)\s+(Jumbo|Extra Large|Large|Medium|Small|\d+)\s+([\d,]+)\s+(\d+\.\d{2})\s*-\s*(\d+\.\d{2})\s+(\d+\.\d{2})\b/i;
export function parseShellEggIndex(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (/^Gradeable Nest Run\b/i.test(line))
            kind = "nest_run";
        else if (/^Graded Loose\b/i.test(line))
            kind = "graded_loose";
        if (!environment)
            continue;
        ...
        if (!Number.isFinite(volume) || volume < 1)
            continue;
        if (!Number.isFinite(avg) || avg < 5 || avg > 500)
            continue;
        const color = row[1];
        const cls = row[2];
...
function parseSignedThousands(raw) {
    const t = raw.trim();
    if (!t)
        return null;
...
export function parseColdStorageWeekly(text, report, sourceUrl) {
    const dated = [];
    let change = null;
    for (const raw of text.split(/\r?\n/)) {
        const line = raw.replace(/\s+/g, " ").trim();
        ...
        if (chg) {
            ...
            if (pair)
                change = { butter: pair[0], cheese: pair[1] };
        }
    }
    ...
    const out = [];
    const products = [
        { key: "butter", commodity: "Cold storage butter", label: "US selected-center butter holdings" },
        { key: "cheese", commodity: "Cold storage cheese", label: "US selected-center cheese holdings" },
    ];
...
const CHICKEN_ROW_RE = /^(.+?)\s+(\d+(?:\.\d+)?)\s*-\s*(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)(?:\s+(-?\d+(?:\.\d+)?))?\s+([\d,]+)\b/;
...
function chickenItemToken(label) {
    const cleaned = label.replace(/:+$/, "").trim();
    if (/^national composite whole(?: bird)?$/i.test(cleaned))
        return "national_composite_whole_bird";
    return token(cleaned);
}
/** Official AMS_3646 Weekly National Chicken — current-week cents/lb weighted averages. */
export function parseWeeklyNationalChicken(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (!Number.isFinite(avg) || avg < 1 || avg > 400)
            continue;
        if (!Number.isFinite(lo) || !Number.isFinite(hi) || lo < 0 || hi > 400)
            continue;
        if (!Number.isFinite(volume) || volume < 1)
            continue;
        ...
        const place = section === "export_fresh"
            ? "Export fresh"
            : section === "export_frozen"
                ? "Export frozen"
                : section === "parts"
    ...
    const have = new Set(out.map((row) => row.id.replace(/^dairy\.ams_\d+\./, "")));
    if (!CHICKEN_REQUIRED_IDS.every((id) => have.has(id)))
        return [];
    return dedupeTicks(out);
}
const TURKEY_ROW_RE = /^(.+?)\s+(?:Yes|No)\s+(\d+(?:\.\d+)?)\s*-\s*(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)(?:\s+(-?\d+(?:\.\d+)?))?\s+([\d,]+)\b/;
function turkeyHeaderKind(line) {
    if (/^Turkey,\s*Whole\b/i.test(line) || /^Turkey,\s*Part\b/i.test(line))
        return "section";
    if (/^(?:Domestic|Export)\s*-\s*(?:Fresh|Frozen)\b/i.test(line))
        return "basis";
    if (TURKEY_ROW_RE.test(line))
        return "row";
    if (/Current Weeks Trading|Previous Weeks Trading|Price Range|Wtd Avg|1,000 lbs|^Offer\b/i.test(line) ||
        ...
        /\b(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},\s+\d{4}\b/i.test(line)) {
        return "skip";
    }
    return "cont";
}
function attachTurkeyContinuation(rowLine, extra) {
    const m = rowLine.match(/^(.*?)(\s+(?:Yes|No)\s+\d)/);
    if (!m)
        return `${rowLine} ${extra}`;
    const label = `${m[1].replace(/[,\s]+$/, "")} ${extra.replace(/^[,\s]+/, "")}`.replace(/\s+/g, " ").trim();
    return `${label}${m[2]}${rowLine.slice(m[0].length)}`;
}
function mergeTurkeyWrappedLines(text) {
    const out = [];
    for (const raw of text.split(/\r?\n/)) {
        const line = raw.replace(/\s+/g, " ").trim();
        if (!line)
...
function turkeyItemToken(label) {
    return token(label.replace(/U\.S\./gi, "US").replace(/:+$/, ""));
}
function turkeyHasRequired(ids) {
    const list = [...ids];
    return (list.some((id) => /whole\./.test(id) && /whole_young_(hen|tom)/.test(id)) &&
...
export function parseWeeklyNationalTurkey(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    ...
    for (const line of mergeTurkeyWrappedLines(text)) {
        ...
        const avg = Number(row[4]);
        const volume = Number(row[6].replace(/,/g, ""));
        if (!Number.isFinite(avg) || avg < 1 || avg > 800)
            continue;
        if (!Number.isFinite(lo) || !Number.isFinite(hi) || lo < 0 || hi > 800)
            continue;
        if (!Number.isFinite(volume) || volume < 1)
            continue;
    ...
    const have = new Set(out.map((row) => row.id.replace(/^dairy\.ams_\d+\./, "")));
    if (!turkeyHasRequired(have))
        return [];
    return dedupeTicks(out);
}
export function parseDairyRegionalDry(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const found = [];
    for (const raw of text.split(/\r?\n/)) {
        const line = raw.replace(/\s+/g, " ").trim();
        const m = line.match(/^(?:(Mostly Range|Price Range)\s+-\s+([^;]+);\s*\$\/LB:|([A-Za-z][A-Za-z /-]+);\s*Price Range\s+-\s*\$\/LB:)\s*(\d*\.\d{2,4})\s*-\s*(\d*\.\d{2,4})/i);
        if (!m)
            continue;
        const lo = Number(m[4]);
        const hi = Number(m[5]);
        if (!Number.isFinite(lo) || !Number.isFinite(hi) || lo < 0.1 || hi > 20)
            continue;
        found.push({
    ...
    for (const row of found) {
        ...
        if (!row.kind.startsWith("mostly") && mostly.has(tok))
            continue;
        pushTick(out, report, sourceUrl, asOf, {
...
export function parseFeederPigReport(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        const composite = line.match(/^Total Composite\s+([\d,]+)\s+(\d+\.\d{2})\s*-\s*(\d+\.\d{2})\s+(\d+\.\d{2})\b/i);
        ...
        const all = line.match(/^Pigs (Early Weaned - All|-\s*All)\s+([\d,]+)\s+(\d+\.\d{2})\b/i);
        if (!all)
            continue;
...
export function parseHogSummary(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    const national = text.match(/NATIONAL\s+-\s+AMS[\s\S]{0,400}?Range:\s+\$(\d+\.\d{2})\s+\$(\d+\.\d{2})[\s\S]{0,200}?Weighted Average:\s+\$(\d+\.\d{2})/i);
    if (national) {
    ...
    const prior = text.match(/Carcass Base Price\s+(\d+\.\d{2})\s+(\d+\.\d{2})\s+(\d+\.\d{2})\s+(\d+\.\d{2})\s+(\d+\.\d{2})/);
    if (prior) {
...
export function parseOrganicGrainReport(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (!commodity)
            continue;
        const spot = line.match(/^(National|Midwest|Central|Northeast|West)\s+(Grower|Dealer)\s+Spot\s+Current\s+(\d+\.\d{2})\s*-\s*(\d+\.\d{2})(?:\s+(\d+\.\d{2}))?/i);
        if (!spot)
            continue;
        ...
        if (!Number.isFinite(avg) || avg < 1 || avg > hiBound)
            continue;
        const id = [
            group,
            `ams_${report.slug}`,
...
export function parseProduceTerminal(text, report, sourceUrl) {
    const asOf = parseReportDate(text);
    if (!asOf)
        return [];
    const out = [];
    ...
    for (let i = 1; i < blocks.length; i += 2) {
        ...
        if (/INSUFFICIENT TO QUOTE/i.test(body) && !/\d+\.\d{2}/.test(body))
            continue;
        ...
        if (origin)
            parts.push({ origin, chunk: body.slice(last) });
        let added = 0;
        for (const part of parts) {
            if (added >= 8)
                break;
            if (/holdovers|one lot|one job|insufficient/i.test(part.chunk.slice(0, 24)))
                continue;
            ...
            if (!Number.isFinite(lo) || lo < 1 || hi > 500)
                continue;
...
const COTTON_REQUIRED_IDS = ["cotton.seven_market.spot_41_4_34", "cotton.fsa.adjusted_world_price"];
function flattenCotton(text) {
    return text.replace(/\u00a0/g, " ").replace(/[ \t]+/g, " ");
}
function parseCottonAsOf(text) {
    const header = text.match(/Weekly Cotton Market Review[\s\S]{0,200}?(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}),\s+(\d{4})/i);
    if (header) {
        const mon = MONTHS[header[1].toLowerCase()];
        if (mon)
            return `${header[3]}-${mon}-${header[2].padStart(2, "0")}`;
    }
    return parseReportDate(text);
}
function cottonCents(raw) {
    const n = Number(String(raw).replace(/[¢,]/g, ""));
    if (!Number.isFinite(n) || n < 0 || n > 200)
        return null;
    return n;
}
function cottonRegionToken(raw) {
    const t = raw.replace(/\s+/g, " ").trim();
    if (/east texas/i.test(t))
        return "east_texas";
    if (/west texas/i.test(t))
        return "west_texas";
    if (/desert southwest/i.test(t))
        return "desert_southwest";
    if (/san joaquin/i.test(t))
        return "san_joaquin";
    if (/american pima/i.test(t))
        return "american_pima";
    if (/north delta/i.test(t))
        return "north_delta";
    if (/south delta/i.test(t))
        return "south_delta";
    if (/southeast/i.test(t))
        return "southeast";
    return token(t);
}
function cottonCropToken(raw) {
    const years = [...raw.matchAll(/\b(20\d{2})\b/g)].map((m) => m[1]);
    if (/new-crop/i.test(raw))
        return "new_crop";
    if (years.length >= 2)
        return `crop_${years[0]}_${years[1]}`;
    if (years.length === 1)
        return `crop_${years[0]}`;
    return "current";
}
function cottonColorToken(raw) {
    const range = raw.match(/\bcolor\s+(\d{2})\s*[-–]\s*(\d{2})\b/i);
    if (range)
        return `color_${range[1]}_${range[2]}`;
    const pair = raw.match(/\bcolor\s+(\d{2})\s+and\s+(\d{2})\b/i);
    if (pair)
        return `color_${pair[1]}_${pair[2]}`;
    const one = raw.match(/\bcolor\s+(\d{2})\b/i);
    if (one)
        return `color_${one[1]}`;
    return "";
}
function pushCottonTick(out, report, sourceUrl, asOf, parts, row) {
    pushTick(out, report, sourceUrl, asOf, {
...
export function parseWeeklyCottonReview(text, report, sourceUrl) {
    const asOf = parseCottonAsOf(text);
    if (!asOf)
        return [];
    const flat = flattenCotton(text);
    const out = [];
    const spot = flat.match(/7-Market Weekly Avg\.?\s+Spot Price\s*\(\s*41-4-34\s*\):[\s\S]{0,240}?(\d+\.\d{2})\s*cents(?:\s*\/\s*pound)?/i) ??
    ...
    const iceDec = flat.match(/ICE\s+DEC\s+Futures\s+Week Ending Settlement\s+Price:\s*(\d+\.\d{2})/i) ??
        flat.match(/ICE\s+(?:Oct|DEC|Dec)\s+settlement price ended the week at\s+(\d+\.\d{2})/i);
    const awp = flat.match(/Adjusted World price\s*1?\/?\s+(\d+\.\d{2})/i) ??
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (!line)
            continue;
        const regionLine = line.match(/^(East Texas\/South Texas|West Texas,\s*Kansas,\s*Oklahoma|Desert Southwest|San Joaquin Valley|American Pima|North Delta|South Delta)\b/i);
        if (regionLine) {
            flushCottonBullet();
    ...
    if (!COTTON_REQUIRED_IDS.every((id) => have.has(id)))
        return [];
    return dedupeTicks(out);
}
export function parseAmsReportText(text, report, sourceUrl) {
    if (report.group === "hay")
    ...
    if (report.group === "dairy") {
        if (report.slug === "2998")
            return parseDairyWeeklyReport(text, report, sourceUrl);
        if (report.slug === "2993")
            return parseDairyNdpsr(text, report, sourceUrl);
        if (report.slug === "2995")
            return parseDairyRetailAds(text, report, sourceUrl);
        if (RETAIL_LP_SLUGS.has(report.slug))
            return parseGroceryRetailLp(text, report, sourceUrl);
        if (report.slug === "1598")
            return parseDairyDrySummary(text, report, sourceUrl);
        if (report.slug === "2997")
            return parseDairyOrganicAds(text, report, sourceUrl);
        if (report.slug === "2843")
            return parseShellEggIndex(text, report, sourceUrl);
        if (report.slug === "1095")
            return parseColdStorageWeekly(text, report, sourceUrl);
        if (report.slug === "3646")
            return parseWeeklyNationalChicken(text, report, sourceUrl);
        if (report.slug === "3647")
            return parseWeeklyNationalTurkey(text, report, sourceUrl);
    ...
    if (report.group === "hogs") {
        if (report.slug === "2810")
            return parseFeederPigReport(text, report, sourceUrl);
        return parseHogSummary(text, report, sourceUrl);
    }
    if (report.group === "produce") {
        if (report.slug === "3324")
            return parseSpecialtyCropsRetail(text, report, sourceUrl);
...
export async function officialPdfCandidates(report) {
    const esmisUrls = [];
...
export async function resolveOfficialPdfUrl(report) {
    return (await officialPdfCandidates(report))[0] ?? MNREPORTS_PDF(report.slug);
}
export function readAmsSnapshot(dir = amsNationalDir()) {
...
export function writeAmsSnapshot(snap, dir = amsNationalDir()) {
    mkdirSync(dir, { recursive: true });
    const path = amsSnapshotPath(dir);
    writeFileSync(path, `${JSON.stringify(snap, null, 2)}\n`);
    return path;
}
export function mergeAmsNationalTicks(payload, snap = readAmsSnapshot()) {
    ...
    const fetchedAt = snap.fetchedAt && (!payload.fetchedAt || snap.fetchedAt > payload.fetchedAt)
        ? snap.fetchedAt
        : payload.fetchedAt;
    return {
        ...payload,
...
function reportSlugFromTickId(id) {
    const m = id.match(/\.ams_([a-z0-9_]+)\./i);
    return m ? m[1] : null;
}
function reportSlugFromFailedId(id) {
    const m = id.match(/^ams_([a-z0-9_]+)$/i);
    return m ? m[1] : null;
}
function reportSlugFromSourceLabel(label) {
    const m = label.match(/^AMS_([a-z0-9_]+)\b/i);
    return m ? m[1] : null;
}
export function collectReportFilter() {
    ...
    const only = leftoverOnly
        ...
        : env("TICKS_AMS_ONLY_SLUGS")
            ...
            .filter(Boolean);
    if (only.length === 0)
        return AMS_NATIONAL_REPORTS;
    const want = new Set(only);
    return AMS_NATIONAL_REPORTS.filter((r) => want.has(r.slug));
}
export function mergePartialAmsSnapshot(prev, next, slugs) {
...
export function mergeFailedAmsSlugs(prev, next) {
    const failedSlugs = new Set(next.failed
...
//# sourceMappingURL=ticks-ams.js.map