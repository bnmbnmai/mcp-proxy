...
export function parseReportDate(text) {
    ...
    const livestockDay = text.match(/Livestock Weighted Average Report for\s+(\d{1,2}\/\d{1,2}\/\d{4})\b/i);
    if (livestockDay)
        return parseMdY(livestockDay[1]);
...
function auctionCowGradeTok(grade) {
    if (/boner/i.test(grade))
        return "boner";
    if (/breaker/i.test(grade))
        return "breaker";
    if (/lean/i.test(grade))
        return "lean";
    if (/2-3/.test(grade))
        return "ml23";
    if (/1-2/.test(grade) && /medium|large/i.test(grade))
        return "ml12";
    if (/^1-2$/i.test(grade.trim()))
        return "12";
...
function parseAuctionCowSale(text, report, source, sourceUrl, asOf) {
    const out = [];
    let section = "";
    let sex = "";
    let grade = "";
