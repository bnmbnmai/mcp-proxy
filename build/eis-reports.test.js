...
import { ATTRIBUTION, BODY_NEEDLE_CLINCH, BODY_NEEDLE_F35, CARD_FIELDS, EIS_REPORTS_AMOUNT_ATOMIC, EIS_REPORTS_MANIFEST_PATH, EIS_REPORTS_ONE_AMOUNT_ATOMIC, EIS_REPORTS_PATH, LAST_WEEK_URL, LICENSE, PRODUCT_ID, SEED_LISTINGS, altchaPayload, assembleEisReportsSnapshot, buildEisReportsManifest, collectEisReports, filterEisReportsManifest, isChromeEisHtml, isCommentLetterTitle, isPdfBytes, isRealEisBody, isSkippedEisAttachment, isSummaryTeaserTitle, isSuperfundRodDump, looksLikeHumanCaptcha, looksLikeLeakedEisBody, looksLikeLeftoverRecaptchaScript, looksLikeLoginGov, officialEisPageUrl, parseDetailsListings, parseSearchDownloadEis, parseSearchRows, selectEisReportCard, solveAltchaPow, } from "./eis-reports.js";
...
async function main() {
    ...
    const payload = altchaPayload({ algorithm: "SHA-256", challenge, salt, signature: "sig", maxnumber: 100 }, 42, 5);
    const decoded = JSON.parse(Buffer.from(payload, "base64").toString("utf8"));
    assert.equal(decoded.number, 42);
    assert.equal(officialEisPageUrl("https://evil.example/eis?eisId=1"), null);
    assert.equal(officialEisPageUrl("https://cdxapps.epa.gov/cdx-enepa-II/public/action/eis/details?eisId=555705"), "https://cdxapps.epa.gov/cdx-enepa-II/public/action/eis/details?eisId=555705");
    ...
    if (prevDir === undefined)
        delete process.env.EIS_REPORTS_DIR;
    else
        process.env.EIS_REPORTS_DIR = prevDir;
    ...
    const seedCard = manifest.cards.find((c) => c.id === "20260036");
    ...
    assert.equal(filtered.cards[0]?.id, "20260036");
...
//# sourceMappingURL=eis-reports.test.js.map