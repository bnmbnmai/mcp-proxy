...
export const LISTING_URL = "https://www.fda.gov/inspections-compliance-enforcement-and-criminal-investigations/compliance-actions-and-activities/warning-letters";
export const LETTER_BASE = "https://www.fda.gov/inspections-compliance-enforcement-and-criminal-investigations/warning-letters/";
export const LETTER_PATH_RE = /\/inspections-compliance-enforcement-and-criminal-investigations\/warning-letters\/([a-z0-9-]+-\d{6}-\d{8})/i;
...
export function warningLettersDir() {
    if (env("WARNING_LETTERS_DIR"))
        return resolve(env("WARNING_LETTERS_DIR"));
...
export function absoluteLetterUrl(href) {
...
export function letterIdFromUrl(url) {
    const m = url.match(LETTER_PATH_RE);
    return m ? m[1].toLowerCase() : null;
}
function slugDate(id) {
...
export function parseListingHtml(html) {
    ...
    for (const row of rows) {
        ...
        const issue = row.match(/views-field-field-letter-issue-datetime[\s\S]*?<time[^>]*datetime="([^"]+)"/i);
...
function extractLetterHtml(html) {
    const start = html.search(/<p[^>]*text-align-center[^>]*>\s*<strong>\s*WARNING LETTER\s*<\/strong>/i);
    if (start < 0)
        return "";
...
export function parseLetterHtml(html, sourceUrl) {
    ...
    const cms = (html.match(/MARCS-CMS\s+(\d+)/i) || [])[1] ||
        (title ? (stripTags(title[1]).match(/\b(\d{6})\b/) || [])[1] : "") ||
        slugCms(id);
    const issuedOn = isoDate((html.match(/<h1[\s\S]*?<time[^>]*datetime="([^"]+)"/i) || [])[1]) ||
    ...
    const issuingOffice = metaContent(html, "dcterms.creator") ||
...
export function assembleSnapshot(letters, fetchedAt = new Date().toISOString()) {
...
function readHtmlDirFile(dir, names) {
    if (!dir)
        return null;
...
export async function fetchFdaHtml(url) {
...
export async function collectWarningLetters(opts) {
    ...
    const listingHtml = readHtmlDirFile(dir, ["listing.html", "listing-excerpt.html"]) ?? (await fetchFdaHtml(LISTING_URL));
    ...
    for (const row of listed) {
        if (!dir)
            await pause(pauseMs);
        try {
            const html = readHtmlDirFile(dir, [`${row.id}.html`, `${row.id}-excerpt.html`]) ??
                (await fetchFdaHtml(row.sourceUrl));
            const parsed = parseLetterHtml(html, row.sourceUrl);
            if (!parsed.firm || parsed.firm === parsed.id)
                parsed.firm = row.firm || parsed.firm;
            if (!parsed.issuedOn)
                parsed.issuedOn = row.issuedOn;
            letters.push(parsed);
        }
        catch (err) {
...
export async function loadWarningLetters() {
    ...
    try {
        ...
        return emptySnapshot(`FDA warning-letter HTML is not on this host and live fetch failed. ${err instanceof Error ? err.message : String(err)}`);
    }
}
export function buildWarningLettersManifest(snap) {
...
export async function loadWarningLettersManifest() {
    const cached = readSnapshot();
    if (cached)
        return buildWarningLettersManifest(cached);
    try {
        const dir = htmlDir();
        if (dir) {
            ...
            if (listingHtml) {
                ...
                return buildWarningLettersManifest(assembleSnapshot(listed.map((row) => ({
                    id: row.id,
                    firm: row.firm,
                    cms: slugCms(row.id),
                    issuedOn: row.issuedOn,
...
//# sourceMappingURL=warning-letters.js.map