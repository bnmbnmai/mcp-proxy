...
const BAG_BY_PATH = {
    ...
    "/ofwat-enforcement": "Ofwat Water Industry Act 1991 enforcement-notice / final-decision / s.19 undertakings text. Newest 10 official texts",
...
export function skuKind(path) {
    if (TABLE_PATHS.has(path))
        return "table";
...
export function searchMarkdown(path) {
    if (path === "/ticks") {
...
export function bagForPath(path, openApi) {
...
export function skusFromWellKnown(wellKnown, openApi) {
...
export function shopIndexMarkdown(skus) {
    const rows = skus
...
export function readmeMarkdown(skus) {
...
export async function fetchLiveCatalog(origin = LIVE_ORIGIN) {
    const base = origin.replace(/\/+$/, "") || LIVE_ORIGIN;
    const wellKnown = (await fetchJson(`${base}${WELL_KNOWN_PATH}`));
    const openApi = (await fetchJson(`${base}${OPENAPI_PATH}`));
    return { wellKnown, openApi, skus: skusFromWellKnown(wellKnown, openApi) };
}
//# sourceMappingURL=shop-catalog.js.map