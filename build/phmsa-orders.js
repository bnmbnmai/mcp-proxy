...
export const ATTRIBUTION = "Pipeline and Hazardous Materials Safety Administration (PHMSA), U.S. Department of Transportation. Work of the United States Government; 17 U.S.C. § 105.";
...
const HTTP_UA = "bnm-data-shop/1.0 (PHMSA order texts; +https://primis.phmsa.dot.gov/enforcement-documents/)";
const OFFICIAL_HOSTS = new Set(["primis.phmsa.dot.gov", "www.primis.phmsa.dot.gov"]);
const MIN_BODY_CHARS = 2000;
const ENTITY_RE = /\b(Inc\.?|LLC|L\.L\.C\.|L\.P\.|LP|Corp\.?|Corporation|Company|Co\.|Ltd\.?|Limited|Partners|Pipeline|Energy|Operating|Transmission|Holdco|Crossing)\b/i;
const PERSON_NAME_RE = /^[A-Z][a-z]+(?:\s+[A-Z]\.?)?(?:\s+[A-Z][a-z]+){1,3}$/;
const PHMSA_TITLE_RE = /Final Order|Notice of Probable Violation|\bNOPV\b|Corrective Action Order|\bCAO\b|Consent Agreement and Order|Consent Order|Safety Order|NOPSO/i;
const OPERATOR_RESPONSE_RE = /operator[- _]?response|operator[- _]?reply|response to the (?:notice|nopv|order)|operator reply/i;
const NARRATIVE_RE = /27nc-rsge|incident[- _]?narrative|\bNARRATIVE\b/i;
const RAW_TSV_RE = /PHMSA Pipeline Enforcement Raw Data|Raw Data\.txt|\.tsv\b|page-data\.json/i;
const HTML_TEASER = /\b(read the (?:full )?(?:order|notice|document)|download the pdf|this page is an index|case-card teaser|index only)\b/i;
const FERC_DUMP = /\bFEDERAL ENERGY REGULATORY COMMISSION\b.*\b(STIPULATION AND CONSENT|Docket No\. IN\d)/is;
export const SEED_LISTINGS = [
    {
        ...
        sourceUrl: "https://primis.phmsa.dot.gov/enforcement-documents/32026023CAO/32026023CAO_Corrective%20Action%20Order_08032026_(26-379109)_text.pdf",
    },
    {
        id: "3-2026-020-cao",
        ...
        sourceUrl: "https://primis.phmsa.dot.gov/enforcement-documents/32026020CAO/32026020CAO_Corrective%20Action%20Order_05162026_(26-372495).pdf",
    },
    {
        id: "4-2026-012-nopv",
        ...
        sourceUrl: "https://primis.phmsa.dot.gov/enforcement-documents/42026012NOPV/42026012NOPV_Final%20Order_04072026_(23-266706).pdf",
    },
    {
        id: "4-2026-004-nopv",
        ...
        sourceUrl: "https://primis.phmsa.dot.gov/enforcement-documents/42026004NOPV/42026004NOPV_Final%20Order_04012026_(25-329817).pdf",
    },
    {
        id: "4-2025-049-nopv",
        ...
        sourceUrl: "https://primis.phmsa.dot.gov/enforcement-documents/42025049NOPV/42025049NOPV_Final%20Order_03262026_(24-296979).pdf",
    },
    {
        id: "4-2026-005-cao",
        ...
        sourceUrl: "https://primis.phmsa.dot.gov/enforcement-documents/42026005CAO/42026005CAO_Corrective%20Action%20Order_02172026_(26-364755).pdf",
    },
];
function env(name, fallback = "") {
    return (process.env[name] ?? fallback).trim();
}
export function phmsaOrdersDir() {
    if (env("PHMSA_ORDERS_DIR"))
        return resolve(env("PHMSA_ORDERS_DIR"));
...
export function isoDate(raw) {
    ...
    const named = raw.match(/\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}),?\s+(\d{4})\b/i);
    if (named) {
        const months = {
            january: "01",
            february: "02",
...
export function cpfFromCompact(raw) {
    if (!raw)
        return null;
...
export function kindFromBlob(raw) {
    const t = raw.replace(/\s+/g, " ");
    if (/Corrective Action Order|\bCAO\b/i.test(t))
        return "corrective-action-order";
    if (/Final Order/i.test(t))
        return "final-order";
    if (/Notice of Probable Violation|\bNOPV\b/i.test(t))
        return "nopv";
    if (/Consent Agreement|Consent Order/i.test(t))
        return "consent-order";
    if (/Safety Order|NOPSO/i.test(t))
        return "safety-order";
    return "enforcement-order";
}
export function idFromCpfAndKind(cpf, kind) {
    const base = cpf.toLowerCase();
    if (kind === "corrective-action-order" && !/-cao$/i.test(base))
        return `${base}-cao`;
    return base;
}
...
export function officialPhmsaOrderPdfUrl(urlOrPath) {
    if (!urlOrPath)
        return null;
    try {
        ...
        if (!OFFICIAL_HOSTS.has(host))
            return null;
        if (/\/enforcement-data\//i.test(parsed.pathname))
            return null;
        ...
        if (!media)
            return null;
        if (NARRATIVE_RE.test(media[1]) || NARRATIVE_RE.test(media[2]))
            return null;
        if (OPERATOR_RESPONSE_RE.test(media[2]))
            return null;
...
export function isOfficialPhmsaOrderPdf(url) {
...
export function parseListingRows(rows) {
    ...
    for (const row of rows) {
        ...
        if (!sourceUrl)
            continue;
        if (!PHMSA_TITLE_RE.test(`${row.title} ${row.kind} ${sourceUrl}`))
            continue;
...
export function parseHubHtml(html, _pageUrl = HUB_URL) {
    const rows = [];
    const links = [
        ...html.matchAll(/(?:(\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}\/\d{4})[\s\S]{0,280}?)?<a[^>]+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi),
    ];
...
export function isIndexTeaserDump(text) {
    const t = String(text || "")
        .replace(/\s+/g, " ")
        .trim();
    if (t.length < MIN_BODY_CHARS && HTML_TEASER.test(t))
        return true;
    if (/Index only — institution \/ CPF \/ date \/ PDF URL/i.test(t))
        return true;
    if (/PHMSA case-card teaser/i.test(t))
        return true;
    return false;
}
export function isOperatorResponseDump(text) {
    const t = String(text || "");
    if (!OPERATOR_RESPONSE_RE.test(t))
        return false;
    return !/\bFINAL ORDER\b|\bCORRECTIVE ACTION ORDER\b|\bNOTICE OF PROBABLE VIOLATION\b/i.test(t);
}
export function isIncidentNarrativeDump(text) {
    return NARRATIVE_RE.test(text) && !/\bFINAL ORDER\b|\bCORRECTIVE ACTION ORDER\b|\bNOTICE OF PROBABLE VIOLATION\b/i.test(text);
}
export function isTsvWrapDump(text) {
    if (/PHMSA Pipeline Enforcement Raw Data/i.test(text))
        return true;
    if (/^CPF_Number\t/m.test(text) && /Opened_Date|Proposed_Penalties/i.test(text))
        return true;
    return false;
}
export function isFercDump(text) {
    return FERC_DUMP.test(text);
}
export function isPeopleDump(text) {
    if (/people-only PHMSA/i.test(text))
        return true;
    if (/\bAn Individual\b/i.test(text) && !ENTITY_RE.test(text))
        return true;
    return false;
}
export function isRealPhmsaOrderBody(text) {
    const t = String(text || "")
        .replace(/\s+/g, " ")
        .trim();
    ...
    const order = /\bFINAL ORDER\b|\bCORRECTIVE ACTION ORDER\b|\bNOTICE OF PROBABLE VIOLATION\b|\bCONSENT (?:AGREEMENT AND )?ORDER\b|\bSAFETY ORDER\b/i.test(t);
    const statute = /49 CFR|49 C\.F\.R\.|49 U\.S\.C\.|\bCPF\b/i.test(t);
    return phmsa && order && statute;
}
export function parsePhmsaOrderText(text, meta) {
...
export function emptyPhmsaOrdersSnapshot(reason) {
    return {
        ok: true,
        product: PRODUCT_ID,
        status: "empty",
...
export function assemblePhmsaOrdersSnapshot(cards, fetchedAt = new Date().toISOString()) {
...
export function readPhmsaOrdersSnapshot() {
...
export function writePhmsaOrdersSnapshot(snap) {
    const path = snapshotPath();
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, `${JSON.stringify(snap, null, 2)}\n`);
}
export async function fetchPhmsaOrderBytes(url) {
...
export async function collectPhmsaOrders(opts) {
    ...
    for (const card of readPhmsaOrdersSnapshot()?.cards ?? []) {
        if (isRealPhmsaOrderBody(card.body))
...
export async function loadPhmsaOrders() {
    ...
    try {
        ...
        return emptyPhmsaOrdersSnapshot(`PHMSA order PDFs are not on this host and live fetch failed. ${err instanceof Error ? err.message : String(err)}`);
    }
}
export function buildPhmsaOrdersManifest(snap) {
    const cards = (snap?.cards ?? []).filter((c) => isRealPhmsaOrderBody(c.body));
    return {
        product: PRODUCT_ID,
        name: PRODUCT_NAME,
        free: true,
        note: paidBodyCatalogNote(PHMSA_ORDERS_PATH, "Count + institution + CPF/docket + date + official PHMSA PDF URL only. Order body is the paid GET /phmsa-orders payload. This free manifest lists the full catalog. Operator-response letters, the killed incident NARRATIVE zip, and the primis TSV (dates/penalties only) are not sold. Distinct from /ferc-orders."),
        license: LICENSE,
        attribution: ATTRIBUTION,
        payTo: PAY_TO,
        network: "base",
...
export function filterPhmsaOrdersManifest(manifest, q) {
    ...
    const matched = cards.filter((raw) => {
        if (!raw || typeof raw !== "object")
            return false;
        const row = raw;
        return ["id", "docket", "institution", "date", "kind", "sourceUrl"].some((k) => String(row[k] ?? "")
            .toLowerCase()
            .includes(needle));
    });
    return { ...manifest, cardCount: matched.length, cards: matched, q: needle };
}
export async function loadPhmsaOrdersManifest(q) {
    return filterPhmsaOrdersManifest(buildPhmsaOrdersManifest(readPhmsaOrdersSnapshot()), q);
}
export function buildPhmsaOrdersPaidPage(snap, opts) {
...
//# sourceMappingURL=phmsa-orders.js.map