...
async function main(): Promise<void> {
  await withServer({
    ...
    assert.equal(wk.resources.length, 11, "well-known lists the eleven always-public doors");
    ...
    for (const paid of [TICKS_PATH, IMPORT_ALERTS_PATH, MARINERS_PATH, MARINERS_D11_PATH, MARINERS_D7_PATH, MARINERS_D8_PATH, WARNING_LETTERS_PATH, UNTITLED_LETTERS_PATH, AWA_PATH, SWISSPAR_PATH, PCAC_PATH]) {
    ...
    assert.equal(
      ...
      11,
      "OpenAPI lists the eleven always-public paid paths",
    );
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.length, 11);
      assert.equal(shop.openapi, OPENAPI_PATH);
      assert.equal(shop.wellKnown, WELL_KNOWN_PATH);
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.length, 13, "thirteenth product is /gmp when a real observation body is cached");
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.length, 14, "fourteenth product is /gmp-md when a real MD body is cached");
  ...
  await withServer(
    ...
    async (base) => {
      assert.equal(cdpEnvStatus(), "CDP env not set");
      for (const path of [TICKS_PATH, IMPORT_ALERTS_PATH, MARINERS_PATH, MARINERS_D11_PATH, MARINERS_D7_PATH, MARINERS_D8_PATH, WARNING_LETTERS_PATH, UNTITLED_LETTERS_PATH, AWA_PATH, SWISSPAR_PATH, PCAC_PATH, FORM_483_PATH, GMP_PATH, GMP_MD_PATH]) {
  ...
  assert.deepEqual(PUBLIC_BAZAAR_SKUS, ["ticks", "import-alerts", "mariners", "mariners-d11", "mariners-d7", "mariners-d8", "warning-letters", "untitled-letters", "awa", "swisspar", "pcac"]);
