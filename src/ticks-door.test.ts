...
async function main(): Promise<void> {
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.length, PUBLIC_BAZAAR_SKUS.length + 2, "/gmp joins when a real observation body is cached");
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.length, PUBLIC_BAZAAR_SKUS.length + 3, "/gmp-md joins when a real MD body is cached");
