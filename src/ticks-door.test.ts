...
import {
  ATSDR_HC_AMOUNT_ATOMIC,
  ATSDR_HC_MANIFEST_PATH,
  ATSDR_HC_PATH,
} from "./atsdr-hc.js";
import {
...
import {
  DOE_NOV_AMOUNT_ATOMIC,
  DOE_NOV_MANIFEST_PATH,
  DOE_NOV_PATH,
} from "./doe-nov.js";
import {
...
async function main(): Promise<void> {
  await withServer({
    ...
    assert.equal(wk.resources.length, 28, "well-known lists the twenty-eight always-public doors");
    ...
    assert.ok((wk.instructions ?? "").includes("twenty-eight paid"));
    ...
    assert.equal(spec.paths[DOE_NOV_PATH], undefined, "OpenAPI must not list /doe-nov");
    ...
    assert.equal(
      ...
      28,
      "OpenAPI lists the twenty-eight always-public paid paths",
    );
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.some((p) => p.path === DOE_NOV_PATH), false, "do not list /doe-nov");
      ...
      assert.ok(!wk.resources.some((r) => r.includes(DOE_NOV_PATH)), "well-known must not list /doe-nov");
      assert.ok(!wk.resources.some((r) => r.includes(NOP_AD_PATH)), "well-known must not list /nop-ad");
      ...
      assert.ok(!llms.includes("GET /doe-nov"), "llms.txt must not list /doe-nov");
      assert.ok(!llms.includes("GET /nop-ad"), "llms.txt must not list /nop-ad");
  ...
  const ccpcMergersDir = mkdtempSync(join(tmpdir(), "ccpc-mergers-"));
  writeFileSync(
    join(ccpcMergersDir, "snapshot.json"),
    JSON.stringify({
      ok: true,
      ...
      sources: {
        ...
        pdfHost: "https://assets.ccpc.ie/data/docs/default-source/merger-attachments/",
      },
      cards: [
        {
          ...
          body: [
            ...
            "40km radius of Kehoe",
            "Associated Hardware/National Hardware",
            ...Array.from({ length: 40 }, (_, i) => `${i + 21}. Numbered article ${i + 21} from the official United Hardware CCPC determination body used only to keep this door fixture above the real-determination length floor.`),
          ].join("\n"),
        },
      ],
    }),
  ...
  await withServer(
    {
      CCPC_MERGERS_DIR: ccpcMergersDir,
      X402_SKIP_SETTLE: "1",
      FORM_483_DIR: join(tmpdir(), "form-483-absent-ccpc-mergers-"),
    },
    async (base) => {
      const unpaid = await fetch(`${base}${CCPC_MERGERS_PATH}`);
      assert.equal(unpaid.status, 402, "unpaid GET /ccpc-mergers must be 402");
      const body402 = (await unpaid.json()) as {
        payTo: string;
        asset: string;
        resource: string;
      ...
      assert.equal(body402.resource, CCPC_MERGERS_PATH);
      assert.equal(body402.accepts[0]?.maxAmountRequired, CCPC_MERGERS_AMOUNT_ATOMIC);
      assert.equal(body402.accepts[0]?.extra?.name, "USD Coin");
      const ccpcPr = unpaid.headers.get("payment-required");
      assert.ok(ccpcPr, "v2 PAYMENT-REQUIRED header");
      ...
      const man = (await manifest.json()) as {
        cardCount?: number;
        cards?: { institution?: string; mNumber?: string; id?: string; body?: string }[];
        openapi?: string;
        wellKnown?: string;
      };
      assert.equal(man.cardCount, 1);
      assert.equal(man.cards?.[0]?.institution, "United Hardware DAC / Ardentia / Kehoe’s Homevalue t/a Dermot Kehoe Supply & DIY");
      ...
      const paid = await fetch(`${base}${CCPC_MERGERS_PATH}`, { headers: { "X-PAYMENT": "test" } });
      assert.equal(paid.status, 200);
      const paidBody = (await paid.json()) as {
        product: string;
        cards: { institution: string; date: string; mNumber: string; body: string }[];
      };
      assert.equal(paidBody.product, "ccpc-institution-merger-determination-bodies");
      assert.equal(paidBody.cards[0]?.institution, "United Hardware DAC / Ardentia / Kehoe’s Homevalue t/a Dermot Kehoe Supply & DIY");
      assert.equal(paidBody.cards[0]?.date, "2026-03-06");
      assert.equal(paidBody.cards[0]?.mNumber, "M/26/006");
      assert.ok(paidBody.cards[0]?.body.includes("18(1A)"));
      assert.ok(paidBody.cards[0]?.body.includes("Share Purchase Agreement"));
      assert.ok(paidBody.cards[0]?.body.includes("Vertical Relationship"));
      assert.ok(paidBody.cards[0]?.body.includes("40km radius of Kehoe"));
      assert.ok(paidBody.cards[0]?.body.includes("Associated Hardware/National Hardware"));
    },
  );

  const bkartaDir = mkdtempSync(join(tmpdir(), "bkarta-entscheidungen-"));
  writeFileSync(
    join(bkartaDir, "snapshot.json"),
    JSON.stringify({
      ok: true,
      ...
      sources: {
        ...
        pdfHost: "https://www.bundeskartellamt.de/SharedDocs/Entscheidung/DE/Entscheidungen/",
      },
      cards: [
        {
          id: "amazon-b2-73-20",
          ...
          body: [
            ...
            "Amazon.com, Inc.",
            "hat die 2. Beschlussabteilung am 04.02.2026 beschlossen:",
            "Price Error Prevention",
            "Atypical Pricing",
            "AP-FOD",
            "SC-FOD",
            ...Array.from({ length: 40 }, (_, i) => `${i + 21}. Numbered article ${i + 21} from the official Amazon BKartA Verfügung body used only to keep this door fixture above the real-Entscheidung length floor.`),
          ].join("\n"),
        },
      ],
    }),
  ...
  await withServer(
    {
      BKARTA_ENTSCHEIDUNGEN_DIR: bkartaDir,
      X402_SKIP_SETTLE: "1",
      FORM_483_DIR: join(tmpdir(), "form-483-absent-bkarta-entscheidungen-"),
    },
    async (base) => {
      const unpaid = await fetch(`${base}${BKARTA_ENTSCHEIDUNGEN_PATH}`);
      assert.equal(unpaid.status, 402, "unpaid GET /bkarta-entscheidungen must be 402");
      const body402 = (await unpaid.json()) as {
        payTo: string;
        asset: string;
        resource: string;
      ...
      assert.equal(body402.resource, BKARTA_ENTSCHEIDUNGEN_PATH);
      assert.equal(body402.accepts[0]?.maxAmountRequired, BKARTA_ENTSCHEIDUNGEN_AMOUNT_ATOMIC);
      assert.equal(body402.accepts[0]?.extra?.name, "USD Coin");
      const bkartaPr = unpaid.headers.get("payment-required");
      assert.ok(bkartaPr, "v2 PAYMENT-REQUIRED header");

      const leak402 = JSON.stringify(body402);
      assert.ok(!leak402.includes("Price Error Prevention"));
      assert.ok(!leak402.includes("Atypical Pricing"));
      assert.ok(!leak402.includes("AP-FOD"));
      assert.ok(!leak402.includes("SC-FOD"));
      assert.ok(!/\bPEP\b/.test(leak402));
      assert.ok(!leak402.includes("Numbered article"));
      ...
      const man = (await manifest.json()) as {
        cardCount?: number;
        cards?: { institution?: string; az?: string; id?: string; body?: string }[];
        openapi?: string;
        wellKnown?: string;
      };
      assert.equal(man.cardCount, 1);
      assert.equal(man.cards?.[0]?.institution, "Amazon.com, Inc. / Amazon EU S.à r.l.");
      assert.equal(man.cards?.[0]?.az, "B2-73/20");
      const manBlob = JSON.stringify(man);
      assert.ok(!manBlob.includes("Price Error Prevention"));
      assert.ok(!manBlob.includes("Atypical Pricing"));
      assert.ok(!manBlob.includes("AP-FOD"));
      assert.ok(!manBlob.includes("SC-FOD"));
      assert.ok(!/\bPEP\b/.test(manBlob));
      assert.ok(!("body" in (man.cards?.[0] ?? {})));

      const paid = await fetch(`${base}${BKARTA_ENTSCHEIDUNGEN_PATH}`, { headers: { "X-PAYMENT": "test" } });
      assert.equal(paid.status, 200);
      const paidBody = (await paid.json()) as {
        product: string;
        cards: { institution: string; date: string; az: string; body: string }[];
      };
      assert.equal(paidBody.product, "bkarta-institution-entscheidung-bodies");
      assert.equal(paidBody.cards[0]?.institution, "Amazon.com, Inc. / Amazon EU S.à r.l.");
      assert.equal(paidBody.cards[0]?.date, "2026-02-04");
      assert.equal(paidBody.cards[0]?.az, "B2-73/20");
      assert.ok(paidBody.cards[0]?.body.includes("Price Error Prevention"));
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      sources: {
        ...
        pdfHost: "https://www2.fmc.gov/readingroom/",
      },
      cards: [
        {
          id: "msc-23-08",
          ...
          body: [
            ...
            "overcharges appeared to have occurred 2,629 times",
            "Securities and Exchange Commission v. Jarkesy, 603 U.S. 109",
            "Descartes database, which includes all rates and rules",
            "civil penalties against MSC in the total amount of $22,670,000.",
            "Administrative Law Judge",
            ...Array.from({ length: 40 }, (_, i) => `${i + 21}. Numbered paragraph ${i + 21} from the official MSC FMC order body used only to keep this door fixture above the real-order length floor.`),
          ].join("\n"),
        },
      ],
    }),
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(body402.accepts[0]?.extra?.name, "USD Coin");
      const fmcPr = unpaid.headers.get("payment-required");
      assert.ok(fmcPr, "v2 PAYMENT-REQUIRED header");

      const leak402 = JSON.stringify(body402);
      assert.ok(!leak402.includes("2,629"));
      assert.ok(!leak402.includes("Jarkesy"));
      assert.ok(!leak402.includes("Descartes"));
      assert.ok(!leak402.includes("22,670,000"));
      assert.ok(!leak402.includes("Numbered paragraph"));
      ...
      assert.equal(man.cardCount, 1);
      assert.equal(man.cards?.[0]?.institution, "MSC Mediterranean Shipping Company S.A.");
      assert.equal(man.cards?.[0]?.docket, "msc-23-08");
      const manBlob = JSON.stringify(man);
      assert.ok(!manBlob.includes("2,629"));
      assert.ok(!manBlob.includes("Jarkesy"));
      assert.ok(!manBlob.includes("Descartes"));
      assert.ok(!manBlob.includes("22,670,000"));
      assert.ok(!("body" in (man.cards?.[0] ?? {})));
      ...
      assert.equal(paidBody.product, "fmc-institution-order-bodies");
      assert.equal(paidBody.cards[0]?.institution, "MSC Mediterranean Shipping Company S.A.");
      assert.equal(paidBody.cards[0]?.date, "2026-01-06");
      assert.equal(paidBody.cards[0]?.docket, "msc-23-08");
      assert.ok(paidBody.cards[0]?.body.includes("2,629"));
  ...
  const atsdrHcDir = mkdtempSync(join(tmpdir(), "atsdr-hc-"));
  writeFileSync(
    join(atsdrHcDir, "snapshot.json"),
    JSON.stringify({
      ok: true,
      ...
      sources: {
        listing: "https://wwwn.cdc.gov/TSP/PHA/PHAListing.aspx",
        pdfHost: "https://www.atsdr.cdc.gov/HAC/pha/",
      },
      cards: [
        {
          ...
          body: [
            ...
            "Health Consultation",
            "Evaluation of Ethylene Oxide in Outdoor Air Near the Sterigenics Sterilization Facility",
            "Cobb County, Georgia",
            "USEPA Facility ID: 110000355963",
            "Final Release August 18, 2026",
            "The lifetime excess risk of cancer from breathing EtO for people who live, work, or attend school near Sterigenics is similar to background.",
            "EtO concentrations measured at sampling locations near Sterigenics after Sterigenics reopened were not statistically greater than the background EtO concentrations.",
            ...Array.from({ length: 20 }, (_, i) => `${i + 1}. Official ATSDR Health Consultation paragraph ${i + 1} used only to keep this door fixture above the real-consultation length floor.`),
          ].join("\n"),
        },
      ],
    }),
  ...
  await withServer(
    {
      ATSDR_HC_DIR: atsdrHcDir,
      X402_SKIP_SETTLE: "1",
      FORM_483_DIR: join(tmpdir(), "form-483-absent-atsdr-hc-"),
    },
    async (base) => {
      const unpaid = await fetch(`${base}${ATSDR_HC_PATH}`);
      assert.equal(unpaid.status, 402, "unpaid GET /atsdr-hc must be 402");
      const body402 = (await unpaid.json()) as {
        payTo: string;
        asset: string;
        resource: string;
      ...
      assert.equal(body402.resource, ATSDR_HC_PATH);
      assert.equal(body402.accepts[0]?.maxAmountRequired, ATSDR_HC_AMOUNT_ATOMIC);
      assert.equal(body402.accepts[0]?.extra?.name, "USD Coin");
      const atsdrPr = unpaid.headers.get("payment-required");
      assert.ok(atsdrPr, "v2 PAYMENT-REQUIRED header");

      const leak402 = JSON.stringify(body402);
      assert.ok(!leak402.includes("lifetime excess risk"));
      assert.ok(!leak402.includes("not statistically greater"));
      assert.ok(!leak402.includes("110000355963"));
      assert.ok(!leak402.includes("Official ATSDR Health Consultation paragraph"));
      ...
      assert.equal(man.cardCount, 1);
      assert.equal(man.cards?.[0]?.institution, "Sterigenics LLC");
      assert.equal(man.cards?.[0]?.date, "2026-08-18");
      const manBlob = JSON.stringify(man);
      assert.ok(!manBlob.includes("lifetime excess risk"));
      assert.ok(!manBlob.includes("not statistically greater"));
      assert.ok(!manBlob.includes("110000355963"));
      assert.ok(!("body" in (man.cards?.[0] ?? {})));

      const paid = await fetch(`${base}${ATSDR_HC_PATH}`, { headers: { "X-PAYMENT": "test" } });
      assert.equal(paid.status, 200);
      const paidBody = (await paid.json()) as {
        product: string;
        cards: { institution: string; date: string; body: string }[];
      };
      assert.equal(paidBody.product, "atsdr-hc-consultation-bodies");
      assert.equal(paidBody.cards[0]?.institution, "Sterigenics LLC");
      assert.equal(paidBody.cards[0]?.date, "2026-08-18");
  ...
  const doeNovDir = mkdtempSync(join(tmpdir(), "doe-nov-"));
  writeFileSync(
    join(doeNovDir, "snapshot.json"),
    JSON.stringify({
      ok: true,
      ...
      fetchedAt: FRESH_FETCHED_AT,
      asOf: "2026-01-15",
      license: "17 U.S.C. § 105 + energy.gov public-domain notice",
      attribution:
      ...
      sources: {
        listing: "https://www.energy.gov/ea/listings/final-notices-violation",
      ...
      cards: [
        {
          ...
          body: [
            ...
            "NEA-2026-01",
            "Title 10 C.F.R. § 830.122, subsection (a), Criterion 1—Management/Program",
            "failed to prevent the operation of crane X-0-CHS-CN-002 beyond its required inspection due date",
            "Base Civil Penalty – $131,000.",
            "Imposed Civil Penalty – $131,000.",
            ...Array.from({ length: 20 }, (_, i) => `${i + 1}. Official DOE Office of Enforcement FNOV paragraph ${i + 1} used only to keep this door fixture above the real-letter length floor.`),
          ].join("\n"),
        },
      ],
    }),
  ...
  await withServer(
    {
      DOE_NOV_DIR: doeNovDir,
      X402_SKIP_SETTLE: "1",
      FORM_483_DIR: join(tmpdir(), "form-483-absent-doe-nov-"),
    },
    async (base) => {
      const unpaid = await fetch(`${base}${DOE_NOV_PATH}`);
      assert.equal(unpaid.status, 402, "unpaid GET /doe-nov must be 402");
      const body402 = (await unpaid.json()) as {
        payTo: string;
        asset: string;
        resource: string;
      ...
      assert.equal(body402.resource, DOE_NOV_PATH);
      assert.equal(body402.accepts[0]?.maxAmountRequired, DOE_NOV_AMOUNT_ATOMIC);
      assert.equal(body402.accepts[0]?.extra?.name, "USD Coin");
      const doePr = unpaid.headers.get("payment-required");
      assert.ok(doePr, "v2 PAYMENT-REQUIRED header");

      const leak402 = JSON.stringify(body402);
      assert.ok(!leak402.includes("$131,000"));
      assert.ok(!leak402.includes("830.122"));
      assert.ok(!leak402.includes("crane X-0-CHS-CN-002"));
      assert.ok(!leak402.includes("Official DOE Office of Enforcement FNOV paragraph"));
      ...
      const manifest = await fetch(`${base}${DOE_NOV_MANIFEST_PATH}`);
      assert.equal(manifest.status, 200, "doe-nov free manifest is free");
      const man = (await manifest.json()) as {
        cardCount?: number;
        cards?: { institution?: string; date?: string; id?: string; docket?: string; body?: string }[];
      };
      assert.equal(man.cardCount, 1);
      assert.equal(man.cards?.[0]?.institution, "Mid-America Conversion Services, LLC");
      assert.equal(man.cards?.[0]?.date, "2026-01-15");
      assert.equal(man.cards?.[0]?.docket, "NEA-2026-01");
      const manBlob = JSON.stringify(man);
      assert.ok(!manBlob.includes("$131,000"));
      assert.ok(!manBlob.includes("830.122"));
      assert.ok(!manBlob.includes("crane X-0-CHS-CN-002"));
      assert.ok(!("body" in (man.cards?.[0] ?? {})));

      const paid = await fetch(`${base}${DOE_NOV_PATH}`, { headers: { "X-PAYMENT": "test" } });
      assert.equal(paid.status, 200);
      const paidBody = (await paid.json()) as {
        product: string;
        cards: { institution: string; date: string; docket: string; body: string }[];
      };
      assert.equal(paidBody.product, "doe-nov-enforcement-bodies");
      assert.equal(paidBody.cards[0]?.institution, "Mid-America Conversion Services, LLC");
      assert.equal(paidBody.cards[0]?.date, "2026-01-15");
      assert.equal(paidBody.cards[0]?.docket, "NEA-2026-01");
      assert.ok(paidBody.cards[0]?.body.includes("$131,000"));
      assert.ok(paidBody.cards[0]?.body.includes("830.122"));
      assert.ok(paidBody.cards[0]?.body.includes("crane X-0-CHS-CN-002"));
    },
  );

  const nopAdDir = mkdtempSync(join(tmpdir(), "nop-ad-"));
  writeFileSync(
    join(nopAdDir, "snapshot.json"),
    JSON.stringify({
      ok: true,
      ...
      fetchedAt: FRESH_FETCHED_AT,
      asOf: "2026-07-30",
      license: "17 U.S.C. § 105 + AMS public information notice",
      attribution:
      ...
      sources: {
        listing: "https://www.ams.usda.gov/services/enforcement/organic/ams-decisions",
      ...
      cards: [
        {
          ...
          body: [
            ...
            "APL-014-25 and APL-049-25",
            "alleging it knowingly relabeled and sold nonorganic frozen diced peaches as organic.",
            "The USDA organic regulations at 7 CFR 205.400, General requirements for certification,",
            ...Array.from({ length: 20 }, (_, i) => `${i + 1}. Official AMS NOP Administrator Decision paragraph ${i + 1} used only to keep this door fixture above the real-letter length floor.`),
          ].join("\n"),
        },
      ],
    }),
  ...
  await withServer(
    {
      NOP_AD_DIR: nopAdDir,
      X402_SKIP_SETTLE: "1",
      FORM_483_DIR: join(tmpdir(), "form-483-absent-nop-ad-"),
    },
    async (base) => {
      const unpaid = await fetch(`${base}${NOP_AD_PATH}`);
      assert.equal(unpaid.status, 402, "unpaid GET /nop-ad must be 402");
      const body402 = (await unpaid.json()) as {
        payTo: string;
        asset: string;
        resource: string;
      ...
      assert.equal(body402.resource, NOP_AD_PATH);
      assert.equal(body402.accepts[0]?.maxAmountRequired, NOP_AD_AMOUNT_ATOMIC);
      assert.equal(body402.accepts[0]?.extra?.name, "USD Coin");
      const nopPr = unpaid.headers.get("payment-required");
      assert.ok(nopPr, "v2 PAYMENT-REQUIRED header");

      const leak402 = JSON.stringify(body402);
      assert.ok(!leak402.includes("diced peaches"));
      assert.ok(!leak402.includes("knowingly relabeled"));
      assert.ok(!leak402.includes("205.400"));
      assert.ok(!leak402.includes("Official AMS NOP Administrator Decision paragraph"));
      ...
      const manifest = await fetch(`${base}${NOP_AD_MANIFEST_PATH}`);
      assert.equal(manifest.status, 200, "nop-ad free manifest is free");
      const man = (await manifest.json()) as {
        cardCount?: number;
        cards?: { institution?: string; date?: string; id?: string; docket?: string; body?: string }[];
      };
      assert.equal(man.cardCount, 1);
      assert.equal(man.cards?.[0]?.institution, "Mapeks USA, LLC dba Mac Global");
      assert.equal(man.cards?.[0]?.date, "2026-07-30");
      assert.equal(man.cards?.[0]?.docket, "APL-014-25");
      const manBlob = JSON.stringify(man);
      assert.ok(!manBlob.includes("diced peaches"));
      assert.ok(!manBlob.includes("knowingly relabeled"));
      assert.ok(!manBlob.includes("205.400"));
      assert.ok(!("body" in (man.cards?.[0] ?? {})));

      const paid = await fetch(`${base}${NOP_AD_PATH}`, { headers: { "X-PAYMENT": "test" } });
      assert.equal(paid.status, 200);
      const paidBody = (await paid.json()) as {
        product: string;
        cards: { institution: string; date: string; docket: string; body: string }[];
      };
      assert.equal(paidBody.product, "nop-ad-decision-bodies");
      assert.equal(paidBody.cards[0]?.institution, "Mapeks USA, LLC dba Mac Global");
      assert.equal(paidBody.cards[0]?.date, "2026-07-30");
      assert.equal(paidBody.cards[0]?.docket, "APL-014-25");
      assert.ok(paidBody.cards[0]?.body.includes("diced peaches"));
      assert.ok(paidBody.cards[0]?.body.includes("knowingly relabeled"));
      assert.ok(paidBody.cards[0]?.body.includes("205.400"));
    },
  );
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.length, 30, "thirtieth product is /gmp when a real observation body is cached");
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.length, 31, "thirty-first product is /gmp-md when a real MD body is cached");
  ...
  await withServer(
    ...
    async (base) => {
      assert.equal(cdpEnvStatus(), "CDP env not set");
      for (const path of [TICKS_PATH, IMPORT_ALERTS_PATH, MARINERS_PATH, MARINERS_D11_PATH, MARINERS_D7_PATH, MARINERS_D8_PATH, WARNING_LETTERS_PATH, UNTITLED_LETTERS_PATH, AWA_PATH, SWISSPAR_PATH, PCAC_PATH, FTC_WL_PATH, CFPB_ORDERS_PATH, OCC_CD_PATH, FDIC_ORDERS_PATH, FRB_ORDERS_PATH, NCUA_ORDERS_PATH, FINCEN_ORDERS_PATH, FERC_ORDERS_PATH, OFAC_ORDERS_PATH, BIS_ORDERS_PATH, CFTC_ORDERS_PATH, FIFRA_ORDERS_PATH, DENOVO_ORDERS_PATH, TTB_OIC_PATH, AIR_LETTERS_PATH, SUPERFUND_RODS_PATH, ICO_MPN_PATH, FORM_483_PATH, GMP_PATH, GMP_MD_PATH]) {
  ...
  const hiddenCcpc = facilitatorPaymentRequirements("https://ticks.bnm.farm/ccpc-mergers", "ccpc-mergers");
  ...
  const hiddenDoeNov = facilitatorPaymentRequirements("https://ticks.bnm.farm/doe-nov", "doe-nov");
