...
async function main(): Promise<void> {
  await withServer({
    ...
    assert.equal(wk.resources.length, 20, "well-known lists the twenty always-public doors");
    ...
    assert.equal(
      ...
      20,
      "OpenAPI lists the twenty always-public paid paths",
    );
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.length, 22, "twenty-second product is /gmp when a real observation body is cached");
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.equal(shop.products.length, 23, "twenty-third product is /gmp-md when a real MD body is cached");
  ...
  await withServer(
    ...
    async (base) => {
      assert.equal(cdpEnvStatus(), "CDP env not set");
      for (const path of [TICKS_PATH, IMPORT_ALERTS_PATH, MARINERS_PATH, MARINERS_D11_PATH, MARINERS_D7_PATH, MARINERS_D8_PATH, WARNING_LETTERS_PATH, UNTITLED_LETTERS_PATH, AWA_PATH, SWISSPAR_PATH, PCAC_PATH, FTC_WL_PATH, CFPB_ORDERS_PATH, OCC_CD_PATH, FDIC_ORDERS_PATH, FRB_ORDERS_PATH, NCUA_ORDERS_PATH, FINCEN_ORDERS_PATH, FERC_ORDERS_PATH, OFAC_ORDERS_PATH, FORM_483_PATH, GMP_PATH, GMP_MD_PATH]) {
  ...
  assert.deepEqual(PUBLIC_BAZAAR_SKUS, ["ticks", "import-alerts", "mariners", "mariners-d11", "mariners-d7", "mariners-d8", "warning-letters", "untitled-letters", "awa", "swisspar", "pcac", "ftc-wl", "cfpb-orders", "occ-cd", "fdic-orders", "frb-orders", "ncua-orders", "fincen-orders", "ferc-orders", "ofac-orders"]);
