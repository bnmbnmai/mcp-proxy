...
function loadCachedKey(): string | null {
  try {
    if (!fs.existsSync(KEY_CACHE_FILE)) return null;
 ...
 */
async function autoProvisionKey(): Promise<string | null> {
  try {
    ...
    const key = data.key;
    if (typeof key !== "string" || !key.startsWith("ak_")) return null;
...
server.tool(
  ...
  async ({ ip, max_age = 30 }) => {
    ...
    const result = await makeApolloRequest<any>("/api/ip-reputation", {
      ip, max_age: String(max_age),
    });
    { const err = handleToolError(result, "IP reputation"); if (err) return err; }
    const d = result.data!;
    const data = d.data || d;
    const parts = [
    ...
    if (data.reports?.length) {
      ...
      (data.reports as any[]).slice(0, 5).forEach((r: any) => {
...
server.tool(
  ...
  async ({ url, wait = false }) => {
    ...
    const result = await makeApolloRequest<any>("/api/malware/url", {
      url, wait: String(wait),
    });
    { const err = handleToolError(result, "Malware URL check"); if (err) return err; }
    const d = result.data!;
...
server.tool(
  "breach_check",
  "Email data breach check via Have I Been Pwned (Troy Hunt). Check if an email has appeared in known data breaches. Returns breach count, risk level, and security recommendations. $0.05/query via x402.",
  {
  ...
  async ({ email, full = false }) => {
    ...
    const result = await makeApolloRequest<any>("/api/breach-check", params);
    { const err = handleToolError(result, "Breach check"); if (err) return err; }
    const d = result.data!;
    const parts = [
    ...
    if (d.breaches?.length) {
      parts.push(``, `### Breaches`);
      (d.breaches as any[]).slice(0, 10).forEach((b: any) => {
        parts.push(`- **${b.Name || b.name}** (${b.BreachDate || b.date || "?"}) — ${b.PwnCount?.toLocaleString() || "?"} accounts`);
        if (full && b.DataClasses) parts.push(`  Data: ${b.DataClasses.join(", ")}`);
      });
    }
    if (d.recommendations?.length) {
      parts.push(``, `### Recommendations`);
      (d.recommendations as string[]).forEach((r: string) => parts.push(`- ${r}`));
    }
...
server.tool(
  "eth_balance",
  "Ethereum address ETH balance lookup via Etherscan. Returns balance in wei and formatted ETH. Essential for crypto wallets and DeFi agents. $0.03/query via x402.",
  {
    address: z.string().describe("Ethereum address (0x...)"),
  },
  async ({ address }) => {
    console.error(`[apollo-mcp] eth_balance: ${address}`);
    const result = await makeApolloRequest<any>("/api/eth/balance", { address });
    { const err = handleToolError(result, "ETH balance"); if (err) return err; }
    const d = result.data!;
    const parts = [
    ...
    if (d.usd_value) parts.push(`**USD Value:** $${d.usd_value.toLocaleString()}`);
...
server.tool(
  "eth_transactions",
  "Ethereum transaction history for an address via Etherscan. Returns recent transactions with value, gas, timestamps, and confirmation details. $0.03/query via x402.",
  {
    address: z.string().describe("Ethereum address (0x...)"),
  ...
  async ({ address, page = 1, offset = 10 }) => {
    ...
    const result = await makeApolloRequest<any>("/api/eth/transactions", {
      address, page: String(page), offset: String(offset),
    });
    { const err = handleToolError(result, "ETH transactions"); if (err) return err; }
    const d = result.data!;
    const txs = d.transactions || [];
    const parts = [`## ETH Transactions: ${d.address || address}`, `**Total:** ${txs.length} transactions`, ``];
    txs.slice(0, offset).forEach((tx: any, i: number) => {
      ...
      if (tx.timeStamp) parts.push(`   ${new Date(Number(tx.timeStamp) * 1000).toISOString().slice(0, 19)} | Gas: ${tx.gasPrice ? (Number(tx.gasPrice) / 1e9).toFixed(1) + " gwei" : "?"}`);
    });
    parts.push(``, `*Source: Etherscan*`);
    return { content: [{ type: "text" as const, text: parts.join("\n") }] };
  }
...
server.tool(
  "eth_gas",
  "Current Ethereum gas prices via Etherscan. Returns safe, standard, and fast gas prices in gwei with estimated USD costs for common transactions. $0.02/query via x402.",
  {},
  async () => {
    console.error(`[apollo-mcp] eth_gas`);
    const result = await makeApolloRequest<any>("/api/eth/gas");
    { const err = handleToolError(result, "ETH gas"); if (err) return err; }
    const d = result.data!;
    const parts = [`## Ethereum Gas Prices`];
    if (d.gas_prices) {
      const g = d.gas_prices;
      parts.push(
        `**Safe:** ${g.safe ?? g.SafeGasPrice ?? "?"} gwei`,
        `**Standard:** ${g.standard ?? g.ProposeGasPrice ?? "?"} gwei`,
        `**Fast:** ${g.fast ?? g.FastGasPrice ?? "?"} gwei`,
      );
    } else {
    ...
    if (d.estimated_costs) {
      ...
      for (const [type, cost] of Object.entries(d.estimated_costs as Record<string, any>)) {
