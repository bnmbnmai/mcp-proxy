...
export function buildForm483Manifest(snap: Form483Snapshot | null): Record<string, unknown> {
  ...
  return {
    ...
    name: PRODUCT_NAME,
    free: true,
    note: "Count + id + firm + dates only. Observation / letter body is the paid GET /form-483 payload. This free manifest lists the full catalog. GET ?id= is one official text for $0.02. Default GET / page of up to 100 official texts is $0.05 (newest 100 official texts, not the entire archive). Older pages are another $0.05 on the same URL (page/before).",
