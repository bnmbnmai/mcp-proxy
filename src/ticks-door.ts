 ...
 * GET /ipo-tm — IPO trade-mark inter partes hearing PDF text ($0.05) (prep; unlisted)
 ...
 * GET /atsdr-hc — ATSDR PHA / Health Consultation PDF text ($0.05) (prep; unlisted)
 * GET /atsdr-hc/manifest.json — free count + site/company/date/sourceUrl (no consultation body)
 * GET /waterboards-acl — California Water Boards ACL order PDF text ($0.05) (prep; unlisted)
 ...
 * GET /nlrb-bd — NLRB Board Decision / Decision and Order PDF text ($0.05) (prep; unlisted)
 * GET /nlrb-bd/manifest.json — free count + employer/case/date/sourceUrl (no body)
...
export type DoorSku = "ticks" | "import-alerts" | "mariners" | "mariners-d11" | "mariners-d7" | "mariners-d8" | "warning-letters" | "untitled-letters" | "awa" | "swisspar" | "pcac" | "ftc-wl" | "cfpb-orders" | "occ-cd" | "fdic-orders" | "frb-orders" | "ncua-orders" | "fincen-orders" | "ferc-orders" | "ofac-orders" | "bis-orders" | "cftc-orders" | "fifra-orders" | "denovo-orders" | "ttb-oic" | "air-letters" | "superfund-rods" | "ico-mpn" | "phmsa-cop" | "acm-besluiten" | "ccpc-mergers" | "bkarta-entscheidungen" | "ipo-tm" | "fmc-orders" | "fsis-hmsa" | "atsdr-hc" | "waterboards-acl" | "doe-nov" | "nop-ad" | "nlrb-bd" | "form-483" | "gmp" | "gmp-md";
...
function amountAtomicFor(sku: DoorSku): string {
  ...
  if (sku === "ipo-tm") {
    const raw = env("IPO_TM_USDC_ATOMIC");
    return raw.length > 0 ? raw : IPO_TM_AMOUNT_ATOMIC;
  }
  if (sku === "fmc-orders") {
  ...
  if (sku === "atsdr-hc") {
    const raw = env("ATSDR_HC_USDC_ATOMIC");
    return raw.length > 0 ? raw : ATSDR_HC_AMOUNT_ATOMIC;
  }
  if (sku === "waterboards-acl") {
    const raw = env("WATERBOARDS_ACL_USDC_ATOMIC");
    return raw.length > 0 ? raw : WATERBOARDS_ACL_AMOUNT_ATOMIC;
  }
  if (sku === "doe-nov") {
  ...
  if (sku === "nlrb-bd") {
    const raw = env("NLRB_BD_USDC_ATOMIC");
    return raw.length > 0 ? raw : NLRB_BD_AMOUNT_ATOMIC;
  }
  if (sku === "form-483") {
...
const SKU_COPY: Record<DoorSku, { description: string; resourcePath: string }> = {
  ...
  "ipo-tm": {
    description:
      "Call GET /ipo-tm when you need official UK IPO institution/company trade-mark inter partes hearing TEXT extracted with pdftotext from ipo.gov.uk/t-challenge-decision-results/ PDFs. Not people-applicant SKUs. Not the year-grid/card. Not TM Journal XML. Not design_decision. Not GOV.UK trademark_decision. Not Find Case Law. Not ICO /ico-mpn. Not PHMSA /phmsa-cop. Not ACM /acm-besluiten. Not CCPC /ccpc-mergers. Not BKartA /bkarta-entscheidungen. Not CMA /cma-ca98. Crown + OGL v3.0. Prep only — do not list.",
    resourcePath: IPO_TM_PATH,
  },
  "fmc-orders": {
    description:
  ...
  "atsdr-hc": {
    description:
      "Call GET /atsdr-hc when you need official ATSDR PHA / Health Consultation TEXT extracted with pdftotext from atsdr.cdc.gov/HAC/pha/{site}/{file}-508.pdf. Not people. Not the state listing HTML. Not the 2-page factsheet teaser. Not /superfund-rods. Not /fsis-hmsa. Not Charity Commission. 17 U.S.C. § 105. Prep only — do not list.",
    resourcePath: ATSDR_HC_PATH,
  },
  "waterboards-acl": {
    description:
      "Call GET /waterboards-acl when you need official California Water Boards ACL order TEXT (stipulated / hearing / MMP-as-ACL) extracted with pdftotext from waterboards.ca.gov/{region}/board_decisions/adopted_orders/{year}/ PDFs. Company/institution only. Not people. Not the discretionary-ACL table + transmittal. Not CIWQS / data.ca.gov. Not ECHO. Not /superfund-rods. Not /atsdr-hc. Public domain (conditions_of_use.html). Prep only — do not list.",
    resourcePath: WATERBOARDS_ACL_PATH,
  },
  "doe-nov": {
    description:
  ...
  "nlrb-bd": {
    description:
      "Call GET /nlrb-bd when you need official NLRB Board Decision / Decision and Order / Supplemental Decision and Order TEXT extracted with pdftotext from apps.nlrb.gov/link/document.aspx/{id} slip-opinion PDFs. Company/institution NLRA ULP / representation only. Not people. Not the Board Decisions listing. Not case-page 8(a) teasers. Not CiteNet. Not case-search CSV. Not ALJ-only. 17 U.S.C. § 105. Prep only — do not list.",
    resourcePath: NLRB_BD_PATH,
  },
  "form-483": {
    description:
...
const BAZAAR_OUTPUT_EXAMPLE: Record<DoorSku, Record<string, unknown>> = {
  ...
  "ipo-tm": {
    ...
    cards: [
      {
        id: "sample-hearing",
        institution: "Applicant Limited / Opponent Limited",
        bl: "O/0000/26",
        date: "2026-01-01",
        sourceUrl: "https://www.ipo.gov.uk/t-challenge-decision-results/o000026.pdf",
        body: "TRADE MARKS ACT 1994\nIN THE MATTER OF\nFor the Registrar",
      },
    ],
  },
  "fmc-orders": {
    ...
    cards: [
      {
        ...
        institution: "Example Facility LLC",
        date: "2026-01-01",
        sourceUrl: "https://www.atsdr.cdc.gov/HAC/pha/Example/Example-HC-508.pdf",
        body: "Agency for Toxic Substances and Disease Registry\nHealth Consultation\nExample Facility LLC",
      },
    ],
  },
  "waterboards-acl": {
    ...
    cards: [
      {
        id: "sample-acl",
        institution: "Example Sanitary District",
        orderNumber: "R3-2026-0000",
        date: "2026-01-01",
        sourceUrl: "https://www.waterboards.ca.gov/centralcoast/board_decisions/adopted_orders/2026/2026-0000-example-aclo.pdf",
        body: "CALIFORNIA REGIONAL WATER QUALITY CONTROL BOARD\nSettlement Agreement and Stipulation for Entry of Administrative Civil Liability Order\nExample Sanitary District",
      },
    ],
  },
  "doe-nov": {
    ...
    cards: [
      {
        id: "sample-fnov",
        institution: "Example Conversion Services, LLC",
        docket: "NEA-2026-00",
        date: "2026-01-01",
  ...
  "nlrb-bd": {
    ...
    cards: [
      {
        id: "sample-board-decision",
        institution: "Example Corporation",
        docket: "19-CA-000000",
        date: "2026-01-01",
        sourceUrl: "https://apps.nlrb.gov/link/document.aspx/09031d4580000001",
        body: "NOTICE: This opinion is subject to formal revision before publication in the bound volumes of NLRB decisions.\nExample Corporation\nDECISION AND ORDER\nNational Labor Relations Board\nAn Agency of the United States Government",
      },
    ],
  },
  "form-483": {
...
export async function handleRequest(req: IncomingMessage, res: ServerResponse, port: number): Promise<void> {
  ...
  if (path === IPO_TM_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadIpoTmManifest(), req, port));
    return;
  }

  if (path === IPO_TM_PATH) {
    await servePaid(req, res, port, "ipo-tm", () => loadIpoTm());
    return;
  }

  if (path === FMC_ORDERS_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadFmcOrdersManifest(), req, port));
    return;
  }

  if (path === FMC_ORDERS_PATH) {
    await servePaid(req, res, port, "fmc-orders", () => loadFmcOrders());
    return;
  }
  ...
  if (path === ATSDR_HC_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadAtsdrHcManifest(), req, port));
    return;
  }

  if (path === ATSDR_HC_PATH) {
    await servePaid(req, res, port, "atsdr-hc", () => loadAtsdrHc());
    return;
  }

  if (path === WATERBOARDS_ACL_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadWaterboardsAclManifest(), req, port));
    return;
  }

  if (path === WATERBOARDS_ACL_PATH) {
    await servePaid(req, res, port, "waterboards-acl", () => loadWaterboardsAcl());
    return;
  }

  if (path === DOE_NOV_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadDoeNovManifest(), req, port));
    return;
  }

  if (path === DOE_NOV_PATH) {
    await servePaid(req, res, port, "doe-nov", () => loadDoeNov());
    return;
  }
  ...
  if (path === NLRB_BD_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadNlrbBdManifest(), req, port));
    return;
  }

  if (path === NLRB_BD_PATH) {
    await servePaid(req, res, port, "nlrb-bd", () => loadNlrbBd());
    return;
  }
...
if (isMain()) {
  ...
  server.listen(port, host, () => {
    ...
    console.error(`${NLRB_BD_PATH} $${Number(amountAtomicFor("nlrb-bd")) / 1e6} USDC (unlisted)`);
