 ...
 * GET /mariners-d1 — USCG D1 / Northeast Local Notice to Mariners ($0.05)
 * GET /mariners-d1/manifest.json — free count + official source (no notice body)
 * GET /mariners-d5 — USCG D5 / Mid-Atlantic Local Notice to Mariners ($0.05)
 * GET /mariners-d5/manifest.json — free count + official source (no notice body)
 * GET /mariners-d9 — USCG D9 / Great Lakes Local Notice to Mariners ($0.05)
 * GET /mariners-d9/manifest.json — free count + official source (no notice body)
 * GET /mariners-d14 — USCG D14 / Pacific Local Notice to Mariners ($0.05)
 * GET /mariners-d14/manifest.json — free count + official source (no notice body)
 * GET /mariners-d17 — USCG D17 / Alaska Local Notice to Mariners ($0.05)
 * GET /mariners-d17/manifest.json — free count + official source (no notice body)
 ...
 * GET /epa-alj — EPA OALJ Initial Decision and Order + ALJ Order PDF text ($0.02 id / $0.05 page)
 * GET /epa-alj/manifest.json — free count + case/docket/date/label (no decision body)
 * GET /epa-eab — EPA EAB Unpublished Final Order / Board Order PDF text ($0.02 id / $0.05 page)
 * GET /epa-eab/manifest.json — free count + case/appeal/date/Type/Result (no Board narrative)
 * GET /faa-civil-penalty — FAA Civil Penalty Appeals Administrator Order PDF text ($0.02 id / $0.05 page)
 * GET /faa-civil-penalty/manifest.json — free count + order no/case/docket/dates/subjects (no Order narrative)
 * GET /stb-decisions — STB Board Decision / Order PDF text ($0.02 id / $0.05 page)
 * GET /stb-decisions/manifest.json — free count + docket/Decision ID/date/deciding body (no Board narrative)
 * GET /oalj-decisions — DOL OALJ / BALCA / ARB Decision and Order PDF text ($0.02 id / $0.05 page)
 ...
 * GET /nlrb-decisions — NLRB published Board Decision PDF text ($0.02 id / $0.05 page)
 * GET /nlrb-decisions/manifest.json — free count + citation/case/date/institution (no Board Decision narrative)
 * GET /flra-decisions — FLRA Authority Decision PDF text ($0.02 id / $0.05 page)
 ...
 * GET /nmb-determinations — NMB RLA §2 Ninth representation determination PDF text ($0.02 id / $0.05 page)
 * GET /nmb-determinations/manifest.json — free count + citation/case/date/institution (no determination narrative)
 * GET /eeoc-appellate — EEOC OFS appellate and reconsideration PDF text ($0.02 id / $0.05 page)
 * GET /eeoc-appellate/manifest.json — free count + citation/case/date/institution (no decision narrative)
 * GET /ttab-decisions — USPTO TTAB reading-room decision PDF text ($0.02 id / $0.05 page)
 * GET /ttab-decisions/manifest.json — free count + parties/mark/type/outcome/precedential/grounds (no decision narrative)
 * GET /ibla-decisions — DOI IBLA precedential decision PDF text ($0.02 id / $0.05 page)
 * GET /ibla-decisions/manifest.json — free count + citation/docket/parties/disposition/date (no decision narrative)
 * GET /ccb-determinations — Copyright Claims Board Final Determination PDF text ($0.02 id / $0.05 page)
 * GET /ccb-determinations/manifest.json — free count + docket/parties/caption/kind/date (no determination narrative)
...
import {
  ...
  MARINERS_D1_PATH,
  MARINERS_D5_MANIFEST_PATH,
  MARINERS_D5_PATH,
  MARINERS_D7_MANIFEST_PATH,
  MARINERS_D7_PATH,
  MARINERS_D8_MANIFEST_PATH,
  MARINERS_D8_PATH,
  MARINERS_D9_MANIFEST_PATH,
  MARINERS_D9_PATH,
  MARINERS_D11_MANIFEST_PATH,
  MARINERS_D11_PATH,
  MARINERS_D14_MANIFEST_PATH,
  MARINERS_D14_PATH,
  MARINERS_D17_MANIFEST_PATH,
  MARINERS_D17_PATH,
  MARINERS_MANIFEST_PATH,
  MARINERS_PATH,
  loadMariners,
  loadMarinersD1,
  loadMarinersD1Manifest,
  loadMarinersD5,
  loadMarinersD5Manifest,
  loadMarinersD7,
  loadMarinersD7Manifest,
  loadMarinersD8,
  loadMarinersD8Manifest,
  loadMarinersD9,
  loadMarinersD9Manifest,
  loadMarinersD11,
  loadMarinersD11Manifest,
  loadMarinersD14,
  loadMarinersD14Manifest,
  loadMarinersD17,
  loadMarinersD17Manifest,
...
import {
  ...
  paidFccEbOrdersBody,
  paidNmbDeterminationsBody,
  paidEeocAppellateBody,
  paidTtabDecisionsBody,
  paidIblaDecisionsBody,
  paidCcbDeterminationsBody,
  paidFsisHumaneBody,
...
function amountAtomicFor(sku: DoorSku): string {
  ...
  if (isMarinersSku(sku)) {
    const envName = sku === "mariners"
      ? "MARINERS_USDC_ATOMIC"
      : `MARINERS_${sku.slice("mariners-".length).toUpperCase().replace("-", "")}_USDC_ATOMIC`;
  ...
  if (sku === "oshrc-orders") {
    const raw = env("OSHRC_ORDERS_USDC_ATOMIC");
    return raw.length > 0 ? raw : OSHRC_ORDERS_AMOUNT_ATOMIC;
  }
  if (sku === "epa-alj") {
    const raw = env("EPA_ALJ_USDC_ATOMIC");
    return raw.length > 0 ? raw : EPA_ALJ_AMOUNT_ATOMIC;
  }
  if (sku === "epa-eab") {
    const raw = env("EPA_EAB_USDC_ATOMIC");
    return raw.length > 0 ? raw : EPA_EAB_AMOUNT_ATOMIC;
  }
  if (sku === "faa-civil-penalty") {
    const raw = env("FAA_CIVIL_PENALTY_USDC_ATOMIC");
    return raw.length > 0 ? raw : FAA_CIVIL_PENALTY_AMOUNT_ATOMIC;
  }
  if (sku === "stb-decisions") {
    const raw = env("STB_DECISIONS_USDC_ATOMIC");
    return raw.length > 0 ? raw : STB_DECISIONS_AMOUNT_ATOMIC;
  }
  if (sku === "oalj-decisions") {
    const raw = env("OALJ_DECISIONS_USDC_ATOMIC");
    return raw.length > 0 ? raw : OALJ_DECISIONS_AMOUNT_ATOMIC;
  }
  if (sku === "fmc-orders") {
  ...
  if (sku === "nlrb-decisions") {
    const raw = env("NLRB_DECISIONS_USDC_ATOMIC");
    return raw.length > 0 ? raw : NLRB_DECISIONS_AMOUNT_ATOMIC;
  }
  if (sku === "flra-decisions") {
    const raw = env("FLRA_DECISIONS_USDC_ATOMIC");
    return raw.length > 0 ? raw : FLRA_DECISIONS_AMOUNT_ATOMIC;
  }
  ...
  if (sku === "nmb-determinations") {
    const raw = env("NMB_DETERMINATIONS_USDC_ATOMIC");
    return raw.length > 0 ? raw : NMB_DETERMINATIONS_AMOUNT_ATOMIC;
  }
  if (sku === "eeoc-appellate") {
    const raw = env("EEOC_APPELLATE_USDC_ATOMIC");
    return raw.length > 0 ? raw : EEOC_APPELLATE_AMOUNT_ATOMIC;
  }
  if (sku === "ttab-decisions") {
    const raw = env("TTAB_DECISIONS_USDC_ATOMIC");
    return raw.length > 0 ? raw : TTAB_DECISIONS_AMOUNT_ATOMIC;
  }
  if (sku === "ibla-decisions") {
    const raw = env("IBLA_DECISIONS_USDC_ATOMIC");
    return raw.length > 0 ? raw : IBLA_DECISIONS_AMOUNT_ATOMIC;
  }
  if (sku === "ccb-determinations") {
    const raw = env("CCB_DETERMINATIONS_USDC_ATOMIC");
    return raw.length > 0 ? raw : CCB_DETERMINATIONS_AMOUNT_ATOMIC;
  }
  if (sku === "form-483") {
...
const IMPORT_ALERTS_FIRM_CHECK_COPY =
  `Free firm search across 483, warning letters, untitled letters, FTC/FMC/NMB/NLRB/FLRA/ECAB/FCC/EEOC/Ofwat/Ofgem/CFPB/OCC/FDIC, and this table: GET ${CANONICAL_ORIGIN}/firm-check?q=`;
...
function isMarinersSku(sku: DoorSku): boolean {
  return sku === "mariners" || sku === "mariners-d11" || sku === "mariners-d7" || sku === "mariners-d8"
    || sku === "mariners-d1" || sku === "mariners-d5" || sku === "mariners-d9" || sku === "mariners-d14"
    || sku === "mariners-d17";
}
...
const SKU_COPY: Record<DoorSku, { description: string; resourcePath: string }> = {
  ...
  "mariners-d1": {
    description:
      "Call GET /mariners-d1 when you need the latest USCG District 1 / Northeast Local Notice to Mariners as structured JSON from the official weekly PDF. Same NavCEN walker as /mariners. Returns week, section, text, and source URL. Does not invent notices. Paid JSON keeps notices[] and adds records[] (id, date, firm, url, type) plus asOf for diffs.",
    resourcePath: MARINERS_D1_PATH,
  },
  "mariners-d5": {
    description:
      "Call GET /mariners-d5 when you need the latest USCG District 5 / Mid-Atlantic Local Notice to Mariners as structured JSON from the official weekly PDF. Same NavCEN walker as /mariners. Returns week, section, text, and source URL. Does not invent notices. Paid JSON keeps notices[] and adds records[] (id, date, firm, url, type) plus asOf for diffs.",
    resourcePath: MARINERS_D5_PATH,
  },
  "mariners-d9": {
    description:
      "Call GET /mariners-d9 when you need the latest USCG District 9 / Great Lakes Local Notice to Mariners as structured JSON from the official weekly PDF. Same NavCEN walker as /mariners. Returns week, section, text, and source URL. Does not invent notices. Paid JSON keeps notices[] and adds records[] (id, date, firm, url, type) plus asOf for diffs.",
    resourcePath: MARINERS_D9_PATH,
  },
  "mariners-d14": {
    description:
      "Call GET /mariners-d14 when you need the latest USCG District 14 / Pacific Local Notice to Mariners as structured JSON from the official weekly PDF. Same NavCEN walker as /mariners. Returns week, section, text, and source URL. Does not invent notices. Paid JSON keeps notices[] and adds records[] (id, date, firm, url, type) plus asOf for diffs.",
    resourcePath: MARINERS_D14_PATH,
  },
  "mariners-d17": {
    description:
      "Call GET /mariners-d17 when you need the latest USCG District 17 / Alaska Local Notice to Mariners as structured JSON from the official weekly PDF. Same NavCEN walker as /mariners. Returns week, section, text, and source URL. Does not invent notices. Paid JSON keeps notices[] and adds records[] (id, date, firm, url, type) plus asOf for diffs.",
    resourcePath: MARINERS_D17_PATH,
  },
  "warning-letters": {
    description:
  ...
  "oshrc-orders": {
    description:
      "Call GET /oshrc-orders when you need official OSHRC ALJ Decision/Order or Commission Final Order TEXT extracted from oshrc.gov decision-search PDF/HTML caches. License 17 USC 105. Does not invent decision text. Index is operator / docket / date only — full TEXT is in the PDF/HTML. " +
      PAID_WINDOW_COPY,
    resourcePath: OSHRC_ORDERS_PATH,
  },
  "epa-alj": {
    description:
      "Call GET /epa-alj when you need official EPA OALJ Initial Decision and Order or substantive ALJ Order TEXT extracted from yosemite.epa.gov/oarm/alj/alj_web_docket.nsf PDFs. License 17 USC 105. Does not invent decision text. Index is case / docket / date / label only — full TEXT is in the PDF. Skip CAFO rows that twin live /fifra-orders / /epa-cafo. Not RHC oa/rhc/epaadmin.nsf. " +
      PAID_WINDOW_COPY,
    resourcePath: EPA_ALJ_PATH,
  },
  "epa-eab": {
    description:
      "Call GET /epa-eab when you need official EPA Environmental Appeals Board Unpublished Final Order or substantive Board Order TEXT extracted from yosemite.epa.gov/oa/EAB_Web_Docket.nsf PDFs. License 17 USC 105. Does not invent Board text. Index is case / appeal / date / Type / Result only — full TEXT is in the $FILE PDF. Prefer Type=Permit. Skip Penalty ESA/CAFO rows that twin live /epa-cafo / /fifra-orders. Not OALJ oarm/alj, not RHC oa/rhc, not /npdes-permits permit text. " +
      PAID_WINDOW_COPY,
    resourcePath: EPA_EAB_PATH,
  },
  "faa-civil-penalty": {
    description:
      "Call GET /faa-civil-penalty when you need official FAA Civil Penalty Appeals Administrator Final Order or Decision & Order TEXT extracted from drs.faa.gov CIVIL_PENALTY_APPEALS PDFs (14 CFR §13.233). License 17 USC 105. Does not invent Order text. Index is order no / case / docket / dates / subjects only — full TEXT is in the DRS PDF (guest session; bare GET is 403). Prefer substantive Order/Decision PDFs. Not /air-letters, not NMS/ASIAS JSON. " +
      PAID_WINDOW_COPY,
    resourcePath: FAA_CIVIL_PENALTY_PATH,
  },
  "stb-decisions": {
    description:
      "Call GET /stb-decisions when you need official Surface Transportation Board Board Decision or Order TEXT extracted from dcms-external.s3.amazonaws.com DCMS PDFs listed on stb.gov/proceedings-actions/decisions. License 17 USC 105. Does not invent Board text. Index is docket / Decision ID / date / deciding body only — full TEXT is in the DCMS PDF. Harvest STB/DCMS PDFs only. Skip FR-mirrored Decision Nos. 21/30 class. Open Data CSVs (STB-1145, EP 770, EP 724) are not this SKU. " +
      PAID_WINDOW_COPY,
    resourcePath: STB_DECISIONS_PATH,
  },
  "oalj-decisions": {
    description:
      "Call GET /oalj-decisions when you need official DOL OALJ, BALCA, or ARB Decision and Order TEXT extracted from oalj.dol.gov/DECISIONS/ALJ PDFs and dol.gov/sites/dolgov/files/OALJ/PUBLIC/ARB/DECISIONS PDFs listed on dol.gov/agencies/oalj caselists. License 17 USC 105. Does not invent Decision text. Index is case number / board / date / institution only — full TEXT is in the official PDF. Harvest DOL-authored Decision/Order PDFs only. OFLC disclosure XLSX/CSV, FR HTML wraps, and caselist HTML-only are not this SKU. Not /oshrc-orders, not /fmshrc-orders, not /stb-decisions, not /ecab-decisions. " +
      PAID_WINDOW_COPY,
    resourcePath: OALJ_DECISIONS_PATH,
  },
  "fmc-orders": {
    description:
  ...
  "nlrb-decisions": {
    description:
      "Call GET /nlrb-decisions when you need official NLRB published Board Decision TEXT extracted from apps.nlrb.gov/link/document.aspx PDFs indexed by the nlrb.gov Board Decisions AJAX sort. License 17 USC 105. Does not invent Decision text. Index is citation / case number / date / institution only — full TEXT is in the official PDF. Harvest NLRB-authored published Board Decision PDFs (NNN NLRB No. N) only. CiteNet, weekly summaries, ALJ-only, and MSPB full-text JSON are not this SKU. Not /flra-decisions, not /oalj-decisions, not /oshrc-orders, not /fmshrc-orders. " +
      PAID_WINDOW_COPY,
    resourcePath: NLRB_DECISIONS_PATH,
  },
  "flra-decisions": {
    description:
      "Call GET /flra-decisions when you need official FLRA Authority Decision TEXT extracted from www.flra.gov/system/files/decisions PDFs indexed by the flra.gov Authority Decisions table. License 17 USC 105. Does not invent Decision text. Index is citation / case number / date / institution only — full TEXT is in the official PDF. Harvest FLRA-authored Authority Decision PDFs (NN FLRA NNN) only. Case digests, ALJ decisions, FSIP decisions, and the HTML slip are not this SKU. No-auth JSON on the decision route does not return the opinion. Not /nlrb-decisions. " +
      PAID_WINDOW_COPY,
    resourcePath: FLRA_DECISIONS_PATH,
  },
  "ecab-decisions": {
    description:
  ...
  "nmb-determinations": {
    description:
      "Call GET /nmb-determinations when you need official National Mediation Board Railway Labor Act §2 Ninth representation determination TEXT extracted from nmb.gov WordPress PDFs indexed by the FY Determinations HTML table. License 17 USC 105. Does not invent determination text. Index is citation / case / date / institution only — full TEXT is in the official PDF. Harvest representation Findings Upon Investigation, certification, and single-carrier determination PDFs. The Representation Manual, PEB reports, jurisdictional opinion letters, and the comments RSS are not this SKU. Not /nlrb-decisions. Not /flra-decisions. " +
      PAID_WINDOW_COPY,
    resourcePath: NMB_DETERMINATIONS_PATH,
  },
  "eeoc-appellate": {
    description:
      "Call GET /eeoc-appellate when you need official EEOC Office of Federal Sector appellate and reconsideration TEXT extracted from eeoc.gov PDFs indexed by the Federal Sector Appellate Decisions search. License 17 USC 105. Does not invent decision text. Index is citation / case / date / institution only — full TEXT is in the official PDF. Harvest Appeal No. and Request No. decisions under 29 C.F.R. Part 1614. Digest-only pages, EEO-1 data, private-sector charge closures, and Copyright Royalty Board rate-determination wraps are not this SKU. Not /flra-decisions. Not /nlrb-decisions. Not /nmb-determinations. Not /ecab-decisions. " +
      PAID_WINDOW_COPY,
    resourcePath: EEOC_APPELLATE_PATH,
  },
  "ttab-decisions": {
    description:
      "Call GET /ttab-decisions when you need official USPTO Trademark Trial and Appeal Board decision TEXT extracted from TTAB reading-room PDFs. License 17 USC 105. Does not invent decision text. Free index is parties, mark, proceeding type, outcome, precedential, and grounds (likelihood of confusion, descriptiveness, issue codes). asOf is the newest issueDateStr. Reading-room decisions only. TTABVUE docket events and Open Data Portal proceeding XML are not this SKU. Not PTAB. Party names are the public case caption only. Not /nlrb-decisions. Not /eeoc-appellate. " +
      PAID_WINDOW_COPY,
    resourcePath: TTAB_DECISIONS_PATH,
  },
  "ibla-decisions": {
    description:
      "Call GET /ibla-decisions when you need official Interior Board of Land Appeals precedential decision TEXT extracted from oha.doi.gov IBLA decision PDFs. License 17 USC 105. Does not invent decision text. Free index is citation, docket, parties, disposition, and date. asOf is the newest decision date. The PDF folder listing is HTTP 500; links come from the DOI chronological year indexes. Precedential decisions only. Dispositive orders are not this SKU. Not /bsee-reports. Not /epa-eab. Not /fmshrc-orders. " +
      PAID_WINDOW_COPY,
    resourcePath: IBLA_DECISIONS_PATH,
  },
  "ccb-determinations": {
    description:
      "Call GET /ccb-determinations when you need official Copyright Claims Board Final Determination TEXT extracted from dockets.ccb.gov/document/download PDFs indexed by the public Document search (Final Determination type:19). License 17 USC 105. Does not invent determination text. Free index is docket, parties, caption, kind, and date. asOf is the newest filed date. Harvest Final Determination PDFs only. eCCB Login HTML, FR wraps, and CourtListener are not this SKU. Not /ttab-decisions. Not /ibla-decisions. Not /oalj-decisions. Not /cbca-decisions. " +
      PAID_WINDOW_COPY,
    resourcePath: CCB_DETERMINATIONS_PATH,
  },
  "form-483": {
    description:
...
const BAZAAR_OUTPUT_EXAMPLE: Record<DoorSku, Record<string, unknown>> = {
  ...
  "mariners-d1": {
    ...
    week: "35-2026",
    asOf: "2026-09-02",
    source: "https://www.navcen.uscg.gov/local-notices-to-mariners?district=1+0&subdistrict=n",
    recordCount: 1,
    records: [
      {
        id: "35-2026:Federal Discrepancies:34850",
        date: "2026-09-02",
        firm: "Ambrose Channel",
        url: "https://www.navcen.uscg.gov/sites/default/files/pdf/lnms/lnm01352026.pdf",
        type: "mariners-d1",
      },
    ],
    notices: [
  ...
  "mariners-d5": {
    ...
    week: "35-2026",
    asOf: "2026-09-02",
    source: "https://www.navcen.uscg.gov/local-notices-to-mariners?district=5+0&subdistrict=n",
    recordCount: 1,
    records: [
      {
        id: "35-2026:Federal Discrepancies:31660",
        date: "2026-09-02",
        firm: "Albemarle Sound",
        url: "https://www.navcen.uscg.gov/sites/default/files/pdf/lnms/lnm05352026.pdf",
        type: "mariners-d5",
      },
    ],
    notices: [
      {
        week: "35-2026",
        section: "Federal Discrepancies",
        waterway: "Albemarle Sound",
        text: "Kendrick Creek Channel Entrance Light 1KC LLNR 31660",
        sourceUrl: "https://www.navcen.uscg.gov/sites/default/files/pdf/lnms/lnm05352026.pdf",
      },
    ],
  },
  "mariners-d9": {
    ...
    week: "35-2026",
    asOf: "2026-09-02",
    source: "https://www.navcen.uscg.gov/local-notices-to-mariners?district=9+0&subdistrict=n",
    recordCount: 1,
    records: [
      {
        id: "35-2026:Federal Discrepancies:11355",
        date: "2026-09-02",
        firm: "Alpena",
        url: "https://www.navcen.uscg.gov/sites/default/files/pdf/lnms/lnm09352026.pdf",
        type: "mariners-d9",
      },
    ],
    notices: [
      {
        ...
        section: "Federal Discrepancies",
        waterway: "Alpena",
        text: "Thunder Bay Traffic Lighted Bell Buoy TB LLNR 11355",
        sourceUrl: "https://www.navcen.uscg.gov/sites/default/files/pdf/lnms/lnm09352026.pdf",
      },
    ],
  },
  "mariners-d14": {
    ok: true,
    product: "uscg-d14-lnm",
    status: "ok",
    week: "35-2026",
    asOf: "2026-09-02",
    source: "https://www.navcen.uscg.gov/local-notices-to-mariners?district=14+0&subdistrict=n",
    recordCount: 1,
    records: [
      {
        id: "35-2026:Federal Discrepancies:28700",
        date: "2026-09-02",
        firm: "Kaneohe Bay",
        url: "https://www.navcen.uscg.gov/sites/default/files/pdf/lnms/lnm14352026.pdf",
        type: "mariners-d14",
      },
    ],
    notices: [
      {
        week: "35-2026",
        section: "Federal Discrepancies",
        waterway: "Kaneohe Bay",
        text: "Kaneohe Bay Channel Daybeacon 5 LLNR 28700",
        sourceUrl: "https://www.navcen.uscg.gov/sites/default/files/pdf/lnms/lnm14352026.pdf",
      },
    ],
  },
  "mariners-d17": {
    ok: true,
    product: "uscg-d17-lnm",
    status: "ok",
    week: "35-2026",
    asOf: "2026-09-02",
    source: "https://www.navcen.uscg.gov/local-notices-to-mariners?district=17+0&subdistrict=n",
    recordCount: 1,
    records: [
      {
        id: "35-2026:Federal Discrepancies:27520",
        date: "2026-09-02",
        firm: "Adak Island",
        url: "https://www.navcen.uscg.gov/sites/default/files/pdf/lnms/lnm17352026.pdf",
        type: "mariners-d17",
      },
    ],
    notices: [
      {
        week: "35-2026",
        section: "Federal Discrepancies",
        waterway: "Adak Island",
        text: "Finger Shoal Lighted Bell Buoy 3 LLNR 27520",
        sourceUrl: "https://www.navcen.uscg.gov/sites/default/files/pdf/lnms/lnm17352026.pdf",
      },
    ],
  },
  "warning-letters": {
  ...
  "fmshrc-orders": {
    ...
    asOf: "2026-07-02",
    source: "https://www.fmshrc.gov/decisions/alj",
    recordCount: 1,
    records: [
      {
        id: "CENT-2025-0091",
        date: "2026-07-02",
        firm: "Big Spring Sands LLC",
        url: "https://www.fmshrc.gov/sites/default/files/decisions/alj/Big%20Spring%20Sands%20LLC%20CENT%202025-0091-Decision%20and%20Order.pdf",
        type: "fmshrc-orders",
      },
    ],
    cards: [
  ...
  "oshrc-orders": {
    ...
    asOf: "2026-09-04",
    source: "https://www.oshrc.gov/decision-search/",
    recordCount: 1,
    records: [
      {
        id: "24-0889-eaja",
        date: "2026-09-04",
        firm: "Finley Farmers Grain & Elevator",
        url: "https://www.oshrc.gov/wp-content/uploads/ALJ-Dec-Finley-Farmers-24-0889-EAJA.pdf",
        type: "oshrc-orders",
      },
    ],
    cards: [
      {
        ...
        body: "DECISION AND ORDER. SECRETARY OF LABOR, Complainant, v. FINLEY FARMERS GRAIN & ELEVATOR. Official OSHRC ALJ findings extracted from the decision PDF.",
      },
    ],
  },
  "epa-alj": {
    ...
    asOf: "2026-09-04",
    source: "https://yosemite.epa.gov/oarm/alj/alj_web_docket.nsf/Decisions+and+Orders?OpenView",
    recordCount: 1,
    records: [
      {
        id: "FIFRA-07-2023-0135-2026-09-01",
        date: "2026-09-01",
        firm: "Timothy Wilson d/b/a Wilson's Pest Control",
        url: "https://yosemite.epa.gov/oarm/alj/alj_web_docket.nsf/Decisions%20and%20Orders/33055BE9DA2755FA85258E650060F6AC/$File/2026-09-01%20-%20wilsons%20pest%20control%20-%20initial%20decision%20and%20order%20(final)%20issued.pdf",
        type: "epa-alj",
      },
    ],
    cards: [
      {
        ...
        body: "INITIAL DECISION AND ORDER. In the Matter of Timothy Wilson, d/b/a Wilson's Pest Control. Official EPA OALJ findings extracted from the decision PDF.",
      },
    ],
  },
  "epa-eab": {
    ...
    asOf: "2026-07-15",
    source: "https://yosemite.epa.gov/oa/EAB_Web_Docket.nsf/Unpublished~Final~Orders?OpenView",
    recordCount: 1,
    records: [
      {
        id: "NPDES-26-03-2026-07-15",
        date: "2026-07-15",
        firm: "South Essex Sewerage District",
        url: "https://yosemite.epa.gov/oa/EAB_Web_Docket.nsf/4d60a7db00f72aa685258e06006de32c/045e51645556248785258e35005c159e/$FILE/South%20Essex%20Order%20Granting%20Petitioner's%20Motion%20and%20Dismissing%20Petition%20for%20Review,%20Issued%207-15-2026.pdf",
        type: "epa-eab",
      },
    ],
    cards: [
      {
        ...
        body: "ENVIRONMENTAL APPEALS BOARD. In re South Essex Sewerage District. NPDES Appeal No. 26-03. Official EPA EAB Board Order extracted from the Unpublished Final Order PDF.",
      },
    ],
  },
  "faa-civil-penalty": {
    ...
    asOf: "2026-07-02",
    source: "https://drs.faa.gov/browse/CIVIL_PENALTY_APPEALS/doctypeDetails",
    recordCount: 1,
    records: [
      {
        id: "2026-04",
        date: "2026-07-02",
        firm: "Matter of Michael Leahey",
        url: "https://drs.faa.gov/browse/CIVIL_PENALTY_APPEALS/doctypeDetails",
        type: "faa-civil-penalty",
      },
    ],
    cards: [
      {
  ...
  "stb-decisions": {
    ...
    asOf: "2026-07-21",
    source: "https://www.stb.gov/proceedings-actions/decisions/",
    recordCount: 1,
    records: [
      {
        id: "52932",
        date: "2026-07-21",
        firm: "BNSF Railway Company—Terminal Trackage Rights—Kansas City Southern Railway Company and Union Pacific Railroad Company",
        url: "https://dcms-external.s3.amazonaws.com/DCMS_External_PROD/1784658941212/52932.pdf",
        type: "stb-decisions",
      },
    ],
    cards: [
  ...
  "oalj-decisions": {
    ...
    asOf: "2026-07-30",
    source: "https://www.dol.gov/agencies/oalj/topics/information/DECISIONS",
    recordCount: 1,
    records: [
      {
        id: "2026-spa-00001",
        date: "2026-07-30",
        firm: "McLaughlin v. Troy Construction, LLC",
        url: "https://www.oalj.dol.gov/DECISIONS/ALJ/SPA/2026/McLaughlin_v_Troy_Construction_LL_2026SPA00001_(JUL_30_2026)_100012_CADEC_PD.PDF",
        type: "oalj-decisions",
      },
    ],
    cards: [
      {
        ...
        body: "U.S. Department of Labor. Office of Administrative Law Judges. OALJ No. 2026-SPA-00001. Official DOL OALJ Decision and Order extracted from the oalj.dol.gov PDF.",
      },
    ],
  },
  "fmc-orders": {
    ...
    asOf: "2026-01-06",
    source: "https://www2.fmc.gov/ReadingRoom/",
    recordCount: 1,
    records: [
      {
        id: "23-08-131865",
        date: "2026-01-06",
        firm: "Mediterranean Shipping Company, S.A.",
        url: "https://www2.fmc.gov/readingroom/docs/23-08/(32)%2023-08%20Order%20on%20Initial%20Decision%20(public).pdf/",
        type: "fmc-orders",
      },
    ],
    cards: [
      {
        ...
        sourceUrl: "https://www2.fmc.gov/readingroom/docs/23-08/(32)%2023-08%20Order%20on%20Initial%20Decision%20(public).pdf/",
        body: "FEDERAL MARITIME COMMISSION. Docket No. 23-08. Official FMC Reading Room Initial Decision / Commission Order extracted from the www2.fmc.gov PDF.",
      },
    ],
  },
  "ftc-orders": {
  ...
  "nlrb-decisions": {
    ...
    asOf: "2026-09-17",
    source: "https://www.nlrb.gov/cases-decisions/decisions/board-decisions",
    recordCount: 1,
    records: [
      {
        id: "375-nlrb-no-40",
        date: "2026-09-17",
        firm: "Nexstar Media Corporation d/b/a NewsNation",
        url: "https://apps.nlrb.gov/link/document.aspx/09031d458436f374",
        type: "nlrb-decisions",
      },
    ],
    cards: [
      {
        ...
        body: "NATIONAL LABOR RELATIONS BOARD. 375 NLRB No. 40. Official NLRB Board Decision extracted from the apps.nlrb.gov PDF.",
      },
    ],
  },
  "flra-decisions": {
    ...
    fetchedAt: "2026-09-22T00:00:00.000Z",
    asOf: "2026-09-22",
    source: "https://www.flra.gov/decisions/authority-decisions",
    recordCount: 1,
    records: [
      {
        id: "74-flra-541",
        date: "2026-09-22",
        firm: "United States Marine Corps, Twentynine Palms, California",
        url: "https://www.flra.gov/system/files/decisions/v74_80.pdf",
        type: "flra-decisions",
      },
    ],
    cards: [
      {
        ...
        body: "FEDERAL LABOR RELATIONS AUTHORITY. 74 FLRA No. 80. Official FLRA Authority Decision extracted from the flra.gov PDF.",
      },
    ],
  },
  "ecab-decisions": {
    ...
    fetchedAt: "2026-09-22T00:00:00.000Z",
    asOf: "2026-08-27",
    source: "https://www.dol.gov/agencies/ecab/decisions",
    recordCount: 1,
    records: [
      {
        id: "26-0528",
        date: "2026-08-27",
        firm: "C.P. and U.S. POSTAL SERVICE, GERMANTOWN POST OFFICE, Philadelphia, PA",
        url: "https://www.dol.gov/sites/dolgov/files/ecab/decisions/2026/AUG/26-0528.pdf",
        type: "ecab-decisions",
      },
    ],
    cards: [
      {
        ...
        body: "Employees' Compensation Appeals Board. Docket No. 26-0528. Official ECAB Decision and Order extracted from the dol.gov PDF.",
      },
    ],
  },
  "fcc-eb-orders": {
  ...
  "nmb-determinations": {
    ...
    fetchedAt: "2026-09-22T00:00:00.000Z",
    asOf: "2026-09-17",
    source: "https://nmb.gov/NMB_Application/index.php/agency-determinations/",
    recordCount: 1,
    records: [
      {
        id: "53-nmb-no-34",
        date: "2026-09-17",
        firm: "California Northern Railroad Company; BRS; Signal Department Employees",
        url: "https://nmb.gov/NMB_Application/wp-content/uploads/2026/09/Certification-R-7687.pdf",
        type: "nmb-determinations",
      },
    ],
    cards: [
      {
        ...
        body: "National Mediation Board. 53 NMB No. 34. CASE NO. R-7687. CERTIFICATION. Railway Labor Act Section 2, Ninth. California Northern Railroad Company.",
      },
    ],
  },
  "eeoc-appellate": {
    ...
    asOf: "2026-08-24",
    source: "https://www.eeoc.gov/federal-sector/appellate-decisions",
    recordCount: 1,
    records: [
      {
        id: "request-2026002858",
        date: "2026-08-24",
        firm: "Lenard T; Department of Transportation; Federal Aviation Administration",
        url: "https://www.eeoc.gov/sites/default/files/decisions/2026_09_08/2026002858.%20DEC.pdf",
        type: "eeoc-appellate",
      },
    ],
    cards: [
      {
        ...
        body: "U.S. Equal Employment Opportunity Commission. Office of Federal Sector. Request No. 2026002858. Decision on request for reconsideration under 29 C.F.R. Part 1614.",
      },
    ],
  },
  "ttab-decisions": {
    ...
    asOf: "2026-09-11",
    source: "https://ttab-reading-room.uspto.gov/efoia/efoia-ui/#/search/decisions",
    recordCount: 1,
    records: [
      {
        id: "92081421-can-45",
        date: "2026-09-11",
        firm: "D-Lize srl v. Genex Enterprises LLC and Saniderm Medical, LLC",
        url: "https://ttab-reading-room.uspto.gov/cms/rest/legal-proceeding/92081421/decision/CAN_45.pdf",
        type: "ttab-decisions",
      },
    ],
    cards: [
      {
        ...
        body: "UNITED STATES PATENT AND TRADEMARK OFFICE. Trademark Trial and Appeal Board. Cancellation No. 92081421. Decision.",
      },
    ],
  },
  "ibla-decisions": {
    ...
    fetchedAt: "2026-09-24T00:00:00.000Z",
    asOf: "2026-06-16",
    source: "https://www.oha.doi.gov/IBLA/Ibladecisions/",
    recordCount: 1,
    records: [
      {
        id: "201-ibla-53",
        date: "2026-06-16",
        firm: "Maverick Solar, LLC et al.",
        url: "https://www.oha.doi.gov/IBLA/Ibladecisions/201IBLA/201ibla53.pdf",
        type: "ibla-decisions",
      },
    ],
    cards: [
      {
        ...
        body: "MAVERICK SOLAR, LLC ET AL. IBLA 2021-0216 et al. Decided June 16, 2026. Set aside and remanded. OPINION BY ADMINISTRATIVE JUDGE BALLENGER. 201 IBLA 53.",
      },
    ],
  },
  "ccb-determinations": {
    ...
    asOf: "2026-09-22",
    source: "https://dockets.ccb.gov/search/documents",
    recordCount: 1,
    records: [
      {
        id: "24-ccb-0180",
        date: "2026-09-22",
        firm: "Ping Lau; Roselinde Skievaski",
        url: "https://dockets.ccb.gov/document/download/21779",
        type: "ccb-determinations",
      },
    ],
    cards: [
      {
        ...
        body: "Docket number: 24-CCB-0180. September 22, 2026. Ping Lau v. Roselinde Skievaski. FINAL DETERMINATION. Copyright Claims Board.",
      },
    ],
  },
  "form-483": {
...
export function llmsTxt(): string {
  ...
  const paid = [
    ...
    "- GET /mariners-d1 — $0.05 — USCG D1 / Northeast Local Notice to Mariners",
    "- GET /mariners-d5 — $0.05 — USCG D5 / Mid-Atlantic Local Notice to Mariners",
    "- GET /mariners-d9 — $0.05 — USCG D9 / Great Lakes Local Notice to Mariners",
    "- GET /mariners-d14 — $0.05 — USCG D14 / Pacific Local Notice to Mariners",
    "- GET /mariners-d17 — $0.05 — USCG D17 / Alaska Local Notice to Mariners",
    ...
    `- GET /epa-eab — $0.05 — EPA EAB Unpublished Final Order / Board Order text (official yosemite.epa.gov/oa/EAB_Web_Docket.nsf PDFs). Newest ${PAID_BODY_N} official texts. Same URL ?before=<id or date> is the next older ${PAID_BODY_N} for another $0.05.`,
    `- GET /faa-civil-penalty — $0.05 — FAA Civil Penalty Appeals Administrator Order text (official drs.faa.gov CIVIL_PENALTY_APPEALS PDFs). Newest ${PAID_BODY_N} official texts. Same URL ?before=<id or date> is the next older ${PAID_BODY_N} for another $0.05.`,
    ...
    `- GET /nmb-determinations — $0.05 — NMB Railway Labor Act §2 Ninth representation determination text (official nmb.gov FY Determinations PDFs). Newest ${PAID_BODY_N} official texts. Same URL ?before=<id or date> is the next older ${PAID_BODY_N} for another $0.05.`,
    `- GET /eeoc-appellate — $0.05 — EEOC Office of Federal Sector appellate and reconsideration text (official eeoc.gov Appeal No. / Request No. PDFs). Newest ${PAID_BODY_N} official texts. Same URL ?before=<id or date> is the next older ${PAID_BODY_N} for another $0.05.`,
    `- GET /ttab-decisions — $0.05 — USPTO TTAB reading-room decision text (official ttab-reading-room.uspto.gov decision PDFs). Newest ${PAID_BODY_N} official texts. Same URL ?before=<id or date> is the next older ${PAID_BODY_N} for another $0.05.`,
    `- GET /ibla-decisions — $0.05 — DOI IBLA precedential decision text (official oha.doi.gov IBLA decision PDFs). Newest ${PAID_BODY_N} official texts. Same URL ?before=<id or date> is the next older ${PAID_BODY_N} for another $0.05.`,
    `- GET /ccb-determinations — $0.05 — Copyright Claims Board Final Determination text (official dockets.ccb.gov document download PDFs). Newest ${PAID_BODY_N} official texts. Same URL ?before=<id or date> is the next older ${PAID_BODY_N} for another $0.05.`,
  ];
  if (listed483) {
  ...
  const free = [
    ...
    "- GET /mariners-d1/manifest.json — D1 LNM count + official PDF (not the notice body)",
    "- GET /mariners-d5/manifest.json — D5 LNM count + official PDF (not the notice body)",
    "- GET /mariners-d9/manifest.json — D9 LNM count + official PDF (not the notice body)",
    "- GET /mariners-d14/manifest.json — D14 LNM count + official PDF (not the notice body)",
    ...
    "- GET /epa-eab/manifest.json — EPA EAB Board Order count + case/appeal/date/Type/Result (full catalog + page cursor; ?q= is free search; not the Board narrative)",
    "- GET /faa-civil-penalty/manifest.json — FAA Civil Penalty Appeals Order count + order no/case/docket/dates/subjects (full catalog + page cursor; ?q= is free search; not the Order narrative)",
    ...
    "- GET /nmb-determinations/manifest.json — NMB representation determination count + citation/case/date/institution (full catalog + page cursor; ?q= is free search; not the determination narrative)",
    "- GET /eeoc-appellate/manifest.json — EEOC OFS appellate decision count + citation/case/date/institution (full catalog + page cursor; ?q= is free search; not the decision narrative)",
    "- GET /ttab-decisions/manifest.json — TTAB reading-room decision count + parties/mark/type/outcome/precedential/grounds (full catalog + page cursor; ?q= is free search; not the decision narrative)",
    "- GET /ibla-decisions/manifest.json — DOI IBLA precedential decision count + citation/docket/parties/disposition/date (full catalog + page cursor; ?q= is free search; not the decision narrative)",
    "- GET /ccb-determinations/manifest.json — Copyright Claims Board Final Determination count + docket/parties/caption/kind/date (full catalog + page cursor; ?q= is free search; not the determination narrative)",
  ];
  if (listed483) {
...
export function buildOpenApi(req: IncomingMessage, port: number): Record<string, unknown> {
  ...
  const lnmD1Atomic = amountAtomicFor("mariners-d1");
  const lnmD5Atomic = amountAtomicFor("mariners-d5");
  const lnmD9Atomic = amountAtomicFor("mariners-d9");
  const lnmD14Atomic = amountAtomicFor("mariners-d14");
  const lnmD17Atomic = amountAtomicFor("mariners-d17");
  ...
  const epaEabAtomic = amountAtomicFor("epa-eab");
  const faaCivilPenaltyAtomic = amountAtomicFor("faa-civil-penalty");
  ...
  const nmbDeterminationsAtomic = amountAtomicFor("nmb-determinations");
  const eeocAppellateAtomic = amountAtomicFor("eeoc-appellate");
  const ttabDecisionsAtomic = amountAtomicFor("ttab-decisions");
  const iblaDecisionsAtomic = amountAtomicFor("ibla-decisions");
  const ccbDeterminationsAtomic = amountAtomicFor("ccb-determinations");
  ...
  const lnmD1Price = (Number(lnmD1Atomic) / 1e6).toFixed(2);
  const lnmD5Price = (Number(lnmD5Atomic) / 1e6).toFixed(2);
  const lnmD9Price = (Number(lnmD9Atomic) / 1e6).toFixed(2);
  const lnmD14Price = (Number(lnmD14Atomic) / 1e6).toFixed(2);
  const lnmD17Price = (Number(lnmD17Atomic) / 1e6).toFixed(2);
  ...
  const epaEabPrice = (Number(epaEabAtomic) / 1e6).toFixed(2);
  const faaCivilPenaltyPrice = (Number(faaCivilPenaltyAtomic) / 1e6).toFixed(2);
  ...
  const nmbDeterminationsPrice = (Number(nmbDeterminationsAtomic) / 1e6).toFixed(2);
  const eeocAppellatePrice = (Number(eeocAppellateAtomic) / 1e6).toFixed(2);
  const ttabDecisionsPrice = (Number(ttabDecisionsAtomic) / 1e6).toFixed(2);
  const iblaDecisionsPrice = (Number(iblaDecisionsAtomic) / 1e6).toFixed(2);
  const ccbDeterminationsPrice = (Number(ccbDeterminationsAtomic) / 1e6).toFixed(2);
  ...
  const paidBits = [
    ...
    "/ecab-decisions ($0.05)",
    "/fcc-eb-orders ($0.05)",
    "/nmb-determinations ($0.05)",
    "/eeoc-appellate ($0.05)",
    "/ttab-decisions ($0.05)",
    "/ibla-decisions ($0.05)",
    "/ccb-determinations ($0.05)",
  ];
  ...
  const doc: Record<string, unknown> = {
    ...
    paths: {
      ...
      [MARINERS_D1_PATH]: {
        get: paidOpenApiOp({
          operationId: "getMarinersD1",
          summary: "USCG D1 / Northeast LNM",
          description: SKU_COPY["mariners-d1"].description,
          priceUsdc: lnmD1Price,
          amountAtomic: lnmD1Atomic,
          example: BAZAAR_OUTPUT_EXAMPLE["mariners-d1"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [MARINERS_D5_PATH]: {
        get: paidOpenApiOp({
          operationId: "getMarinersD5",
          summary: "USCG D5 / Mid-Atlantic LNM",
          description: SKU_COPY["mariners-d5"].description,
          priceUsdc: lnmD5Price,
          amountAtomic: lnmD5Atomic,
          example: BAZAAR_OUTPUT_EXAMPLE["mariners-d5"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [MARINERS_D9_PATH]: {
        get: paidOpenApiOp({
          operationId: "getMarinersD9",
          summary: "USCG D9 / Great Lakes LNM",
          description: SKU_COPY["mariners-d9"].description,
          priceUsdc: lnmD9Price,
          amountAtomic: lnmD9Atomic,
          example: BAZAAR_OUTPUT_EXAMPLE["mariners-d9"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [MARINERS_D14_PATH]: {
        get: paidOpenApiOp({
          operationId: "getMarinersD14",
          summary: "USCG D14 / Pacific LNM",
          description: SKU_COPY["mariners-d14"].description,
          priceUsdc: lnmD14Price,
          amountAtomic: lnmD14Atomic,
          example: BAZAAR_OUTPUT_EXAMPLE["mariners-d14"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [MARINERS_D17_PATH]: {
        get: paidOpenApiOp({
          operationId: "getMarinersD17",
          summary: "USCG D17 / Alaska LNM",
          description: SKU_COPY["mariners-d17"].description,
          priceUsdc: lnmD17Price,
          amountAtomic: lnmD17Atomic,
          example: BAZAAR_OUTPUT_EXAMPLE["mariners-d17"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [FMSHRC_ORDERS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getFmshrcOrders",
          summary: "FMSHRC ALJ + Commission Decision/Order text",
          description: SKU_COPY["fmshrc-orders"].description,
          priceUsdc: fmshrcOrdersPrice,
          amountAtomic: fmshrcOrdersAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["fmshrc-orders"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [OSHRC_ORDERS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getOshrcOrders",
          summary: "OSHRC ALJ Decision/Order + Commission Final Order text",
          description: SKU_COPY["oshrc-orders"].description,
          priceUsdc: oshrcOrdersPrice,
          amountAtomic: oshrcOrdersAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["oshrc-orders"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [EPA_ALJ_PATH]: {
        get: paidOpenApiOp({
          operationId: "getEpaAlj",
          summary: "EPA OALJ Initial Decision and Order + ALJ Order text",
          description: SKU_COPY["epa-alj"].description,
          priceUsdc: epaAljPrice,
          amountAtomic: epaAljAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["epa-alj"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [EPA_EAB_PATH]: {
        get: paidOpenApiOp({
          operationId: "getEpaEab",
          summary: "EPA EAB Unpublished Final Order / Board Order text",
          description: SKU_COPY["epa-eab"].description,
          priceUsdc: epaEabPrice,
          amountAtomic: epaEabAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["epa-eab"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [FAA_CIVIL_PENALTY_PATH]: {
        get: paidOpenApiOp({
          operationId: "getFaaCivilPenalty",
          summary: "FAA Civil Penalty Appeals Administrator Order text",
          description: SKU_COPY["faa-civil-penalty"].description,
          priceUsdc: faaCivilPenaltyPrice,
          amountAtomic: faaCivilPenaltyAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["faa-civil-penalty"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [STB_DECISIONS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getStbDecisions",
          summary: "STB Board Decision / Order text",
          description: SKU_COPY["stb-decisions"].description,
          priceUsdc: stbDecisionsPrice,
          amountAtomic: stbDecisionsAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["stb-decisions"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [OALJ_DECISIONS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getOaljDecisions",
          summary: "DOL OALJ / BALCA / ARB Decision and Order text",
          description: SKU_COPY["oalj-decisions"].description,
          priceUsdc: oaljDecisionsPrice,
          amountAtomic: oaljDecisionsAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["oalj-decisions"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [FMC_ORDERS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getFmcOrders",
          summary: "FMC Initial Decision / Commission Order text",
          description: SKU_COPY["fmc-orders"].description,
          priceUsdc: fmcOrdersPrice,
          amountAtomic: fmcOrdersAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["fmc-orders"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [NLRB_DECISIONS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getNlrbDecisions",
          summary: "NLRB published Board Decision text",
          description: SKU_COPY["nlrb-decisions"].description,
          priceUsdc: nlrbDecisionsPrice,
          amountAtomic: nlrbDecisionsAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["nlrb-decisions"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [FLRA_DECISIONS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getFlraDecisions",
          summary: "FLRA Authority Decision text",
          description: SKU_COPY["flra-decisions"].description,
          priceUsdc: flraDecisionsPrice,
          amountAtomic: flraDecisionsAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["flra-decisions"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [ECAB_DECISIONS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getEcabDecisions",
          summary: "ECAB FECA Decision and Order text",
          description: SKU_COPY["ecab-decisions"].description,
          priceUsdc: ecabDecisionsPrice,
          amountAtomic: ecabDecisionsAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["ecab-decisions"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [NMB_DETERMINATIONS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getNmbDeterminations",
          summary: "NMB representation determination text",
          description: SKU_COPY["nmb-determinations"].description,
          priceUsdc: nmbDeterminationsPrice,
          amountAtomic: nmbDeterminationsAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["nmb-determinations"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [EEOC_APPELLATE_PATH]: {
        get: paidOpenApiOp({
          operationId: "getEeocAppellate",
          summary: "EEOC OFS appellate decision text",
          description: SKU_COPY["eeoc-appellate"].description,
          priceUsdc: eeocAppellatePrice,
          amountAtomic: eeocAppellateAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["eeoc-appellate"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [TTAB_DECISIONS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getTtabDecisions",
          summary: "USPTO TTAB reading-room decision text",
          description: SKU_COPY["ttab-decisions"].description,
          priceUsdc: ttabDecisionsPrice,
          amountAtomic: ttabDecisionsAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["ttab-decisions"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [IBLA_DECISIONS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getIblaDecisions",
          summary: "DOI IBLA precedential decision text",
          description: SKU_COPY["ibla-decisions"].description,
          priceUsdc: iblaDecisionsPrice,
          amountAtomic: iblaDecisionsAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["ibla-decisions"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [CCB_DETERMINATIONS_PATH]: {
        get: paidOpenApiOp({
          operationId: "getCcbDeterminations",
          summary: "Copyright Claims Board Final Determination text",
          description: SKU_COPY["ccb-determinations"].description,
          priceUsdc: ccbDeterminationsPrice,
          amountAtomic: ccbDeterminationsAtomic,
          example: BAZAAR_OUTPUT_EXAMPLE["ccb-determinations"],
          outputSchema: {
            type: "object",
            properties: {
              ok: { type: "boolean" },
      ...
      [MARINERS_D1_MANIFEST_PATH]: {
        get: freeOpenApiOp("USCG D1 LNM free manifest", "Count, week, and official PDF URL. Not the notice body."),
      },
      [MARINERS_D5_MANIFEST_PATH]: {
        get: freeOpenApiOp("USCG D5 LNM free manifest", "Count, week, and official PDF URL. Not the notice body."),
      },
      [MARINERS_D9_MANIFEST_PATH]: {
        get: freeOpenApiOp("USCG D9 LNM free manifest", "Count, week, and official PDF URL. Not the notice body."),
      },
      [MARINERS_D14_MANIFEST_PATH]: {
        get: freeOpenApiOp("USCG D14 LNM free manifest", "Count, week, and official PDF URL. Not the notice body."),
      },
      [MARINERS_D17_MANIFEST_PATH]: {
        get: freeOpenApiOp("USCG D17 LNM free manifest", "Count, week, and official PDF URL. Not the notice body."),
      },
      [WARNING_LETTERS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
      ...
      [OSHRC_ORDERS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "OSHRC Decision/Order free manifest",
          "Count, operator, docket, date, and official PDF/HTML URL. Not the decision body.",
        ),
      },
      [EPA_ALJ_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "EPA OALJ Decision/Order free manifest",
          "Count, case, docket, date, label, and paidUrl. Not the decision body.",
        ),
      },
      [EPA_EAB_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "EPA EAB Board Order free manifest",
          "Count, case, appeal, date, Type, Result, and paidUrl. Not the Board narrative.",
        ),
      },
      [FAA_CIVIL_PENALTY_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "FAA Civil Penalty Appeals Order free manifest",
          "Count, order no, case, docket, dates, subjects, and paidUrl. Not the Order narrative.",
        ),
      },
      [STB_DECISIONS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "STB Board Decision / Order free manifest",
          "Count, docket, Decision ID, date, deciding body, and paidUrl. Not the Board narrative.",
        ),
      },
      [OALJ_DECISIONS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "DOL OALJ / BALCA / ARB Decision and Order free manifest",
          "Count, case number, board, date, institution, and paidUrl. Not the Decision/Order narrative.",
        ),
      },
      [FMC_ORDERS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "FMC Initial Decision / Commission Order free manifest",
      ...
      [NLRB_DECISIONS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "NLRB Board Decision free manifest",
          "Count, citation, case number, date, institution, and paidUrl. Not the Board Decision narrative.",
        ),
      },
      [FLRA_DECISIONS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "FLRA Authority Decision free manifest",
          "Count, citation, case number, date, institution, and paidUrl. Not the Authority Decision narrative.",
        ),
      },
      [ECAB_DECISIONS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
      ...
      [NMB_DETERMINATIONS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "NMB representation determination free manifest",
          "Count, citation, case, date, institution, and paidUrl. Not the determination narrative.",
        ),
      },
      [EEOC_APPELLATE_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "EEOC OFS appellate decision free manifest",
          "Count, citation, case, date, institution, and paidUrl. Not the decision narrative.",
        ),
      },
      [TTAB_DECISIONS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "USPTO TTAB reading-room decision free manifest",
          "Count, parties, mark, proceeding type, outcome, precedential, grounds, and paidUrl. Not the decision narrative.",
        ),
      },
      [IBLA_DECISIONS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "DOI IBLA precedential decision free manifest",
          "Count, citation, docket, parties, disposition, date, and paidUrl. Not the decision narrative.",
        ),
      },
      [CCB_DETERMINATIONS_MANIFEST_PATH]: {
        get: freeOpenApiOp(
          "Copyright Claims Board Final Determination free manifest",
          "Count, docket, parties, caption, kind, date, and paidUrl. Not the determination narrative.",
        ),
      },
      ...(listed483
        ? {
...
export async function handleRequest(req: IncomingMessage, res: ServerResponse, port: number): Promise<void> {
    ...
    sendJson(res, 200, {
      ...
      products: [
        ...
        },
        {
          ...
          product: D1_SPEC.productId,
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("mariners-d1"),
          manifest: MARINERS_D1_MANIFEST_PATH,
        },
        {
          path: MARINERS_D5_PATH,
          product: D5_SPEC.productId,
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("mariners-d5"),
          manifest: MARINERS_D5_MANIFEST_PATH,
        },
        {
          path: MARINERS_D9_PATH,
          product: D9_SPEC.productId,
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("mariners-d9"),
          manifest: MARINERS_D9_MANIFEST_PATH,
        },
        {
          path: MARINERS_D14_PATH,
          product: D14_SPEC.productId,
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("mariners-d14"),
          manifest: MARINERS_D14_MANIFEST_PATH,
        },
        {
          path: MARINERS_D17_PATH,
          product: D17_SPEC.productId,
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("mariners-d17"),
          manifest: MARINERS_D17_MANIFEST_PATH,
        },
        {
        ...
        {
          path: OSHRC_ORDERS_PATH,
          product: "oshrc-order-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("oshrc-orders"),
          manifest: OSHRC_ORDERS_MANIFEST_PATH,
        },
        {
          path: EPA_ALJ_PATH,
          product: "epa-alj-decision-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("epa-alj"),
          manifest: EPA_ALJ_MANIFEST_PATH,
        },
        {
          path: EPA_EAB_PATH,
          product: "epa-eab-board-order-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("epa-eab"),
          manifest: EPA_EAB_MANIFEST_PATH,
        },
        {
          path: FAA_CIVIL_PENALTY_PATH,
          product: "faa-civil-penalty-order-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("faa-civil-penalty"),
          manifest: FAA_CIVIL_PENALTY_MANIFEST_PATH,
        },
        {
          path: STB_DECISIONS_PATH,
          product: "stb-board-decision-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("stb-decisions"),
          manifest: STB_DECISIONS_MANIFEST_PATH,
        },
        {
          path: OALJ_DECISIONS_PATH,
          product: "oalj-decision-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("oalj-decisions"),
          manifest: OALJ_DECISIONS_MANIFEST_PATH,
        },
        {
          path: FMC_ORDERS_PATH,
          product: "fmc-order-bodies",
          priceUsdc: "0.05",
        ...
        {
          path: NLRB_DECISIONS_PATH,
          product: "nlrb-board-decision-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("nlrb-decisions"),
          manifest: NLRB_DECISIONS_MANIFEST_PATH,
        },
        {
          path: FLRA_DECISIONS_PATH,
          product: "flra-authority-decision-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("flra-decisions"),
          manifest: FLRA_DECISIONS_MANIFEST_PATH,
        },
        {
          path: ECAB_DECISIONS_PATH,
          product: "ecab-decision-bodies",
          priceUsdc: "0.05",
        ...
        {
          path: NMB_DETERMINATIONS_PATH,
          product: "nmb-determination-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("nmb-determinations"),
          manifest: NMB_DETERMINATIONS_MANIFEST_PATH,
        },
        {
          path: EEOC_APPELLATE_PATH,
          product: "eeoc-appellate-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("eeoc-appellate"),
          manifest: EEOC_APPELLATE_MANIFEST_PATH,
        },
        {
          path: TTAB_DECISIONS_PATH,
          product: "ttab-decision-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("ttab-decisions"),
          manifest: TTAB_DECISIONS_MANIFEST_PATH,
        },
        {
          path: IBLA_DECISIONS_PATH,
          product: "ibla-decision-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("ibla-decisions"),
          manifest: IBLA_DECISIONS_MANIFEST_PATH,
        },
        {
          path: CCB_DETERMINATIONS_PATH,
          product: "ccb-determination-bodies",
          priceUsdc: "0.05",
          amountAtomic: amountAtomicFor("ccb-determinations"),
          manifest: CCB_DETERMINATIONS_MANIFEST_PATH,
        },
        ...(form483IsPublic()
          ? [
              {
  ...
  if (path === MARINERS_D5_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadMarinersD5Manifest(), req, port));
    return;
  }

  if (path === MARINERS_D9_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadMarinersD9Manifest(), req, port));
    return;
  }

  if (path === MARINERS_D14_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadMarinersD14Manifest(), req, port));
    return;
  }

  if (path === MARINERS_D17_MANIFEST_PATH) {
    sendJson(res, 200, withShopDiscovery(await loadMarinersD17Manifest(), req, port));
    return;
  }
  ...
  if (path === MARINERS_D5_PATH) {
    await servePaid(req, res, port, "mariners-d5", async () => paidMarinersD5Body(await loadMarinersD5()));
    return;
  }

  if (path === MARINERS_D9_PATH) {
    await servePaid(req, res, port, "mariners-d9", async () => paidMarinersD9Body(await loadMarinersD9()));
    return;
  }

  if (path === MARINERS_D14_PATH) {
    await servePaid(req, res, port, "mariners-d14", async () => paidMarinersD14Body(await loadMarinersD14()));
    return;
  }

  if (path === MARINERS_D17_PATH) {
    await servePaid(req, res, port, "mariners-d17", async () => paidMarinersD17Body(await loadMarinersD17()));
    return;
  }
  ...
  if (path === FMSHRC_ORDERS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadFmshrcManifest());
    return;
  }

  if (path === FMSHRC_ORDERS_PATH) {
    await servePaid(req, res, port, "fmshrc-orders", async (opts) => paidFmshrcOrdersBody(await loadFmshrcOrders(), opts));
    return;
  }
  ...
  if (path === OSHRC_ORDERS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadOshrcManifest());
    return;
  }

  if (path === OSHRC_ORDERS_PATH) {
    await servePaid(req, res, port, "oshrc-orders", async (opts) => paidOshrcOrdersBody(await loadOshrcOrders(), opts));
    return;
  }

  if (path === EPA_ALJ_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadEpaAljManifest());
    return;
  }

  if (path === EPA_ALJ_PATH) {
    await servePaid(req, res, port, "epa-alj", async (opts) => paidEpaAljBody(await loadEpaAlj(), opts));
    return;
  }

  if (path === EPA_EAB_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadEpaEabManifest());
    return;
  }

  if (path === EPA_EAB_PATH) {
    await servePaid(req, res, port, "epa-eab", async (opts) => paidEpaEabBody(await loadEpaEab(), opts));
    return;
  }

  if (path === FAA_CIVIL_PENALTY_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadFaaCivilPenaltyManifest());
    return;
  }

  if (path === FAA_CIVIL_PENALTY_PATH) {
    await servePaid(req, res, port, "faa-civil-penalty", async (opts) =>
      paidFaaCivilPenaltyBody(await loadFaaCivilPenalty(), opts),
    );
    return;
  }

  if (path === STB_DECISIONS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadStbDecisionsManifest());
    return;
  }

  if (path === STB_DECISIONS_PATH) {
    await servePaid(req, res, port, "stb-decisions", async (opts) =>
      paidStbDecisionsBody(await loadStbDecisions(), opts),
    );
    return;
  }

  if (path === OALJ_DECISIONS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadOaljDecisionsManifest());
    return;
  }

  if (path === OALJ_DECISIONS_PATH) {
    await servePaid(req, res, port, "oalj-decisions", async (opts) =>
      paidOaljDecisionsBody(await loadOaljDecisions(), opts),
    );
    return;
  }

  if (path === FMC_ORDERS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadFmcOrdersManifest());
    return;
  }

  if (path === FMC_ORDERS_PATH) {
    await servePaid(req, res, port, "fmc-orders", async (opts) =>
      paidFmcOrdersBody(await loadFmcOrders(), opts),
    );
    return;
  }
  ...
  if (path === NLRB_DECISIONS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadNlrbDecisionsManifest());
    return;
  }

  if (path === NLRB_DECISIONS_PATH) {
    await servePaid(req, res, port, "nlrb-decisions", async (opts) =>
      paidNlrbDecisionsBody(await loadNlrbDecisions(), opts),
    );
    return;
  }

  if (path === FLRA_DECISIONS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadFlraDecisionsManifest());
    return;
  }

  if (path === FLRA_DECISIONS_PATH) {
    await servePaid(req, res, port, "flra-decisions", async (opts) =>
      paidFlraDecisionsBody(await loadFlraDecisions(), opts),
    );
    return;
  }

  if (path === ECAB_DECISIONS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadEcabDecisionsManifest());
    return;
  }

  if (path === ECAB_DECISIONS_PATH) {
    await servePaid(req, res, port, "ecab-decisions", async (opts) =>
      paidEcabDecisionsBody(await loadEcabDecisions(), opts),
    );
    return;
  }
  ...
  if (path === NMB_DETERMINATIONS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadNmbDeterminationsManifest());
    return;
  }

  if (path === NMB_DETERMINATIONS_PATH) {
    await servePaid(req, res, port, "nmb-determinations", async (opts) =>
      paidNmbDeterminationsBody(await loadNmbDeterminations(), opts),
    );
    return;
  }

  if (path === EEOC_APPELLATE_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadEeocAppellateManifest());
    return;
  }

  if (path === EEOC_APPELLATE_PATH) {
    await servePaid(req, res, port, "eeoc-appellate", async (opts) =>
      paidEeocAppellateBody(await loadEeocAppellate(), opts),
    );
    return;
  }

  if (path === TTAB_DECISIONS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadTtabManifest());
    return;
  }

  if (path === TTAB_DECISIONS_PATH) {
    await servePaid(req, res, port, "ttab-decisions", async (opts) =>
      paidTtabDecisionsBody(await loadTtabDecisions(), opts),
    );
    return;
  }

  if (path === IBLA_DECISIONS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadIblaManifest());
    return;
  }

  if (path === IBLA_DECISIONS_PATH) {
    await servePaid(req, res, port, "ibla-decisions", async (opts) =>
      paidIblaDecisionsBody(await loadIblaDecisions(), opts),
    );
    return;
  }

  if (path === CCB_DETERMINATIONS_MANIFEST_PATH) {
    sendExtractedManifest(req, res, port, url, await loadCcbManifest());
    return;
  }

  if (path === CCB_DETERMINATIONS_PATH) {
    await servePaid(req, res, port, "ccb-determinations", async (opts) =>
      paidCcbDeterminationsBody(await loadCcbDeterminations(), opts),
    );
    return;
  }
...
if (isMain()) {
  ...
  server.listen(port, host, () => {
    ...
    console.error(`${MARINERS_D1_PATH} $${Number(amountAtomicFor("mariners-d1")) / 1e6} USDC`);
    console.error(`${MARINERS_D5_PATH} $${Number(amountAtomicFor("mariners-d5")) / 1e6} USDC`);
    console.error(`${MARINERS_D9_PATH} $${Number(amountAtomicFor("mariners-d9")) / 1e6} USDC`);
    console.error(`${MARINERS_D14_PATH} $${Number(amountAtomicFor("mariners-d14")) / 1e6} USDC`);
    console.error(`${MARINERS_D17_PATH} $${Number(amountAtomicFor("mariners-d17")) / 1e6} USDC`);
    ...
    console.error(`${EPA_EAB_PATH} $${Number(amountAtomicFor("epa-eab")) / 1e6} USDC`);
    console.error(`${FAA_CIVIL_PENALTY_PATH} $${Number(amountAtomicFor("faa-civil-penalty")) / 1e6} USDC`);
    ...
    console.error(`${NMB_DETERMINATIONS_PATH} $${Number(amountAtomicFor("nmb-determinations")) / 1e6} USDC`);
    console.error(`${EEOC_APPELLATE_PATH} $${Number(amountAtomicFor("eeoc-appellate")) / 1e6} USDC`);
    console.error(`${TTAB_DECISIONS_PATH} $${Number(amountAtomicFor("ttab-decisions")) / 1e6} USDC`);
    console.error(`${IBLA_DECISIONS_PATH} $${Number(amountAtomicFor("ibla-decisions")) / 1e6} USDC`);
    console.error(`${CCB_DETERMINATIONS_PATH} $${Number(amountAtomicFor("ccb-determinations")) / 1e6} USDC`);
