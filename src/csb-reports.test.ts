...
import {
  ...
  PRODUCT_ID,
  SEED_LISTINGS,
  assembleCsbReportsSnapshot,
  buildCsbReportsManifest,
  collectCsbReports,
  filterCsbReportsManifest,
  isKeptCsbFinalPdfName,
  isOfficialCsbFinalPdf,
  isPdfBytes,
  isSkippedCsbPdfName,
  officialCsbFinalPdfUrl,
  officialCsbPageUrl,
  parseCompletedIndex,
  parseInvestigationPage,
  parseListingRows,
  selectCsbReportCard,
} from "./csb-reports.js";
...
async function main(): Promise<void> {
  ...
  assert.equal(
    fromIndex.some((r) => /current-investigations|teaser/i.test(r.pageUrl)),
    false,
    "skip current-investigation teasers",
  );
  ...
  assert.equal(
    officialCsbPageUrl("https://www.csb.gov/investigations/completed-investigations/bio-lab-inc-conyers-fire-and-chemical-release-/"),
    BIO_PAGE,
    "completed-investigations slug normalizes to the official page",
  );
  ...
  assert.equal(
    officialCsbFinalPdfUrl("https://www.csb.gov/assets/1/20/bio-lab_status_change_summary.pdf"),
    null,
    "Status Change Summary is not the SKU spine",
  );
  ...
  assert.equal(
    officialCsbFinalPdfUrl("https://www.saferproducts.gov/RestWebServices/Recall?format=json"),
    null,
    "CPSC SaferProducts.gov stays killed",
  );
  ...
  const givaudan = parseInvestigationPage(
    readFx("givaudan-sense-colour-explosion.html"),
  ...
  if (prevDir === undefined) delete process.env.CSB_REPORTS_DIR;
  else process.env.CSB_REPORTS_DIR = prevDir;
  ...
  const bioCard = (manifest.cards as { id?: string; body?: string; title?: string }[]).find((c) => c.id === BIO_ID);
  ...
  assert.equal((filtered.cards as { id: string }[])[0]?.id, "givaudan-sense-colour-explosion");
