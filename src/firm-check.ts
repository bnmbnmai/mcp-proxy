...
function bodyMatch(door: string, row: Record<string, unknown>): FirmCheckMatch {
  const id = rowId(row);
  return {
    door,
    id,
...
function tableMatch(row: Record<string, unknown>): FirmCheckMatch {
  return {
    door: "import-alerts",
    id: rowId(row),
    title: rowTitle(row),
...
function searchImportAlertIndex(manifest: Record<string, unknown>, needle: string): FirmCheckMatch[] {
  ...
  return [...sampleHits, ...catalogHits].map((row) => tableMatch(row));
}
