...
async function main(): Promise<void> {
  ...
  const seed = JSON.parse(readFx("seed-snapshot.json")) as {
    product: string;
    status: string;
    cards: { id: string; sourceUrl: string; body: string; permit: string }[];
  };
  assert.equal(seed.product, "epa-npdes-individual-permit-bodies");
  assert.equal(seed.status, "ok");
  assert.ok(seed.cards.length >= 14, `bundled seed is real official permits, got ${seed.cards.length}`);
  assert.ok(seed.cards.every((c) => isRealNpdesPermitBody(c.body)), "seed bodies are official individual NPDES text");
  assert.ok(seed.cards.every((c) => officialIndividualPermitUrl(c.sourceUrl)));
  assert.ok(seed.cards.some((c) => c.id === "ma0003531" && c.body.includes("AUTHORIZATION TO DISCHARGE")));
  assert.ok(!seed.cards.some((c) => /echo\.epa\.gov|semspub\.epa\.gov|waterboards/i.test(c.sourceUrl)));
