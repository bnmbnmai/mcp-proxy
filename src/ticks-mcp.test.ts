...
async function main(): Promise<void> {
  ...
  await withServer(
    ...
    async (base) => {
      ...
      assert.ok(shop.products.some((p) => p.path === "/epa-eab"));
      assert.ok(shop.products.some((p) => p.path === "/faa-civil-penalty"));
      assert.ok(shop.products.some((p) => p.path === "/stb-decisions"));
      assert.ok(shop.products.some((p) => p.path === "/oalj-decisions"));
      assert.ok(shop.products.some((p) => p.path === "/fmc-orders"));
      assert.ok(shop.products.some((p) => p.path === "/ftc-orders"));
      assert.ok(shop.products.some((p) => p.path === "/nlrb-decisions"));
      assert.ok(shop.products.some((p) => p.path === "/flra-decisions"));
      assert.ok(shop.products.some((p) => p.path === "/ecab-decisions"));
      assert.ok(shop.products.some((p) => p.path === "/fcc-eb-orders"));
      assert.ok(shop.products.some((p) => p.path === "/nmb-determinations"));
      assert.ok(shop.products.some((p) => p.path === "/eeoc-appellate"));
      ...
      assert.ok(localPaths.includes("/epa-eab"), "local well-known lists /epa-eab");
      assert.ok(localPaths.includes("/faa-civil-penalty"), "local well-known lists /faa-civil-penalty");
      assert.ok(localPaths.includes("/stb-decisions"), "local well-known lists /stb-decisions");
      assert.ok(localPaths.includes("/oalj-decisions"), "local well-known lists /oalj-decisions");
      assert.ok(localPaths.includes("/fmc-orders"), "local well-known lists /fmc-orders");
      assert.ok(localPaths.includes("/ftc-orders"), "local well-known lists /ftc-orders");
      assert.ok(localPaths.includes("/nlrb-decisions"), "local well-known lists /nlrb-decisions");
      assert.ok(localPaths.includes("/flra-decisions"), "local well-known lists /flra-decisions");
      assert.ok(localPaths.includes("/ecab-decisions"), "local well-known lists /ecab-decisions");
      assert.ok(localPaths.includes("/fcc-eb-orders"), "local well-known lists /fcc-eb-orders");
      assert.ok(localPaths.includes("/nmb-determinations"), "local well-known lists /nmb-determinations");
      assert.ok(localPaths.includes("/eeoc-appellate"), "local well-known lists /eeoc-appellate");
      ...
      assert.ok(listBody.result.tools.some((t) => t.name === "epa-eab"), "MCP tools include /epa-eab from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "faa-civil-penalty"), "MCP tools include /faa-civil-penalty from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "stb-decisions"), "MCP tools include /stb-decisions from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "oalj-decisions"), "MCP tools include /oalj-decisions from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "fmc-orders"), "MCP tools include /fmc-orders from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "ftc-orders"), "MCP tools include /ftc-orders from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "nlrb-decisions"), "MCP tools include /nlrb-decisions from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "flra-decisions"), "MCP tools include /flra-decisions from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "ecab-decisions"), "MCP tools include /ecab-decisions from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "fcc-eb-orders"), "MCP tools include /fcc-eb-orders from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "nmb-determinations"), "MCP tools include /nmb-determinations from well-known");
      assert.ok(listBody.result.tools.some((t) => t.name === "eeoc-appellate"), "MCP tools include /eeoc-appellate from well-known");
