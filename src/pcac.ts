...
export function buildPcacManifest(snap: PcacSnapshot | null): Record<string, unknown> {
  ...
  return {
    ...
    name: PRODUCT_NAME,
    free: true,
    note: "Count + substance + date + meeting + mediaId + official source only. Evaluation / weighs-against / we-propose-not-adding text is the paid GET /pcac payload. This free manifest lists the full catalog. One $0.05 GET returns the newest 100 official texts; older pages are another $0.05 on the same URL (page/before).",
    license: LICENSE,
    attribution: ATTRIBUTION,
