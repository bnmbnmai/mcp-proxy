...
export function buildIcoMpnManifest(snap: IcoMpnSnapshot | null): Record<string, unknown> {
  ...
  return {
    ...
    name: PRODUCT_NAME,
    free: true,
    note: "Count + institution + docket + date + official PDF URL only. MPN body is the paid GET /ico-mpn payload. This free manifest lists the full catalog. One $0.05 GET returns the newest 100 official texts; older pages are another $0.05 on the same URL (page/before).",
    license: LICENSE,
    attribution: ATTRIBUTION,
