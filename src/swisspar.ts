...
export function buildSwissparManifest(snap: SwissparSnapshot | null): Record<string, unknown> {
  ...
  return {
    ...
    name: PRODUCT_NAME,
    free: true,
    note: "Count + name + date + MA + official source only. Evaluation / benefit-risk / study-result text is the paid GET /swisspar payload. This free manifest lists the full catalog. One $0.05 GET returns the newest 100 official texts; older pages are another $0.05 on the same URL (page/before).",
    license: LICENSE,
    attribution: ATTRIBUTION,
