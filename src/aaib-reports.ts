...
export const SEARCH_URL =
  "https://www.gov.uk/api/search.json?filter_format=aaib_report&count=20&order=-public_timestamp";
...
export const ATTRIBUTION =
  "Air Accidents Investigation Branch. Contains public sector information licensed under the Open Government Licence v3.0.";
...
export type AaibReportsSnapshot = {
  ...
  reused?: number;
  addedThisRun?: number;
  sources: { index: string; search: string; pdfHost: string };
...
const SYNOPSIS_RE =
  /this page is an index synopsis|GOV\.UK Content API body|INDEX ONLY — title/i;
const HTML_SYNOPSIS_RE = /<p>[\s\S]{80,}<\/p>/i;

export const SEED_LISTINGS: AaibReportListing[] = [
  {
  ...
  },
  {
    ...
    sourceUrl:
      "https://assets.publishing.service.gov.uk/media/6a73474ad7839e24e776398d/Skyranger_Swift_912S_1__G-MLZZ_08-26.pdf",
  },
  {
    ...
    pageUrl:
      "https://www.gov.uk/aaib-reports/aaib-investigation-to-skymagic-x245-uas-registration-n-slash-a",
    sourceUrl:
      "https://assets.publishing.service.gov.uk/media/6a73486e6e8c7d8d7c3c2867/Skymagic_X245_UAS_registration_n_a_08-26.pdf",
  },
  {
    ...
    sourceUrl:
      "https://assets.publishing.service.gov.uk/media/6a4cb0bdc5aa1b17abab40d4/Embraer_190AR_ZS-YAD_08-26.pdf",
  },
  {
    ...
    sourceUrl:
      "https://assets.publishing.service.gov.uk/media/6a47bede732d8e7ce5f53cf3/Jodel_D18_G-BODT_08-26.pdf",
  },
  {
    ...
    sourceUrl:
      "https://assets.publishing.service.gov.uk/media/6a478997732d8e7ce5f53c07/Boeing_737-8_G-CRUX.pdf",
  },
  {
    ...
    sourceUrl:
      "https://assets.publishing.service.gov.uk/media/6a4789ca8effd97622f53bfb/Westland_Scout_AH1_G-BWHU_07-26.pdf",
  },
  {
    ...
    pageUrl:
      "https://www.gov.uk/aaib-reports/aaib-investigation-to-de-havilland-aircraft-of-canada-limited-dhc-8-9h-lwb",
    sourceUrl:
      "https://assets.publishing.service.gov.uk/media/6a42410db9f58f0eb4c13f14/De_Havilland_Aircraft_of_Canada_Limited_DHC-8_9H-LWB_07-26.pdf",
  },
  {
    ...
    sourceUrl:
      "https://assets.publishing.service.gov.uk/media/6a1eae0559fb7a60f827f5fe/Dyn_Aero_MCR-01_G-TOMX_07-26.pdf",
  },
  {
    ...
    sourceUrl:
      "https://assets.publishing.service.gov.uk/media/6a1da728b95db968c8f3bd0e/MW6-1-1_G-MNMW_06-26.pdf",
  },
];
...
export function aaibReportsDir(): string {
  if (env("AAIB_REPORTS_DIR")) return resolve(env("AAIB_REPORTS_DIR"));
...
export function officialAaibPageUrl(raw: string | null | undefined): string | null {
  if (!raw) return null;
  try {
    const parsed = new URL(raw.trim(), "https://www.gov.uk");
    if (!OFFICIAL_PAGE_HOSTS.has(parsed.hostname.toLowerCase())) return null;
    const path = parsed.pathname.replace(/\/+$/, "") || "/";
    if (!PAGE_PATH_RE.test(path)) return null;
    if (ANNUAL_REVIEW_RE.test(path) || /\/raib-reports\//i.test(path)) return null;
...
export function officialAaibReportPdfUrl(urlOrPath: string | null | undefined): string | null {
  if (!urlOrPath) return null;
  try {
    const parsed = new URL(urlOrPath.trim(), PDF_ORIGIN);
    const host = parsed.hostname.toLowerCase();
    if (host === "web.archive.org" || host === "www.raib.gov.uk" || host === "raib.gov.uk") return null;
    if (!OFFICIAL_PDF_HOSTS.has(host)) return null;
    const path = decodeURIComponent(parsed.pathname);
    if (GLOSSARY_RE.test(path) || ANNUAL_REVIEW_RE.test(path)) return null;
    const media = path.match(MEDIA_RE) || parsed.pathname.match(MEDIA_RE);
    if (!media) return null;
    if (GLOSSARY_RE.test(media[2])) return null;
...
export function isOfficialAaibReportPdf(url: string | null | undefined): boolean {
...
export function isAnnualReviewRow(row: Pick<AaibReportListing, "id" | "title" | "pageUrl">): boolean {
  return ANNUAL_REVIEW_RE.test(`${row.id} ${row.title} ${row.pageUrl}`);
}

export function isGlossaryRow(row: Pick<AaibReportListing, "title" | "sourceUrl">): boolean {
  return GLOSSARY_RE.test(`${row.title} ${row.sourceUrl}`);
}

export function isRaibRow(row: Pick<AaibReportListing, "title" | "pageUrl" | "sourceUrl">): boolean {
  return /\/raib-reports\//i.test(`${row.pageUrl} ${row.sourceUrl}`) || /\bRAIB investigation\b/i.test(row.title);
}

export function isPeopleRow(row: Pick<AaibReportListing, "title" | "id" | "aircraft">): boolean {
  return PEOPLE_RE.test(`${row.title} ${row.id} ${row.aircraft}`);
}

export function parseListingRows(rows: AaibReportListing[]): AaibReportListing[] {
  const found: AaibReportListing[] = [];
  const seen = new Set<string>();
  for (const row of rows) {
    if (isAnnualReviewRow(row) || isGlossaryRow(row) || isRaibRow(row) || isPeopleRow(row)) continue;
...
export function parseSearchJson(raw: string): AaibReportListing[] {
  let parsed: unknown;
  try {
...
export function parseContentDocument(raw: string): AaibReportListing | null {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch {
  ...
  const details = (doc.details && typeof doc.details === "object" ? doc.details : {}) as Record<string, unknown>;
  const metadata = (details.metadata && typeof details.metadata === "object" ? details.metadata : {}) as Record<
    string,
    unknown
  >;
  ...
  for (const att of attachments) {
    ...
    if (!url || GLOSSARY_RE.test(title)) continue;
    sourceUrl = url;
    break;
  }
...
export function isIndexTeaserDump(text: string): boolean {
  ...
  if (SYNOPSIS_RE.test(t)) return true;
  if (HTML_SYNOPSIS_RE.test(t) && !/AAIB Bulletin/i.test(t)) return true;
  if (/Index only — title \/ registration \/ date \/ PDF URL/i.test(t)) return true;
  return false;
}

export function isRaibDump(text: string): boolean {
  return RAIB_RE.test(text) && !/AAIB Bulletin/i.test(text);
}

export function isAnnualReviewDump(text: string): boolean {
  return ANNUAL_REVIEW_RE.test(text) && !/Aircraft Type and Registration/i.test(text);
}

export function isGlossaryDump(text: string): boolean {
  return /Glossary of abbreviations/i.test(text) && !/Aircraft Type and Registration/i.test(text);
}

export function isCadorsOrNoAuthDump(text: string): boolean {
  if (/"CADORS"\s*:/.test(text)) return true;
  if (/"raw_text"/.test(text) && /edgar|usgs|nwis/i.test(text)) return true;
  return false;
}
...
export function isRealAaibReportBody(text: string): boolean {
...
export function parseAaibReportText(
  text: string,
  meta: Partial<AaibReportListing> & { sourceUrl: string },
): AaibReportCard {
...
export function emptyAaibReportsSnapshot(reason: string): AaibReportsSnapshot {
  return {
    ok: true,
    product: PRODUCT_ID,
    status: "empty",
...
export function assembleAaibReportsSnapshot(
  cards: AaibReportCard[],
  fetchedAt = new Date().toISOString(),
): AaibReportsSnapshot {
...
function parseSnapshotFile(raw: unknown): AaibReportsSnapshot | null {
  if (!raw || typeof raw !== "object") return null;
  const snap = raw as AaibReportsSnapshot;
  if (snap.product !== PRODUCT_ID || !Array.isArray(snap.cards)) return null;
  return snap;
}

export function readAaibReportsSnapshot(): AaibReportsSnapshot | null {
  ...
  if (env("AAIB_REPORTS_DIR")) return null;
...
export function writeAaibReportsSnapshot(snap: AaibReportsSnapshot): void {
  const path = snapshotPath();
  mkdirSync(dirname(path), { recursive: true });
  writeFileSync(path, `${JSON.stringify(snap, null, 2)}\n`);
}

export async function fetchAaibBytes(url: string): Promise<Uint8Array> {
...
export async function fetchAaibText(url: string): Promise<string> {
...
async function loadOfficialListings(
  dir: string,
): Promise<{ listed: AaibReportListing[]; listedCount: number }> {
  if (dir) {
  ...
  try {
    ...
    const enriched: AaibReportListing[] = [];
...
export async function collectAaibReports(opts?: {
  ...
  const prior = new Map<string, AaibReportCard>();
  for (const card of readAaibReportsSnapshot()?.cards ?? []) {
    if (isRealAaibReportBody(card.body)) prior.set(card.id, card);
  }
  const cards: AaibReportCard[] = [];
  const seen = new Set<string>();
  let fetchedPdfs = 0;
  let skippedNoText = 0;
  let reused = 0;
...
export async function loadAaibReports(): Promise<AaibReportsSnapshot> {
  ...
  try {
    ...
    return emptyAaibReportsSnapshot(
      `AAIB investigation-report PDFs are not on this host and live fetch failed. ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}

export function buildAaibReportsManifest(snap: AaibReportsSnapshot | null): Record<string, unknown> {
  ...
  return {
    ...
    note: paidBodyCatalogNote(
      AAIB_REPORTS_PATH,
      "Count + title + registration + aircraft + date + official GOV.UK page + PDF URL only. Investigation body is the paid GET /aaib-reports payload. This free manifest lists the full catalog. GOV.UK Content API / Search synopsis is not sold. Glossary PDFs and the Annual Safety Review are skipped. Distinct from RAIB.",
    ),
    license: LICENSE,
    attribution: ATTRIBUTION,
    payTo: PAY_TO,
...
export function filterAaibReportsManifest(
  ...
  const matched = cards.filter((raw) => {
    ...
    return ["id", "registration", "aircraft", "date", "title", "pageUrl", "sourceUrl"].some((k) =>
      String(row[k] ?? "")
        .toLowerCase()
        .includes(needle),
    );
...
export async function loadAaibReportsManifest(q?: string): Promise<Record<string, unknown>> {
  return filterAaibReportsManifest(buildAaibReportsManifest(readAaibReportsSnapshot()), q);
}

export function buildAaibReportsPaidPage(
  snap: AaibReportsSnapshot | null,
