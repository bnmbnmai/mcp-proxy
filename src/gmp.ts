...
export function buildGmpManifest(snap: GmpSnapshot | null): Record<string, unknown> {
  ...
  return {
    ...
    name: PRODUCT_NAME,
    free: true,
    note: "Count + id + firm + dates + rating only. Observation text / C.02 cites are the paid GET /gmp payload. This free manifest lists the full catalog. One $0.05 GET returns the newest 100 official texts; older pages are another $0.05 on the same URL (page/before).",
    license: OGL_NOTE,
